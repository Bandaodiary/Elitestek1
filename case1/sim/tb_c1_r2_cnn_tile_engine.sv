`timescale 1ns/1ps
module tb_c1_r2_cnn_tile_engine;
    ...
    always @(posedge clk) begin
        ...
        if(rst) begin held=0;weight_pending=0;end
            ...
            if(checking && start_valid && start_ready) begin
                ...
                job_groups=mode==1 ? 1 : start_channels/8;job_reads=0;job_windows=0;job_channels=start_channels;job_outputs=start_outputs;job_weight_reads=0;job_feature_reads=0;
            ...
            if(checking && out_valid && out_ready) begin
                ...
                expected_vectors=job_mode==0 ? (job_size*job_outputs+5)/6 : job_mode==1 ? (job_size+1)/2 : job_mode==2 ? ((job_size+1)/2)*job_groups*3 : (job_size+5)/6;
                ...
                if(out_last) begin
                    expected_beats=expected_vectors*(job_mode==0 ? (job_channels+15)/16 : job_mode==1 ? 5 : 1);
                    expected_windows=(job_mode==1 || job_mode==2) ? ((job_size+1)/2)*job_groups : 0;
                    if(job_vectors!=expected_vectors || job_beats!=expected_beats || job_windows!=expected_windows || job_reads!=expected_windows*2 || job_weight_reads!=(job_mode==3 ? 0 : expected_beats) || job_feature_reads!=((job_mode==0 || job_mode==3) ? expected_beats : 0))
                        $fatal(1,"CNN wrong compute/RAM count vectors=%0d beats=%0d windows=%0d reads=%0d",job_vectors,job_beats,job_windows,job_reads);
                    offset=(job_mode==0 || job_mode==3) ? 13 : 16;
                    if(!STALLS && (cycles-job_start!=job_beats+offset || last_output-first_output!=(job_vectors-1)*(job_mode==0 ? (job_channels+15)/16 : job_mode==1 ? 5 : 1)))
                    ...
                    $display("C1_R2_TILE_JOB stalls=%0d job=%0d mode=%0d size=%0d channels=%0d outputs=%0d groups=%0d vectors=%0d mac_beats=%0d windows=%0d ram_reads=%0d weight_reads=%0d feature_reads=%0d cycles=%0d",STALLS,jobs,job_mode,job_size,job_channels,job_outputs,job_groups,job_vectors,job_beats,job_windows,job_reads,job_weight_reads,job_feature_reads,cycles-job_start);
                    jobs=jobs+1;
                end
            end
        end
    end
    task automatic reject_start(input [1:0] mm,input integer size,channels,input integer outputs=8);
        @(negedge clk);mode=mm;start_size=size;start_channels=channels;start_outputs=outputs;start_valid=1;
    ...
    initial begin
        ...
        for(integer i=0;i<n;i=i+1) begin
            ...
            if(commands[i][51:48]<4) begin
                ...
                start_size=commands[i][13:0];start_channels=commands[i][19:14];start_outputs=commands[i][27:22];row_top=commands[i][20];row_bottom=commands[i][21];
                ...
                @(negedge clk);mode=mode+1;start_size=1;start_channels=7;start_outputs=12;row_top=~row_top;row_bottom=~row_bottom;
                load_valid=1;load_kind=3;load_addr=0;load_data=0;
                while(busy) begin
                    if(load_ready || start_ready) $fatal(1,"CNN accepted active mutation");
                    @(negedge clk);mode=mode+1;start_channels=start_channels+1;start_outputs=start_outputs+1;
                end
        ...
        if(received!=m || jobs!=j || mode_changes<30 || busy || out_valid || dut.compute_busy || busy_rejections<20 ||
           !pw_reset_tested || parameter_checks<20 || zero_masks<1 || checked_windows<20 || simultaneous_push_pop<20 || (STALLS && blocked<20)) $fatal(1,"CNN incomplete verification");
        $display("C1_R2_TILE_PASS stalls=%0d jobs=%0d vectors=%0d mode_changes=%0d blocked=%0d busy_rejections=%0d windows=%0d zero_masks=%0d full_slots=%0d simultaneous_push_pop=%0d parameter_checks=%0d reset_inflight=4 invalid_commands=24",STALLS,jobs,received,mode_changes,blocked,busy_rejections,checked_windows,zero_masks,full_slots,simultaneous_push_pop,parameter_checks);
        $finish;
    end
endmodule
