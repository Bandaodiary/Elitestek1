`timescale 1ns/1ps
module tb_c1_r2_cnn_compact_engine;
            ...
            if(checking)begin
                ...
                if(out_valid && out_ready)begin : check_result
                    ...
                    if(out_last)begin
                        ...
                        $display("C1_R2_COMPACT_OPERATOR_JOB stalls=%0d job=%0d mode=%0d size=%0d channels=%0d outputs=%0d vectors=%0d mac_beats=%0d cycles=%0d",STALLS,jobs,job_mode,job_size,job_channels,job_outputs,job_vectors,job_beats,cycles-job_start);
    ...
    initial begin
        ...
        if(BOUNDARY_ONLY)begin
            ...
            $display("C1_R2_COMPACT_OPERATOR_BOUNDARY_PASS legal_groups=3 overflow_rejected=1");$finish;
        end
        ...
        $display("C1_R2_COMPACT_OPERATOR_PASS stalls=%0d jobs=%0d vectors=%0d bulk_writes=%0d parameter_reads=%0d reset_modes=%0d busy_rejections=%0d held_cycles=%0d packed_weights=1 lanes=6",STALLS,jobs,received,bulk_writes,parameter_reads,reset_tests,rejections,held_cycles);
        $finish;
    end
endmodule
