`timescale 1ns/1ps
module tb_c1_r2_bound_plan_graph;
    ...
    c1_r2_planned_pingpong_graph #(.USE_ROW_CACHE(USE_ROW_CACHE),.OVERLAP_WRITE(OVERLAP_WRITE),.OVERLAP_COMPUTE(OVERLAP_COMPUTE),.PRIORITIZE_REFILL(PRIORITIZE_REFILL)) dut(.*);
    ...
    always @(posedge clk) begin : memory_model
        ...
        if(rst) begin
            ...
            if(stage_done) begin
                ...
                $display("C1_R2_BOUND_GRAPH_STAGE_COMMIT frame=%0d fault=%0d stage=%0d cycles=%0d words=%0d",frame_id,fault_mode,stage_done_index,frame_cycles,expected_index-frame_expected_base);
            end
    ...
    task run_frame(input integer fid,input integer fault);
        ...
        begin
            ...
            if(fault==0) begin
                ...
                $display("C1_R2_BOUND_GRAPH_FAULT stalls=%0d fault=%0d commits=%0d drained=1 overlapped_error=%0d compute_error=%0d two_pages_error=%0d cycles=%0d",STALLS,fault,commits,fault_overlap_seen,fault_compute_seen,fault_two_pages_seen,frame_cycles);
            end
    ...
    task reset_frame(input integer phase);
        ...
        begin
            ...
            run_frame(1,0);
            $display("C1_R2_BOUND_GRAPH_RESET_PASS phase=%0d restart_golden=1 cleared_pages=1",phase);
        end
    endtask
    initial begin
        ...
        if(RESET_PROBE) begin
            ...
            $display("C1_R2_BOUND_GRAPH_RESET_SUITE_PASS phases=6 normal_restarts=6 system_reset_only=1");$finish;
        end
