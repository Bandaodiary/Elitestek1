`timescale 1ns/1ps
module tb_c1_r2_row_fused_faults;
    ...
    integer shadow_rows=0,pw_rows=0,pw_packets=0;
        ...
        if(rst) begin
            ...
            if(reading && (!rd_valid || rd_ready) && !(rd_valid && rd_ready && rd_last)) begin
                if(read_delay!=0) read_delay<=read_delay-1;
                    ...
                    if(!fault_injected) begin
                        if(active_stage==FUSED_DW_STAGE && ipos==0 &&
                           ((fault_mode==13 && raddr[31:23]==5 && !dut.fusion_pw_q) ||
                            (fault_mode==14 && raddr[31:23]==5 && dut.fusion_pw_q) ||
                            (fault_mode==15 && raddr[31:23]!=5 && shadow_rows>=2)))begin rd_error<=1;inject=1;end
            ...
            if(response_pending && !wr_response_valid) begin
                if(response_delay!=0) response_delay<=response_delay-1;
                    ...
                    if(!fault_injected && fault_mode==16 && active_stage==FUSED_DW_STAGE)begin
                        wr_response_error<=1;fault_injected<=1;
                    end
    ...
    always @(posedge clk) if(!rst)begin
        ...
        if(dut.fusion_active && dut.fusion_pw_q && dut.op_valid && dut.op_ready)pw_packets=pw_packets+1;
    ...
    task prepare_frame(input integer fid,input integer fault);
        begin
            ...
            dw_index=fid*(ndw/2);dw_frame_base=dw_index;shadow_rows=0;pw_rows=0;pw_packets=0;shadow_corrupted=0;
    ...
    task run_frame(input integer fid,input integer fault);
        ...
        begin
            ...
            if(fault==0) begin
                ...
                if(fault==11 && !fault_compute_seen || (fault==11 || fault==12) && !fault_two_pages_seen) $fatal(1,"missing two-page/compute error drain");
                if(fault>=13 && commits!=FUSED_DW_STAGE)$fatal(1,"fused failure committed logical DW/PW or missed target");
        ...
        begin
            ...
            while(!reached && guard<200000) begin
                ...
                case(phase)
                    ...
                    6:reached=dut.fusion_active && !dut.fusion_pw_q && dut.op_busy && dw_index-dw_frame_base>=6;
                    7:reached=dut.fusion_active && dut.fusion_pw_q && dut.op_busy && pw_packets>=3;
                    8:reached=dut.fusion_active && !dut.fusion_pw_q && dut.row_y>=2 && dut.state==3 && reading;
                endcase
            end
    ...
    initial begin
        ...
        run_frame(0,0);
        for(integer f=13;f<=16;f=f+1)begin
            run_frame(0,f);
            run_frame(1,0); // Fresh poison + new input, no reset after drained error.
        end
        for(integer phase=6;phase<=8;phase=phase+1)reset_frame(phase);
        if(faults_passed!=4)$fatal(1,"missing fusion fault coverage");
        $display("C35_ROW_FUSED_FAULTS_PASS faults=4 normal_frames=8 fused_resets=3 same_instance_recovery=1 system_reset_only=1 actual_AXI=0 native_fps_claim=0");
        $finish;
    end
endmodule
