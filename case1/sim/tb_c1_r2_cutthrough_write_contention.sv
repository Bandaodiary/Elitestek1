`timescale 1ns/1ps
// Independent C32 integration probe: real writer -> RGBX/AXI DMA -> retained
...
module tb_c1_r2_cutthrough_write_contention;
    parameter integer CANDIDATE=1,MODE=2,CHANNELS=24,WIDTH=32;
    ...
    reg cap_last=0;
    reg [70:0] packets_mem[0:8191];
    reg [127:0] expected_mem[0:2047];
    reg seen_nn[0:2047],seen_cap[0:63];
    ...
    wire stream_enable=cycle%83<70;
...
`define C32_WRITER_PORTS \
    ...
    generate if(CANDIDATE)begin : g_candidate
        c1_r2_tensor_cutthrough_writer u_writer (`C32_WRITER_PORTS);
    end else begin : g_reference
        c1_r2_tensor_pingpong_writer u_writer (`C32_WRITER_PORTS);
    end endgenerate
`undef C32_WRITER_PORTS
    c1_r2_rgbx_row_write #(.BURST_BEATS(16),.OUTSTANDING(4)) nn_dma (
    ...
    always @(posedge clk)if(!rst)begin : monitor
        ...
        cycle<=cycle+1;
        if(cycle>100000)$fatal(1,"write contention timeout");
    ...
    initial begin : cnn_source
        ...
        if(PACKETS*NN_ROWS>8192 || PHYSICAL_WORDS*NN_ROWS>2048 || CAPTURE_ROWS!=4)$fatal(1,"contention fixture capacity");
        ...
        for(integer r=0;r<NN_ROWS;r=r+1)begin
            ...
            for(integer p=0;p<PACKETS;p=p+1)begin
                packet=packets_mem[r*PACKETS+p];nn_valid=1;
        ...
        if((CANDIDATE && (early_words==0 || blocked_empty_head==0)) || (!CANDIDATE && early_words!=0))$fatal(1,"candidate early-write/contention coverage absent");
        $display("C32_WRITE_AXI_PASS candidate=%0d mode=%0d channels=%0d width=%0d aw_wait_w=%0d inject_b=%0d nn_rows=%0d capture_rows=%0d nn_words=%0d capture_words=%0d aw=%0d b=%0d early_words=%0d blocked_empty_head=%0d max_empty_run=%0d max_nn_w_occupancy=%0d capture_command_wait_max=%0d capture_due_wait_max=%0d nn_admit_first_w_max=%0d nn_span_cycles=%0d nn_errors=%0d capture_errors=%0d actual_AXI=1 actual_camera=0 whole_CNN=0",CANDIDATE,MODE,CHANNELS,WIDTH,AW_WAIT_W,INJECT_B,NN_ROWS,CAPTURE_ROWS,nn_words_seen,cap_words_seen,memory_aw,memory_b,early_words,blocked_empty_head,max_empty_run,max_nn_w_occupancy,capture_command_wait_max,capture_due_wait_max,nn_admit_first_w_max,nn_last_done-nn_first_start,nn_errors,cap_errors);
        $finish;
    end
