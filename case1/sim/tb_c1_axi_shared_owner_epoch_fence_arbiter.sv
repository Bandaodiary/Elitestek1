...
// Integration gate between the real ID-less serial arbiter and the
// owner/epoch fence.  The source-side mask keeps the currently selected
// arbiter owner alive during a fence, while blocking a second idle owner.
module tb_c1_axi_shared_owner_epoch_fence_arbiter;
    ...
    logic arb_read_busy, arb_read_quiescent;
    logic arb_write_busy, arb_write_quiescent;
    logic [INDEX_W-1:0] arb_read_owner, arb_write_owner;
    ...
    logic raw_arvalid0 = 1'b0, raw_arvalid1 = 1'b0;
    logic raw_awvalid0 = 1'b0, raw_wvalid0 = 1'b0;
    logic raw_arready0, raw_arready1, raw_awready0, raw_wready0;
    logic raw_rready0 = 1'b0, raw_bready0 = 1'b0;
    logic r_pending_q = 1'b0, aw_seen_q = 1'b0, w_seen_q = 1'b0;
    logic b_pending_q = 1'b0;
    integer cycles = 0;
    integer ar_accepts = 0, aw_accepts = 0, r_accepts = 0, b_accepts = 0;

    // A selected owner remains admitted while a maintenance fence drains it.
    // An idle second owner is masked as soon as read_admit/write_admit drops.
    always_comb begin
        s_arvalid = '0;
        s_arvalid[0] = raw_arvalid0 &&
                       (read_admit ||
                        (arb_read_busy && (arb_read_owner == 1'b0)));
        s_arvalid[1] = raw_arvalid1 &&
                       (read_admit ||
                        (arb_read_busy && (arb_read_owner == 1'b1)));
        s_awvalid = '0;
        s_awvalid[0] = raw_awvalid0 &&
                       (write_admit ||
                        (arb_write_busy && (arb_write_owner == 1'b0)));
        s_wvalid = '0;
        s_wvalid[0] = raw_wvalid0 &&
                      (write_admit ||
                       (arb_write_busy && (arb_write_owner == 1'b0)));

        s_rready = '0;
        s_rready[0] = raw_rready0;
        s_bready = '0;
        s_bready[0] = raw_bready0;
        raw_arready0 = s_arready[0];
        raw_arready1 = s_arready[1];
        raw_awready0 = s_awready[0];
        raw_wready0 = s_wready[0];
    end

    c1_axi_n_serial_arbiter_128 #(
        .CLIENTS(CLIENTS), .READ_RESPONSE_SKID(0)
    ) u_arbiter (
        .clk(clk), .rst(rst),
        ...
        .m_rready(m_rready), .read_busy(arb_read_busy),
        .read_quiescent(arb_read_quiescent), .write_busy(arb_write_busy),
        .write_quiescent(arb_write_quiescent), .read_owner(arb_read_owner),
        .write_owner(arb_write_owner)
    );
    ...
    c1_axi_shared_owner_epoch_fence #(
        ...
        .read_busy(arb_read_busy), .read_quiescent(arb_read_quiescent),
        .write_busy(arb_write_busy), .write_quiescent(arb_write_quiescent),
        .read_fire(m_arvalid && m_arready),
        .write_fire(m_awvalid && m_awready),
    ...
    // Minimal one-beat-per-transaction downstream model.  R/B are registered
    // so the arbiter never sees a same-cycle fall-through response.
    ...
    task automatic fail(input string message);
        begin
            $display("C1_AXI_SHARED_OWNER_EPOCH_FENCE_ARBITER_FAIL %s", message);
            $fatal(1, "%s", message);
        end
    endtask
    ...
    task automatic issue_read0;
        begin
            @(negedge clk);
            raw_arvalid0 <= 1'b1;
            while (!raw_arready0) @(negedge clk);
            @(posedge clk);
            @(negedge clk);
            raw_arvalid0 <= 1'b0;
        end
    ...
    task automatic issue_write0;
        begin
            @(negedge clk);
            raw_awvalid0 <= 1'b1;
            while (!raw_awready0) @(negedge clk);
            @(posedge clk);
            @(negedge clk);
            raw_awvalid0 <= 1'b0;
            raw_wvalid0 <= 1'b1;
            while (!raw_wready0) @(negedge clk);
            @(posedge clk);
            @(negedge clk);
            raw_wvalid0 <= 1'b0;
        end
    ...
    initial begin
        ...
        if (!context_active || current_epoch !== 4'd0 ||
            ...
            fail("context start did not open arbiter admissions");

        // Hold the selected read response while abort closes admissions.
        issue_read0();
        repeat (3) @(posedge clk);
        if (!arb_read_busy || arb_read_quiescent)
            fail("arbiter did not expose an active read owner");
        @(negedge clk);
        raw_arvalid1 <= 1'b1;
        abort_req <= 1'b1;
        #1;
        if (read_admit || write_admit || raw_arready1)
            fail("abort did not block a new idle read owner");
        @(posedge clk);
        #1;
        if (!fence_busy || !arb_read_busy)
            fail("abort fence lost the selected stalled read owner");
        @(negedge clk);
        raw_rready0 <= 1'b1;
        raw_arvalid1 <= 1'b0;
        @(posedge clk);
        #1;
        // One additional cycle lets the arbiter publish quiescent to the
        // fence after the terminal R handshake.
        @(posedge clk);
        #1;
        if (!abort_done || !epoch_bump || current_epoch !== 4'd1 ||
            context_active || protocol_error || ar_accepts != 1 ||
            r_accepts != 1)
            fail("abort did not drain the selected read owner atomically");
        @(negedge clk);
        abort_req <= 1'b0;

        pulse_start();
        if (!context_active || current_epoch !== 4'd1)
            fail("restart after read abort failed");

        // Repeat the same check on the write owner and B response.
        raw_bready0 <= 1'b0;
        issue_write0();
        repeat (2) @(posedge clk);
        if (!arb_write_busy || arb_write_quiescent)
            fail("arbiter did not expose an active write owner");
        @(negedge clk);
        flush_req <= 1'b1;
        #1;
        if (read_admit || write_admit)
            fail("flush did not block new owners");
        @(posedge clk);
        #1;
        if (!fence_busy || !arb_write_busy)
            fail("flush fence lost the selected write owner");
        @(negedge clk);
        raw_bready0 <= 1'b1;
        flush_req <= 1'b0;
        @(posedge clk);
        ...
        if (!flush_done || !epoch_bump || current_epoch !== 4'd2 ||
            !context_active || protocol_error || aw_accepts != 1 ||
            b_accepts != 1)
            fail("flush did not drain the selected write owner atomically");

        $display("C1_AXI_SHARED_OWNER_EPOCH_FENCE_ARBITER_PASS ar=%0d r=%0d aw=%0d b=%0d epoch=%0d cycles=%0d",
