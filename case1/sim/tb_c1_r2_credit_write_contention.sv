`timescale 1ns/1ps
// C33 three-version integration probe: real writer -> RGBX/AXI DMA -> retained
...
module tb_c1_r2_credit_write_contention;
    parameter integer FORCE_REFILL=0,BREAK_DRAIN=0,BREAK_CREDIT=0;
    parameter integer CANDIDATE=2,MODE=2,CHANNELS=24,WIDTH=32;
...
`define C33_WRITER_PORTS \
    ...
    generate if(CANDIDATE==2)begin : g_credit
...
`undef C33_DMA_PORTS
    ...
    initial begin : cnn_source
        ...
        $display("C33_WRITE_AXI_PASS candidate=%0d mode=%0d channels=%0d width=%0d aw_wait_w=%0d inject_b=%0d nn_rows=%0d capture_rows=%0d nn_words=%0d capture_words=%0d aw=%0d b=%0d early_words=%0d blocked_empty_head=%0d max_empty_run=%0d max_nn_w_occupancy=%0d capture_command_wait_max=%0d capture_due_wait_max=%0d nn_admit_first_w_max=%0d nn_span_cycles=%0d nn_errors=%0d capture_errors=%0d actual_AXI=1 actual_camera=0 whole_CNN=0 force_refill=%0d refill_fetches=%0d refill_wlast=%0d",CANDIDATE,MODE,CHANNELS,WIDTH,AW_WAIT_W,INJECT_B,NN_ROWS,CAPTURE_ROWS,nn_words_seen,cap_words_seen,memory_aw,memory_b,early_words,blocked_empty_head,max_empty_run,max_nn_w_occupancy,capture_command_wait_max,capture_due_wait_max,nn_admit_first_w_max,nn_last_done-nn_first_start,nn_errors,cap_errors,FORCE_REFILL,refill_fetches,refill_wlast);
        $finish;
    end
