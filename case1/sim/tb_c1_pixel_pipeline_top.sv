...
module tb_c1_pixel_pipeline_top;
    localparam integer INPUT_WIDTH = 18;
    localparam integer INPUT_HEIGHT = 14;
    localparam integer RGB_WIDTH = 8;
    localparam integer RGB_HEIGHT = 6;
    localparam integer OUTPUT_WIDTH = RGB_WIDTH-4;
    localparam integer OUTPUT_HEIGHT = RGB_HEIGHT-4;
    localparam integer IN_X_BITS = $clog2(INPUT_WIDTH);
    localparam integer IN_Y_BITS = $clog2(INPUT_HEIGHT);
    localparam integer OUT_X_BITS = $clog2(RGB_WIDTH);
    localparam integer OUT_Y_BITS = $clog2(RGB_HEIGHT);
    ...
    c1_pixel_pipeline_top #(
        .INPUT_WIDTH(INPUT_WIDTH), .INPUT_HEIGHT(INPUT_HEIGHT),
        .OUTPUT_WIDTH(RGB_WIDTH), .OUTPUT_HEIGHT(RGB_HEIGHT),
        .X_OFFSET(1), .Y_OFFSET(1), .H_STEP(2), .V_STEP(2)
    ) dut (
        .clk, .rst, .in_valid, .in_sof, .in_eol, .in_eof, .in_x, .in_y,
        .in_raw10, .isp_cfg_we(1'b0), .isp_cfg_addr('0), .isp_cfg_wdata('0),
        .cnn_cfg_we(1'b0), .cnn_cfg_addr('0), .cnn_cfg_wdata('0),
        .out_valid, .out_sof, .out_eol, .out_eof, .out_x, .out_y, .out_rgb
    );

    initial begin
        $readmemh("../vectors/isp_input_raw10.hex", input_memory);
        $readmemh("../vectors/system_expected_rgb24.hex", expected_memory);
        if ((^input_memory[0] === 1'bx) || (^expected_memory[0] === 1'bx))
            $fatal(1, "system vector files were not loaded");
        ...
        repeat (5) @(negedge clk);
        $display("C1_PIXEL_SYSTEM_PASS input=%0d output=%0d cycles=%0d",
                 INPUT_PIXELS, output_index, cycles);
        $finish;
    end
    ...
    always @(negedge clk) begin
        ...
        if (!rst && out_valid) begin
            ...
            if (out_x !== expected_x[OUT_X_BITS-1:0] || out_y !== expected_y[OUT_Y_BITS-1:0])
                $fatal(1, "system coordinate mismatch idx=%0d", output_index);
            if (out_rgb !== expected_memory[output_index])
                $fatal(1, "system pixel mismatch idx=%0d got=%h expected=%h",
                       output_index, out_rgb, expected_memory[output_index]);
            if (out_sof !== (output_index == 0))
                $fatal(1, "system SOF mismatch idx=%0d", output_index);
            if (out_eol !== ((output_index % OUTPUT_WIDTH) == OUTPUT_WIDTH-1))
                $fatal(1, "system EOL mismatch idx=%0d", output_index);
            if (out_eof !== (output_index == OUTPUT_PIXELS-1))
                $fatal(1, "system EOF mismatch idx=%0d", output_index);
            output_index = output_index + 1;
        end
        if (cycles > 5000)
            $fatal(1, "system timeout output_index=%0d", output_index);
    end
endmodule
