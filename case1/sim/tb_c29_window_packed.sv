...
module tb_c29_window_packed;
    ...
    c1_r2_overlay_window_store_packed dut(.*);
    ...
    always @(posedge clk)if(!rst)begin
        ...
        if(req_ready!==old_req_ready || out_valid!==old_out_valid ||
           (out_valid && {out_window,out_x,out_group}!=={old_out_window,old_out_x,old_out_group}))
            $fatal(1,"C29 window cycle-equivalence mismatch");
        if(was_held && (!out_valid || {out_window,out_x,out_group}!==held_payload))$fatal(1,"C29 window held output changed");
    ...
    initial begin
        ...
        for(wi=0;wi<6;wi=wi+1)for(gi=0;gi<4;gi=gi+1)for(up=0;up<2;up=up+1)begin
            case(wi)0:w=1;1:w=2;2:w=17;3:w=340;4:w=640;default:w=1024;endcase
            case(gi)0:groups=1;1:groups=2;2:groups=3;default:groups=6;endcase
            physical_width=up ? w/2 : w;
            if(!(up && w%2) && ((physical_width+1)/2)*groups<=1024)begin
        ...
        drain();
        if(held==0 || configs<20 || pushed!=popped)$fatal(1,"C29 window coverage missing");
        $display("C29_WINDOW_PACKED_PASS configurations=%0d requests=%0d bulk_writes=%0d held=%0d cycle_equivalent=1 independent_golden=1",configs,popped,writes,held);
        $finish;
    end
    initial begin #10000000;$fatal(1,"C29 window timeout");end
endmodule
