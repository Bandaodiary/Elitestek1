`timescale 1ns/1ps
module tb_c1_r2_weight_store8_tdp;
    ...
    wire [1023:0] old_w,new_w;
    wire [255:0] old_b,new_b;
    wire [199:0] old_a,new_a;
    c1_r2_weight_store8 baseline(.clk(clk),.rst(rst),.load_valid(load_valid),.load_kind(load_kind),
    ...
    c1_r2_weight_store8_tdp dut(.clk(clk),.rst(rst),.load_valid(load_valid),.load_kind(load_kind),
        .load_addr(load_addr),.load_data(load_data),.read_en(read_en),.read_addr(read_addr),
        .read_weights(new_w),.read_bias(new_b),.read_affine(new_a));
    // Independent logical bank/row/word model; no physical paired addressing.
    ...
    task check_held;
        begin
            if(have_read)begin
                if(old_w!==saved_w || new_w!==saved_w || old_b!==saved_b || new_b!==saved_b ||
    ...
    task get(input [47:0] addresses);
        ...
        begin
            ...
            for(integer bank=0;bank<8;bank=bank+1)begin
                ...
                for(integer word_id=0;word_id<4;word_id=word_id+1)
                    if(old_w[bank*128+word_id*32+:32]!==weights[bank][row][word_id] ||
                       new_w[bank*128+word_id*32+:32]!==weights[bank][row][word_id])
            ...
            saved_w=new_w;saved_b=new_b;saved_a=new_a;have_read=1;reads=reads+1;
        end
    endtask
    task reset_consumers;
        begin
            @(negedge clk);load_valid=0;read_en=0;rst=1;
            repeat(2)begin @(posedge clk);#1;check_held;end
            @(negedge clk);rst=0;resets=resets+1;
        end
    endtask
    reg [47:0] addresses;
    integer address;
    initial begin
        ...
        for(integer row=0;row<48;row=row+1)begin
            for(integer bank=0;bank<8;bank=bank+1)addresses[bank*6+:6]=(row+7*bank)%48;
            get(addresses);
        end
        ...
        if(reads!=1048 || loads!=2134 || resets!=11)$fatal(1,"wrong directed/random coverage");
        $display("C1_R2_WEIGHT_TDP_PASS reads=%0d loads=%0d resets=%0d hold_checks=%0d logical_banks=8 legal_rows=48 weight_words=1536 checked_weight_words=%0d independent_model=1 retained_baseline=1 read_latency=1 arbitrary_word_updates=1",reads,loads,resets,hold_checks,reads*32);
        $finish;
    end
    initial begin repeat(20000)@(posedge clk);$fatal(1,"weight TDP test timeout");end
endmodule
