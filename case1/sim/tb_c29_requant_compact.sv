...
module tb_c29_requant_compact;
    ...
    always #5 clk=~clk;
    c1_requant_bank8_compact dut(.*);
    wire ref_ready,ref_valid,ref_sof,ref_eol,ref_eof;
    wire [63:0] ref_data;wire [9:0] ref_x;wire [8:0] ref_y;
    c1_requant_bank8 retained(
        .in_ready(ref_ready),.out_valid(ref_valid),.out_data_s8(ref_data),
        .out_sof(ref_sof),.out_eol(ref_eol),.out_eof(ref_eof),.out_x(ref_x),.out_y(ref_y),.*);
    integer equivalence_cycles=0;
    always @(posedge clk)if(!rst)begin
        if(in_ready!==ref_ready || out_valid!==ref_valid ||
           (out_valid && {out_data_s8,out_sof,out_eol,out_eof,out_x,out_y} !==
                         {ref_data,ref_sof,ref_eol,ref_eof,ref_x,ref_y}))
            $fatal(1,"C29 quant cycle/bit mismatch");
    ...
    initial begin
        ...
        $display("C29_REQUANT_CYCLE_EQUIVALENCE_PASS cycles=%0d elastic_latency_unchanged=1",equivalence_cycles);
        $display("C29_REQUANT_COMPACT_PASS vectors=%0d lanes=%0d shifts=48 stalls=%0d",popped,popped*8,stalls);
        $finish;
    end
