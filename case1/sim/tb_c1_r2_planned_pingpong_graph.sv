`timescale 1ns/1ps
module tb_c1_r2_planned_pingpong_graph;
        ...
        if(rst) begin
            ...
            if(wr_valid && wr_ready) begin
                ...
                if(wr_last) begin
                    ...
                    response_delay<=fault_mode==11 ? 0 : fault_mode==9 ? 60 : COMMAND_LATENCY+(active_stage==20 ? 19 : STALLS ? 5 : 1);
                end
            end
            ...
            if(stage_done) begin
                ...
                if(height==480) $display("C1_R2_PLAN_GRAPH_STAGE_COMMIT stage=%0d cycles=%0d words=%0d",stage_done_index,frame_cycles,expected_index-frame_expected_base);
            end
    ...
    task run_frame(input integer fid,input integer fault);
        ...
        begin
            ...
            if(fault==0) begin
                ...
                $display("C1_R2_PLAN_GRAPH_FAULT stalls=%0d fault=%0d commits=%0d drained=1 overlapped_error=%0d compute_error=%0d two_pages_error=%0d cycles=%0d",STALLS,fault,commits,fault_overlap_seen,fault_compute_seen,fault_two_pages_seen,frame_cycles);
            end
    ...
    task reset_frame(input integer phase);
        ...
        begin
            ...
            run_frame(1,0);
            $display("C1_R2_PLAN_GRAPH_RESET_PASS phase=%0d restart_golden=1 cleared_pages=1",phase);
        end
    endtask
    initial begin
        ...
        if(RESET_PROBE) begin
            ...
            $display("C1_R2_PLAN_GRAPH_RESET_SUITE_PASS phases=6 normal_restarts=6 system_reset_only=1");$finish;
        end
        ...
        $display("C1_R2_PLAN_GRAPH_PASS stalls=%0d cache=%0d overlap=%0d compute_overlap=%0d refill_priority=%0d write_throttle=%0d memdiv=%0d latency=%0d width=%0d height=%0d normal_frames=%0d faults=%0d invalid_shapes=%0d reads=%0d writes=%0d blocked_writes=%0d overlap_cycles=%0d bulk_beats=%0d max_read_run=%0d compute_write_cycles=%0d compute_write_beats=%0d max_pages=%0d full_pages_cycles=%0d paused_held_cycles=%0d expected_input_only=1",STALLS,USE_ROW_CACHE,OVERLAP_WRITE,OVERLAP_COMPUTE,PRIORITIZE_REFILL,WRITE_THROTTLE,MEMORY_DIV,COMMAND_LATENCY,width,height,width==12 && FRAME_RUNS>1 ? 3 : FRAME_RUNS,faults_passed,invalid_shapes,rd_beats,wr_beats,blocked_writes,overlap_cycles,bulk_beats,max_read_run,compute_write_cycles,compute_write_beats,max_pages,full_pages_cycles,paused_held_cycles);
        $finish;
    end
endmodule
