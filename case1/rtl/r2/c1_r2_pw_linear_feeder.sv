`timescale 1ns/1ps
// Shared PW16x8 / same-scale residual feeder. Residual start_count is scalars;
// each 128-bit SRAM record stores eight A bytes followed by eight B bytes.
// Reuses the PW RAM; identity requant implements saturating add then ReLU.
...
module c1_r2_pw_linear_feeder #(
    ...
    input wire [PIXEL_BITS+3:0] start_count,
    input wire start_linear,
    output logic busy,
    input wire finish,
    ...
    assign start_ready=!rst && !busy && start_count!=0 && (start_linear ? start_count<=MAX_PIXELS*8 : start_count<=MAX_PIXELS);
    ...
    always_ff @(posedge clk) begin
        if(rst) begin
            busy<=0; linear_q<=0; total_q<=0; issue_base_q<=0; rd_valid_q<=0; rd_base_q<=0; rd_mask_q<=0;
        end else begin
            if(start_fire) begin
                busy<=1; linear_q<=start_linear; total_q<=start_linear ? start_count : (start_count<<3); issue_base_q<=0;
            end
            if(finish) busy<=0;
    ...
    for(genvar r=0;r<6;r=r+1) begin : g_request
        ...
        wire [127:0] feature=pixel_parity ? bank_data[255:128] : bank_data[127:0];
        wire [7:0] aa=feature[channel[2:0]*8+:8];
        wire [7:0] bb=feature[64+channel[2:0]*8+:8];
        assign req_a[r*128+:128]=linear_q ? {112'd0,bb,aa} : feature;
        assign req_b[r*128+:128]=linear_q ? 128'h0101 : weights[channel[2:0]*128+:128];
        assign req_bias[r*32+:32]=linear_q ? 32'd0 : biases[channel[2:0]*32+:32];
