`timescale 1ns/1ps
// Four-operator R2 row/tile executor, one shared MAC/quantizer.
// 0 PW16->8; 1 RGB3x3 Cin8->3; 2 DW3x3 Cin16/24/48; 3 same-scale add+ReLU.
// Two SRAM pools, not four: PW/residual and RGB/DW. Internal prototype ABI;
// no DDR/CPU integration. All job writes and starts are mutually exclusive.
module c1_r2_cnn_row_engine (
    input wire clk,rst,
    input wire [1:0] mode,
    ...
    wire [2:0] selected_groups=mode==1 ? 3'd1 : start_channels==16 ? 3'd2 : start_channels==24 ? 3'd3 : start_channels==48 ? 3'd6 : 3'd0;
    // Capacity is PER parity bank: ceil(width/2)*groups <= 1024.
    // In particular C48/W341 overflows even though width*groups <= 2048.
    wire legal_shape=start_size!=0 &&
        (mode==0 ? (start_size<=1024 && start_channels==16) :
         mode==1 ? (start_size<=1024 && start_channels==8) :
         mode==2 ? ((start_channels==16 && start_size<=1024) || (start_channels==24 && start_size<=682) || (start_channels==48 && start_size<=340)) : start_size<=8192);
    ...
    wire legal_address=mode==0 ? (load_kind==0 ? load_addr<4096 : load_kind==1 ? load_addr<32 : load_addr<8) :
                       mode==1 ? (load_kind==0 ? load_addr<12288 : load_kind==1 ? load_addr<60 : load_addr<3) :
                       mode==2 ? (load_kind==0 ? load_addr<12288 : load_kind==1 ? load_addr<144 : load_addr<48) :
                                 (load_kind==0 && load_addr<4096);
    ...
    assign load_ready=load_gate && (load_linear_pool ? f_load_ready[0] : f_load_ready[1]);
    ...
    assign out_last=owner_q==0 || owner_q==3 ? ({1'b0,out_index}+6>=total_q) : owner_q==1 ? ({1'b0,out_index}+2>=total_q) :
    ...
    always_ff @(posedge clk) begin
        if(rst) begin busy<=0;owner_q<=0;total_q<=0;groups_q<=1;request_valid_q<=0;end
            ...
            if(start_fire) begin busy<=1;owner_q<=mode;total_q<=mode==0 ? start_size<<3 : start_size;groups_q<=selected_groups;end
            if(finish) busy<=0;
    ...
    c1_r2_pw_linear_feeder u_linear (
        .clk(clk),.rst(rst),.load_valid(load_valid && load_gate && load_linear_pool),.load_ready(f_load_ready[0]),
        ...
        .start_count(start_size),.start_linear(mode==3),.busy(f_busy[0]),.finish(finish && own_linear_pool),
    ...
    c1_r2_spatial_feeder u_spatial (
        .clk(clk),.rst(rst),.load_valid(load_valid && load_gate && !load_linear_pool),.load_ready(f_load_ready[1]),
        ...
        .start_dw(mode==2),.row_width(start_size[10:0]),.start_groups(selected_groups),.row_top(row_top),.row_bottom(row_bottom),
        .finish(finish && !own_linear_pool),.busy(f_busy[1]),.req_valid(f_valid[1]),.req_ready(f_ready[1]),
