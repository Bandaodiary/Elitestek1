`timescale 1ns/1ps
// Shared RGB/DW spatial store: 3 rows x 2 parity banks, 1024 C8 words/bank.
...
module c1_r2_group_window_store (
    ...
    for(genvar j=0;j<4;j=j+1) begin : g_column
        if(j==0) assign column[j]=req_x==0 ? 10'd0 : req_x-1'b1;
        else begin : g_right
            wire [10:0] xx={1'b0,req_x}+(j-1);
            assign column[j]=xx>=req_width ? req_width-1'b1 : xx[9:0];
        end
    end
    ...
    always_ff @(posedge clk) begin
        if(rst) begin
            ...
            if(push) begin
                ...
                second_top<=req_top;second_bottom<=req_bottom;
            ...
            if(read_fire) begin
                ...
                pending_top<=second_q ? second_top : req_top;pending_bottom<=second_q ? second_bottom : req_bottom;
...
`ifndef SYNTHESIS
    always @(posedge clk) if(!rst) begin
        ...
        if(push && (req_width==0 || req_width>1024 || req_x>=req_width || req_group>=req_groups ||
          !(req_groups==1 || req_groups==2 || req_groups==3 || req_groups==6) || ((req_width+1)/2)*req_groups>1024))
