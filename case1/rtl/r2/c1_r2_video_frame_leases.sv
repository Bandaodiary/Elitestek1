`timescale 1ns/1ps
// C11 three raw inputs + two styled outputs. A raw/styled pair remains leased
...
module c1_r2_video_frame_leases #(
    ...
    output wire [2:0] input_owned,
    output wire [1:0] output_owned,
    ...
    logic [2:0] ins[0:2],ins_n[0:2],outs[0:1],outs_n[0:1];
    logic [31:0] tags[0:2],tags_n[0:2];
    logic [1:0] out_input[0:1],out_input_n[0:1];
    ...
    logic nn_o,front_o,latest_o,nn_o_n,front_o_n,latest_o_n;
    ...
    function automatic [31:0] input_address(input [1:0] i);
        case(i) 0:input_address=ARENA_BASE;1:input_address=ARENA_BASE+32'h03000000;default:input_address=ARENA_BASE+32'h04000000;endcase
    endfunction
    function automatic [31:0] output_address(input bit i);
        output_address=ARENA_BASE+(i ? 32'h03800000 : 32'h02000000);
    endfunction
    ...
    for(genvar i=0;i<3;i=i+1)assign input_owned[i]=ins[i]!=FREE;
    for(genvar i=0;i<2;i=i+1)assign output_owned[i]=outs[i]!=FREE;
    // One priority-ordered next-state transaction prevents two same-edge
    // requesters from allocating the same just-freed buffer.
    always_comb begin
        for(integer i=0;i<3;i=i+1)begin ins_n[i]=ins[i];tags_n[i]=tags[i];end
        for(integer i=0;i<2;i=i+1)begin outs_n[i]=outs[i];out_input_n[i]=out_input[i];end
        ...
        for(integer i=0;i<2;i=i+1)if(outs_n[i]==PAIR_READY)display_choice=i;
        ...
        if(display_request && !rst && enable && !protocol_error && disp_admit)begin
            if(display_choice>=0)begin
                ...
                for(integer i=0;i<2;i=i+1)if(outs_n[i]==PAIR_READY && i!=display_choice)begin
        ...
        for(integer i=0;i<3;i=i+1)if(ins_n[i]==READY)nn_choice=i;
        if(ins_n[latest_i_n]==READY)nn_choice=latest_i_n;
        for(integer i=1;i>=0;i=i-1)if(outs_n[i]==FREE)out_choice=i;
        nn_admit=!nn_owned_n && nn_choice>=0 && out_choice>=0;
        if(nn_request && !rst && enable && !protocol_error && nn_admit)begin
            for(integer i=0;i<3;i=i+1)if(ins_n[i]==READY && i!=nn_choice)begin ins_n[i]=FREE;dropped_n=dropped_n+1'b1;end
        ...
        for(integer i=2;i>=0;i=i-1)if(ins_n[i]==READY)cap_choice=i;
        for(integer i=2;i>=0;i=i-1)if(ins_n[i]==FREE)cap_choice=i;
    ...
    always_ff @(posedge clk)begin
        if(rst)begin
            for(integer i=0;i<3;i=i+1)begin ins[i]<=FREE;tags[i]<=0;end
            for(integer i=0;i<2;i=i+1)begin outs[i]<=FREE;out_input[i]<=0;end
            ...
            for(integer i=0;i<3;i=i+1)begin ins[i]<=ins_n[i];tags[i]<=tags_n[i];end
            for(integer i=0;i<2;i=i+1)begin outs[i]<=outs_n[i];out_input[i]<=out_input_n[i];end
...
`ifndef SYNTHESIS
    initial if(ARENA_BASE[22:0]!=0 || {1'b0,ARENA_BASE}+33'h4800000>33'h10000000)$fatal(1,"video arena outside 256MiB DDR");
