...
module c1_r2_spatial_banked_feeder (
    ...
    wire read_fire=!rst && window_valid && read_slot_ready;
    wire last_beat=dw_q ? beat_q==2 : beat_q==4;
    assign window_ready=read_slot_ready && last_beat;
    assign weight_read_en=read_fire;
    for(genvar bank=0;bank<8;bank=bank+1) begin : g_address
        assign weight_read_addr[bank*6+:6]=dw_q ? {window_group,3'd0} : {3'd0,beat_q};
    end
    wire [767:0] issue_a;
    wire [5:0] issue_mask;
    ...
    assign req_mask=rd_mask_q;assign req_tag=rd_tag_q;assign req_a=rd_a_q;
    for(genvar r=0;r<6;r=r+1) begin : g_lane
        ...
        assign req_b[r*128+:128]=rd_dw_q ? dw_b : weight_data[RGB_C*128+:128];
        assign req_bias[r*32+:32]=rd_dw_q ? dw_bias : bias_data[RGB_C*32+:32];
