...
module c1_r2_overlay_window_store_packed (
    ...
    for(genvar bank_id=2;bank_id<6;bank_id=bank_id+1) begin : g_ram
        ...
        c1_ram_sdp_read_first #(.DATA_WIDTH(64),.DEPTH(1024),.ADDR_WIDTH(10)) u_ram (
            ...
            .wr_addr(bulk_address),.wr_data(bulk_data[BANK*64+:64])
        );
    end
