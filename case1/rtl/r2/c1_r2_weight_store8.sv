...
module c1_r2_weight_store8 (
    ...
    // Flattened generate indices also work with Icarus module port slicing.
    for(genvar ram_id=0;ram_id<32;ram_id=ram_id+1) begin : g_weight
        localparam integer BANK=ram_id/4,WORD=ram_id%4;
        c1_ram_sdp_read_first #(.DATA_WIDTH(32),.DEPTH(64),.ADDR_WIDTH(6)) u_ram (
            .clk(clk),.rd_en(read_en && !rst),.rd_addr(read_addr[BANK*6+:6]),
            .rd_data(read_weights[ram_id*32+:32]),
            .wr_en(!rst && load_valid && load_kind==1 && load_addr[10:8]==BANK && load_addr[1:0]==WORD),
            .wr_addr(load_addr[7:2]),.wr_data(load_data)
        );
    end
`ifndef SYNTHESIS
