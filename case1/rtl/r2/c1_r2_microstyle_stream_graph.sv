...
module c1_r2_microstyle_stream_graph #(
    ...
    always_comb begin
        mode_c_decoded=0;cin_c_decoded=24;cout_c_decoded=48;src_width_decoded=width_q>>2;dst_width_decoded=width_q>>2;
        src_height_decoded=height_q>>2;dst_height_decoded=height_q>>2;src_slot_decoded=1;dst_slot_decoded=0;skip_slot_decoded=1;
        parameter_beats_decoded=240;virtual_c_decoded=0;view_c_decoded=0;
        case(stage_q)
            0:begin mode_c_decoded=4;cin_c_decoded=3;cout_c_decoded=12;src_width_decoded=width_q;src_height_decoded=height_q;dst_width_decoded=width_q>>1;dst_height_decoded=height_q>>1;dst_slot_decoded=0;parameter_beats_decoded=60;end
            1:begin mode_c_decoded=5;cin_c_decoded=12;cout_c_decoded=24;src_width_decoded=width_q>>1;src_height_decoded=height_q>>1;src_slot_decoded=0;dst_slot_decoded=1;parameter_beats_decoded=360;end
            2,6,10:begin src_slot_decoded=stage_q==6 ? 2 : 1;end
            3,7,11:begin mode_c_decoded=2;cin_c_decoded=48;cout_c_decoded=48;src_slot_decoded=0;dst_slot_decoded=stage_q==7 ? 1 : 2;parameter_beats_decoded=144;end
            4,8,12:begin cin_c_decoded=48;cout_c_decoded=24;src_slot_decoded=stage_q==8 ? 1 : 2;dst_slot_decoded=0;parameter_beats_decoded=168;end
            5,9,13:begin mode_c_decoded=3;cout_c_decoded=24;src_slot_decoded=0;dst_slot_decoded=stage_q==9 ? 1 : 2;skip_slot_decoded=stage_q==9 ? 2 : 1;parameter_beats_decoded=0;end
            14,17,21:begin view_c_decoded=1;parameter_beats_decoded=0;end
            15:begin mode_c_decoded=2;cin_c_decoded=24;cout_c_decoded=24;virtual_c_decoded=1;src_slot_decoded=2;dst_slot_decoded=0;dst_width_decoded=width_q>>1;dst_height_decoded=height_q>>1;parameter_beats_decoded=72;end
            16:begin cin_c_decoded=24;cout_c_decoded=16;src_slot_decoded=0;dst_slot_decoded=1;src_width_decoded=width_q>>1;dst_width_decoded=width_q>>1;src_height_decoded=height_q>>1;dst_height_decoded=height_q>>1;parameter_beats_decoded=80;end
            18:begin mode_c_decoded=2;cin_c_decoded=16;cout_c_decoded=16;virtual_c_decoded=1;src_slot_decoded=1;dst_slot_decoded=0;src_width_decoded=width_q>>1;dst_width_decoded=width_q;src_height_decoded=height_q>>1;dst_height_decoded=height_q;parameter_beats_decoded=48;end
            19:begin cin_c_decoded=16;cout_c_decoded=8;src_slot_decoded=0;dst_slot_decoded=1;src_width_decoded=width_q;dst_width_decoded=width_q;src_height_decoded=height_q;dst_height_decoded=height_q;parameter_beats_decoded=24;end
            20:begin mode_c_decoded=1;cin_c_decoded=8;cout_c_decoded=3;src_slot_decoded=1;dst_slot_decoded=0;src_width_decoded=width_q;dst_width_decoded=width_q;src_height_decoded=height_q;dst_height_decoded=height_q;parameter_beats_decoded=33;end
            default:begin view_c_decoded=1;parameter_beats_decoded=0;end
        endcase
    end
    ...
    wire [15:0] op_index;
    wire [2:0] op_mode;
    wire issue=state==ISSUE && op_start_ready && w_start_ready && !write_pending && !read_pending && !(w_done && w_error);
    ...
    always_ff @(posedge clk) begin
        if(rst) begin
            ...
            read_pending<=0;write_pending<=0;cache_phase<=0;fill_bank<=0;fill_row<=0;cache_valid<=0;row_map<=6'b10_01_00;
            ...
            if(issue) write_pending<=1;
            if(w_done) write_pending<=0;
            ...
            case(state)
                ...
                CONFIG_CAPTURE:begin
                    ...
                    state<=view_c ? COMMIT : parameter_beats!=0 ? PARAM_START : USE_ROW_CACHE && spatial_c ? CACHE_PICK : ADDRESS_COORD;
                end
                ...
                default:state<=IDLE;
            endcase
            // Writes from the preceding row may still be in flight when a
            // prefetch fails (or vice versa). Finish BOTH accepted transfers;
            // never launch another compute job or publish a failed frame.
...
`ifndef SYNTHESIS
    always @(posedge clk) if(!rst) begin
        ...
        if(issue && (read_pending || write_pending)) $fatal(1,"graph issued over pending transfer");
