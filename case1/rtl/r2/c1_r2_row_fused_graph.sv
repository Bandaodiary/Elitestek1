// C35 full-graph row fusion candidate; C34 baseline remains unchanged.
// C33 per-physical-burst publication and granted drain; independent C31 derivative.
...
module c1_r2_row_fused_graph #(
    parameter integer ENABLE_ROW_FUSION=1,
    ...
    output wire wr_issue_enable,
    output wire [15:0] wr_published_beats,
    input wire [15:0] wr_granted_beats,
    ...
    localparam IDLE=0,SETUP=1,PARAM_START=2,PARAM_WAIT=3,ROW_START=4,ROW_WAIT=5,
               ...
               SHADOW_WAIT=18,PW_PARAM_START=19,PW_PARAM_WAIT=20,PW_ISSUE=21,PAIR_COMMIT=22;
    logic [4:0] state;
    logic [4:0] stage_q;
    logic [10:0] width_q;
    ...
    wire fusion_enable_decoded;
    wire [4:0] pw_parameter_block_decoded;
    wire [15:0] pw_parameter_beats_decoded;
    wire [1:0] pw_dst_slot_decoded;
    c1_r2_row_fusion_plan u_fusion_plan (
        .index(stage_q),.enable(fusion_enable_decoded),
        .pw_parameter_block(pw_parameter_block_decoded),
        .pw_parameter_beats(pw_parameter_beats_decoded),.pw_dst_slot(pw_dst_slot_decoded)
    );
    logic fusion_active,fusion_pw_q;
    logic [4:0] pw_parameter_block_q;
    logic [15:0] pw_parameter_beats_q;
    logic [1:0] pw_dst_slot_q;
    // Keep the DW geometry/cache owner across both operators. Only operator
    // and parameter-loader mode change to PW; no DDR intermediate is read.
    wire [2:0] execution_mode=fusion_active && fusion_pw_q ? 3'd0 : mode_c;
    wire [5:0] execution_cout=fusion_active && fusion_pw_q ? 6'd8 : cout_c;
    wire [10:0] execution_width=fusion_active && fusion_pw_q ? dst_width : src_width;
    wire execution_virtual=virtual_c && !(fusion_active && fusion_pw_q);
    wire [8:0] partition_base=dst_width>>2;
    wire [9:0] partition_end=(dst_width>>2)+(dst_width>>1);
    wire shadow_available,shadow_error,op_done;
    ...
    wire [15:0] dst_row_beats_c=fusion_active ? ((dst_width+1)>>1) : ((dst_width+1)>>1)*output_groups;
    ...
    wire [31:0] destination_base=output_rgb_c ? output_q : workspace_q+({30'd0,(fusion_active ? pw_dst_slot_q : dst_slot)}<<23);
    ...
    wire param_start=state==PARAM_START || state==PW_PARAM_START;
    c1_r2_tensor_stream_loader u_loader (
        .clk(clk),.rst(rst),.start_valid(param_start || state==ROW_START),.start_ready(l_start_ready),
        .start_mode(execution_mode),.start_parameters(param_start),.start_residual_b(residual_b),.start_input_rgb(input_rgb_c && !param_start),
        .start_row(USE_ROW_CACHE && spatial_c ? fill_bank : physical_row),.start_groups(input_groups),
        .start_address(param_start ? parameter_q+({27'd0,(fusion_active && fusion_pw_q ? pw_parameter_block_q : parameter_block_c)}<<13) : source_address),
        .start_beats(param_start ? (fusion_active && fusion_pw_q ? pw_parameter_beats_q : parameter_beats) : src_row_beats),
    ...
    wire [15:0] op_index;
    wire [2:0] op_mode;
    wire capture_issue=fusion_active && !fusion_pw_q;
    wire issue=(state==ISSUE || state==PW_ISSUE) && op_start_ready &&
        (capture_issue || w_start_ready) && (OVERLAP_COMPUTE || !write_pending) &&
        !read_pending && !(w_done && w_error);
    wire writer_issue=issue && !capture_issue;
    wire [13:0] op_size=mode_c==3 ? ({3'd0,dst_width}<<4)+({3'd0,dst_width}<<3) : execution_width;
    c1_r2_cnn_row_shadow_engine u_operator (
        .partition_en(fusion_active),.partition_base(partition_base),.partition_end(partition_end),
        .start_shadow_capture(capture_issue),.start_shadow_read(fusion_active && fusion_pw_q),
        .shadow_available(shadow_available),.shadow_error(shadow_error),.op_done(op_done),
        .clk(clk),.rst(rst),.mode(execution_mode),.load_valid(l_valid),.load_ready(l_ready),.load_kind(l_kind),.load_addr(l_addr),.load_data(l_data),
        ...
        .start_valid(issue),.start_ready(op_start_ready),.start_size(op_size),.start_channels(cin_c),.start_outputs(execution_cout),
        .row_top(center_y==0),.row_bottom(center_y+1==src_height),.virtual_up2(execution_virtual),.row_phase(row_y[0]),.busy(op_busy),
    ...
    assign wr_issue_enable=writer_stream_enable;
    c1_r2_tensor_credit_ring_writer u_writer (
        .published_beats(wr_published_beats),.granted_beats(wr_granted_beats),
        .clk(clk),.rst(rst),.start_valid(writer_issue),.start_ready(w_start_ready),.start_mode(execution_mode),.start_width(dst_width),
        .stream_enable(writer_stream_enable),
        .start_channels(execution_cout),.start_rgb(output_rgb_c),.start_address(destination_address),
    ...
    always_ff @(posedge clk) begin
        if(rst) begin
            fusion_active<=0;fusion_pw_q<=0;pw_parameter_block_q<=0;pw_parameter_beats_q<=0;pw_dst_slot_q<=0;
            ...
            stage_done<=0;
            if((param_start || state==ROW_START) && l_start_ready) read_pending<=1;
            if(l_done) read_pending<=0;
            case({writer_issue,w_done})
            ...
            case(state)
                ...
                SETUP:begin
                    fusion_active<=ENABLE_ROW_FUSION && fusion_enable_decoded;fusion_pw_q<=0;
                    pw_parameter_block_q<=pw_parameter_block_decoded;pw_parameter_beats_q<=pw_parameter_beats_decoded;pw_dst_slot_q<=pw_dst_slot_decoded;
                ...
                ISSUE:if(issue) state<=capture_issue ? SHADOW_WAIT : WRITE_WAIT;
                SHADOW_WAIT:if(op_done) begin
                    if(!shadow_available || shadow_error)begin done_error<=1;state<=ERROR_DRAIN;end
                    else begin fusion_pw_q<=1;state<=PW_PARAM_START;end
                end
                PW_PARAM_START:if(l_start_ready)state<=PW_PARAM_WAIT;
                PW_PARAM_WAIT:if(l_done)state<=PW_ISSUE;
                PW_ISSUE:if(issue)state<=WRITE_WAIT;
                WRITE_WAIT:if(op_valid && op_ready && op_last) begin
                    if(row_y+1==dst_height) state<=FINAL_WRITE_WAIT;
                        ...
                        if(fusion_active)fusion_pw_q<=0;
                        state<=fusion_active ? PARAM_START : USE_ROW_CACHE && spatial_c ? CACHE_PICK : ADDRESS_COORD;
                ...
                COMMIT:begin
                    ...
                    else begin stage_q<=stage_q+1'b1;state<=fusion_active ? PAIR_COMMIT : SETUP;end
                end
                PAIR_COMMIT:begin
                    // Both logical nodes retire only after all PW row B
                    // responses; the DW tensor never existed in DDR.
                    stage_done<=1;stage_done_index<=stage_q;
                    stage_q<=stage_q+1'b1;fusion_active<=0;fusion_pw_q<=0;state<=SETUP;
                end
...
`ifndef SYNTHESIS
    always @(posedge clk) if(!rst) begin
        if(state==CONFIG_CAPTURE && cache_valid!=0)$fatal(1,"overlay graph kept cache valid across layers");
        ...
        if(op_valid && op_mode!=execution_mode) $fatal(1,"graph operator owner changed");
        if(fusion_active && (mode_c!=2 || cin_c!=16 || cout_c!=16 || !virtual_c || src_slot==pw_dst_slot_q))
            $fatal(1,"illegal fusion shape or live-source alias");
        if(fusion_active && !fusion_pw_q && op_valid)$fatal(1,"DW intermediate escaped shadow storage");
        if(state==PW_ISSUE && !shadow_available)$fatal(1,"PW before shadow publication");
        if(fusion_active && fusion_pw_q && state==ROW_START)$fatal(1,"fused PW attempted DDR feature read");
