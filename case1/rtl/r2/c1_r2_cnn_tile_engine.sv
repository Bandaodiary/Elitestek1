`timescale 1ns/1ps
// Four-operator R2 tile executor with shared banked weights and compute.
// 0 PW Cin16/24/48 -> Cout8/16/24/48; 1 RGB; 2 DW; 3 add+ReLU.
...
module c1_r2_cnn_tile_engine (
    ...
    wire legal_weight=mode==0 ? (load_addr<2048 && load_addr[7:5]<6 && load_addr[4:2]<3) :
                      mode==1 ? (load_addr<768 && load_addr[7:5]==0 && load_addr[4:2]<5) :
                      mode==2 ? (load_addr<2048 && load_addr[7:5]<6 && load_addr[4:2]==0) : 1'b0;
    wire legal_address=load_kind==0 ? (load_linear_pool ? load_addr<4096 : load_addr<12288) :
                       load_kind==1 ? legal_weight : (mode==0 || mode==2 ? load_addr<48 : mode==1 ? load_addr<3 : 1'b0);
    ...
    always_ff @(posedge clk) begin
        if(rst) begin busy<=0;owner_q<=0;total_q<=0;groups_q<=1;request_valid_q<=0;end
            ...
            if(start_fire) begin busy<=1;owner_q<=mode;total_q<=mode==0 ? (start_outputs==8 ? start_size<<3 : start_outputs==16 ? start_size<<4 : start_outputs==24 ? (start_size<<4)+(start_size<<3) : (start_size<<5)+(start_size<<4)) : {2'd0,start_size};groups_q<=selected_groups;end
            if(finish) busy<=0;
    ...
    c1_r2_spatial_banked_feeder u_spatial (
