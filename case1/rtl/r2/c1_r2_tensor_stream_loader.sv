...
// Feature words use P2C8. One accepted beat writes four distinct SRAM slices
// on the SAME edge; no per-word unpack FSM or duplicate feature buffer.
module c1_r2_tensor_stream_loader (
    ...
    output wire bulk_valid,
    input wire bulk_ready,
    output wire [1:0] bulk_row,
    output wire [9:0] bulk_pair,
    output wire [2:0] bulk_group,bulk_groups,
    output wire bulk_residual_b,
    output wire [127:0] bulk_data,
    ...
    wire bad_response=data_error || data_last!=(remaining_q==1);
    assign data_ready=!rst && (state==DRAIN || (state==RECEIVE && (parameters_q || bad_response || bulk_ready)));
    assign bulk_valid=!rst && state==RECEIVE && !parameters_q && data_valid && !bad_response;
    assign bulk_row=row_q;assign bulk_pair=pair_q[9:0];assign bulk_group=group_q;assign bulk_groups=groups_q;
    assign bulk_residual_b=residual_b_q;
    assign bulk_data=data^(rgb_q ? {16{8'h80}} : 128'd0);
    ...
    always_ff @(posedge clk) begin
        if(rst) begin
            ...
            if(data_valid && data_ready) begin
                ...
                end else if(parameters_q) begin data_q<=data;last_q<=data_last;part_q<=0;remaining_q<=remaining_q-1'b1;state<=UNPACK;end
                else begin
                    remaining_q<=remaining_q-1'b1;
                    if(group_q+1==groups_q) begin group_q<=0;pair_q<=pair_q+1'b1;end else group_q<=group_q+1'b1;
                    if(data_last) begin state<=IDLE;done<=1;end
                end
            end
