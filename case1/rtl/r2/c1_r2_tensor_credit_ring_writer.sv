`timescale 1ns/1ps
// C34: shared circular word reservation; C33's publication/grant contract retained.
// Prior C32 and C31 implementations remain separate, unchanged baselines.
// Two row metadata slots share STORAGE_WORDS physical P2C8 words.
// A RAM word is reusable after its synchronous read, while row metadata
// remains owned until producer LAST and the actual complete B response.
...
// A row metadata slot releases ONLY after collection and real response.
// A complete row is reserved atomically before producer admission, so byte
// writes never overwrite any unread word, even across physical ring wrap.
// Candidate only: full RTL simulation/PNR/integration still require validation.
module c1_r2_tensor_credit_ring_writer #(
    parameter integer STORAGE_WORDS=1024
) (
    ...
    localparam integer RAM_AW=$clog2(STORAGE_WORDS);
    logic [RAM_AW:0] used_words;
    logic [RAM_AW-1:0] allocation_cursor,collect_base,send_base;
    logic [RAM_AW-1:0] page_base[0:1];
    ...
    wire [10:0] requested_words=times_groups((start_width+1)>>1,(start_channels+7)>>3);
    wire [RAM_AW+1:0] reserve_end={1'b0,used_words}+requested_words;
    assign start_ready=!rst && !collecting && !allocated[allocating_page] &&
                       requested_words!=0 && reserve_end<=STORAGE_WORDS;
    ...
    wire [127:0] ram_data;
    wire [RAM_AW-1:0] send_ram_address=send_base+read_index;
    ...
    for(genvar b=0;b<16;b=b+1) begin : g_byte_bank
        logic wr;
        logic [RAM_AW-1:0] addr;
        logic [7:0] value;
        always_comb begin
            wr=0;addr=0;value=0;
            for(integer l=0;l<6;l=l+1) if(take && in_mask[l] && lane_bank[l]==b) begin
                wr=1;addr=collect_base+lane_address[l];value=in_data[l*8+:8]^(rgb_q ? 8'h80 : 8'h00);
            end
        end
        c1_ram_sdp_read_first #(.DATA_WIDTH(8),.DEPTH(STORAGE_WORDS),.ADDR_WIDTH(RAM_AW)) u_ram (
            .clk(clk),.wr_en(wr),.wr_addr(addr),.wr_data(value),
            .rd_en(read_fire),.rd_addr(send_ram_address),.rd_data(ram_data[b*8+:8])
        );
    ...
    always_ff @(posedge clk) begin
        if(rst) begin
            ...
            used_words<=0;allocation_cursor<=0;collect_base<=0;send_base<=0;
            done<=0;error<=0;
            for(integer p=0;p<2;p=p+1) begin page_base[p]<=0;ready_words[p]<=0;page_width[p]<=0;page_words[p]<=0;page_channels[p]<=0;page_groups[p]<=0;page_address[p]<=0;end
        end else begin
            done<=0;error<=0;
            // Free the RAM word only once the registered read captures all
            // bytes. Backpressure holds that read result in the original
            // output register; reusing the SRAM slot cannot change it.
            case({reserve,read_fire})
                2'b10:used_words<=used_words+requested_words;
                2'b01:used_words<=used_words-1'b1;
                2'b11:used_words<=used_words+requested_words-1'b1;
                default:begin end
            endcase
            if(reserve) begin
                collect_base<=allocation_cursor;page_base[allocating_page]<=allocation_cursor;
                allocation_cursor<=allocation_cursor+requested_words;
...
`ifndef SYNTHESIS
    initial if(STORAGE_WORDS!=1024 && STORAGE_WORDS!=2048)
        $fatal(1,"C34 ring must preserve at least one maximum-size row");
    always @(posedge clk) if(!rst) begin
        if(used_words>STORAGE_WORDS || (read_fire && used_words==0))
            $fatal(1,"C34 circular reservation occupancy");
        if(reserve && reserve_end>STORAGE_WORDS)
            $fatal(1,"C34 accepted row without physical reservation");
        if(state==IDLE && allocated==0 && used_words!=0)
            $fatal(1,"C34 lost ring reservation at row drain");
