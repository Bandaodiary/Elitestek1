...
module c1_r2_overlay_window_store (
    ...
    for(genvar slice_id=4;slice_id<12;slice_id=slice_id+1) begin : g_ram
        ...
        c1_ram_sdp_read_first #(.DATA_WIDTH(32),.DEPTH(1024),.ADDR_WIDTH(10)) u_ram (
            ...
            .wr_addr(bulk_address),.wr_data(bulk_data[BANK*64+WORD*32+:32])
        );
    end
