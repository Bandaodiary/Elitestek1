...
module c1_r2_spatial_partitioned_feeder #(
    ...
    wire [11:0] pairs=({1'b0,source_width}+12'd1)>>1;
    wire legal_capacity=start_groups==1 ? pairs<=ROW_WORDS : start_groups==2 ? pairs<=ROW_WORDS/2 :
        start_groups==3 ? pairs<=ROW_WORDS/3 : pairs<=ROW_WORDS/6;
    ...
    c1_r2_partitioned_window_store #(.ROW_WORDS(ROW_WORDS)) u_store (
        ...
        .bulk_en(bulk_en),.bulk_row(bulk_row),.bulk_pair(bulk_pair),.bulk_group(bulk_group),.bulk_groups(bulk_groups),.bulk_data(bulk_data),
    ...
    wire [107:0] e_mult;
    wire [35:0] e_shift;
    c1_r2_encoder_lane_feeder u_encoder (
        ...
        .weight_read_en(e_weight_en),.weight_read_addr(e_weight_addr),.weight_lane_addr(e_lane_addr),
        ...
        if(rst) begin
            ...
            if(read_slot_ready) begin
                ...
                if(read_fire) begin
                    ...
                    rd_dw_q<=dw_q;rd_batch_q<=beat_q[1:0];rd_packed_a_q<=issue_packed_a;rd_mask_q<=issue_mask;
    ...
    for(genvar r=0;r<6;r=r+1) begin : g_lane
        ...
        wire pixel=scalar[3];
        localparam [2:0] RGB_BANK=RGB_C;
        assign weight_lane_addr[r*9+:9]=encoder_q ? e_lane_addr[r*9+:9] :
            dw_q ? {scalar[2:0],window_group,3'd0} : {RGB_BANK,3'd0,beat_q};
        ...
        for(genvar batch=0;batch<3;batch=batch+1) begin : g_dw_map
            ...
            if(LOCAL_SCALAR<16) begin : g_valid
                ...
                assign b_options[batch*128+:128]={56'd0,weight_data[r*128+:72]};
        ...
        assign req_b[r*128+:128]=encoder_q ? e_b[r*128+:128] : rd_dw_q ? dw_b : weight_data[r*128+:128];
