...
module r1_bilinear_interp_rgb888 #(
    ...
    (* DONT_TOUCH = "yes" *) logic [7:0] stage1_r0;
    (* DONT_TOUCH = "yes" *) logic signed [22:0] stage1_r1;
    (* DONT_TOUCH = "yes" *) logic [7:0] stage1_g0;
    (* DONT_TOUCH = "yes" *) logic signed [22:0] stage1_g1;
    (* DONT_TOUCH = "yes" *) logic [7:0] stage1_b0;
    (* DONT_TOUCH = "yes" *) logic signed [22:0] stage1_b1;
    ...
    // C37 exact Q12 difference form. Signed widening is explicit; the full
    // convex numerator remains nonnegative. Rounding stays +2048 then >>12
    // independently in the horizontal and vertical stages.
    function automatic logic signed [22:0] multiply_difference_q12 (
        input logic [7:0] sample0,sample1,
        input logic [12:0] weight1
    );
        logic signed [8:0] difference;
        begin
            difference=$signed({1'b0,sample1})-$signed({1'b0,sample0});
            multiply_difference_q12=difference*$signed({1'b0,weight1});
        end
    endfunction

    function automatic logic [7:0] round_pair_products_u8 (
        input logic [7:0] base_sample,
        input logic signed [22:0] difference_product
    );
        logic signed [24:0] numerator;
        begin
            numerator=$signed({1'b0,base_sample,12'd0})+
                {{2{difference_product[22]}},difference_product}+25'sd2048;
            round_pair_products_u8=numerator[19:12];
        end
    endfunction

    function automatic logic [7:0] interpolate_pair_u8 (
        input logic [7:0] sample0,sample1,
        input logic [12:0] weight0,weight1
    );
        begin
            interpolate_pair_u8=round_pair_products_u8(
                sample0,multiply_difference_q12(sample0,sample1,weight1));
        end
    endfunction
        ...
        if (rst) begin
            ...
            if (stage1_ready) begin
                ...
                if (stage0b_valid) begin
                    stage1_r0 <= stage0b_h0[23:16];
                    stage1_r1 <= multiply_difference_q12(
                        stage0b_h0[23:16],stage0b_h1[23:16],stage0b_wy1);
                    stage1_g0 <= stage0b_h0[15:8];
                    stage1_g1 <= multiply_difference_q12(
                        stage0b_h0[15:8],stage0b_h1[15:8],stage0b_wy1);
                    stage1_b0 <= stage0b_h0[7:0];
                    stage1_b1 <= multiply_difference_q12(
                        stage0b_h0[7:0],stage0b_h1[7:0],stage0b_wy1);
                    stage1_x <= stage0b_x;
                    stage1_y <= stage0b_y;
                end
            end
