...
//
// Each physical row is represented by two explicit portable simple-dual-port
// RAM instances.  Both replicas receive every write; the x0 replica supplies
// x0 and the x1 replica supplies x1.  Four RAMs therefore consume
// 4*MAX_WIDTH*24 storage bits and expose only one synchronous read plus one
// write port apiece.  No vendor primitive is instantiated and RAM contents are
// never reset; only valid bits and row tags are reset.
//
...
module c1_r1_resize_line_sampler #(
    ...
    c1_ram_sdp_read_first #(
        ...
        .clk     (clk),
        .rd_en   (ram_rd_en),
        .rd_addr (ram_x0_rd_addr),
        .rd_data (ram_line0_x0_rd_data),
        .wr_en   (ram_line0_wr_en),
        .wr_addr (ram_wr_addr),
        .wr_data (ram_wr_data)
    );
    ...
    c1_ram_sdp_read_first #(
        ...
        .clk     (clk),
        .rd_en   (ram_rd_en),
        .rd_addr (ram_x1_rd_addr),
        .rd_data (ram_line0_x1_rd_data),
        .wr_en   (ram_line0_wr_en),
        .wr_addr (ram_wr_addr),
        .wr_data (ram_wr_data)
    );
    ...
    c1_ram_sdp_read_first #(
        ...
        .rd_en   (ram_rd_en),
        .rd_addr (ram_x0_rd_addr),
        .rd_data (ram_line1_x0_rd_data),
        .wr_en   (ram_line1_wr_en),
        .wr_addr (ram_wr_addr),
        .wr_data (ram_wr_data)
    );
