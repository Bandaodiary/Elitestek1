// C29 compact, bit-exact five-stage requantization; retained R1 bank unchanged.
// Shifted magnitudes need only low8 + overflow + sign before final saturation.
`timescale 1ns/1ps
...
//   4. variable shift into a low8/overflow/sign record
//   5. signed8 sign restoration, saturation and activation
//
...
module c1_requant_bank8_compact #(
    ...
    // {negative, magnitude_overflow_above_8bits, magnitude_low8}.
    // Keep only information needed for EXACT signed8 saturation/activation.
    logic [9:0] rounded_record [0:7];
    ...
    function automatic logic [9:0] shifted_record(
    ...
    function automatic logic signed [7:0] saturate_activate_s8(
        input logic [9:0] record,
        input logic [1:0] activation
    );
        logic negative, overflow;
        logic [7:0] magnitude;
        begin
            {negative, overflow, magnitude} = record;
            if ((activation == ACT_RELU) && negative)
                saturate_activate_s8 = 8'd0;
            else if (overflow || (negative ? magnitude > 8'd128 : magnitude > 8'd127))
                saturate_activate_s8 = negative ? 8'h80 : 8'h7f;
            else
                saturate_activate_s8 = negative ? (8'd0 - magnitude) : magnitude;
        end
    endfunction
                ...
                if (rounded_valid) begin
                    for (channel = 0; channel < 8; channel = channel + 1)
                        out_data_s8[channel*8 +: 8] <=
                            saturate_activate_s8(rounded_record[channel],
            ...
            if (rounded_stage_ready) begin
                ...
                if (biased_valid) begin
                    for (channel = 0; channel < 8; channel = channel + 1) begin
                        rounded_record[channel] <= shifted_record(
        ...
        if (!rst && in_valid && in_ready) begin
            for (assert_channel = 0; assert_channel < 8;
                ...
                if (in_shift_u6[assert_channel*6 +: 6] > 6'd47)
                    $fatal(1,
                           "c1_requant_bank8_compact channel %0d shift must be in 0..47",
                           assert_channel);
                if ((in_activation[assert_channel*2 +: 2] != ACT_NONE) &&
                    (in_activation[assert_channel*2 +: 2] != ACT_RELU))
                    $fatal(1,
                           "c1_requant_bank8_compact channel %0d unsupported activation %0d",
                           assert_channel,
