"""C12 RGBX32 memory, unchanged P2C8 numeric golden and retained compute."""
from __future__ import annotations
import argparse,json,subprocess,tempfile
from pathlib import Path
from run_r2_video_system_probe import SOURCES as C11
from run_r2_graph_probe import vectors
ROOT=Path(__file__).resolve().parents[1]
SOURCES=C11+['rtl/r2/c1_r2_rgbx_row_read.sv','rtl/r2/c1_r2_rgbx_row_write.sv','rtl/r2/c1_r2_microstyle_rgbx_axi_graph.sv',
             'rtl/r2/c1_r2_video_capture_rgbx32.sv','rtl/r2/c1_r2_video_scanout_rgbx32.sv','rtl/r2/c1_r2_video_rgbx_csr.sv','rtl/r2/c1_r2_video_rgbx_leases.sv','rtl/r2/c1_r2_video_rgbx_system.sv']
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='c1_r2_video_rgbx_',dir=ROOT/'sim') as td:
        root=Path(td)
        if a.compile_only:
            ...
            print(f'C1_R2_RGBX_SYSTEM_COMPILE_PASS sources={len(SOURCES)}')
        else:
            for shape in a.shapes.split(','):
                w,h=map(int,shape.split('x'));folder=root/shape;folder.mkdir();m=vectors(folder,w,h)
                print('C1_R2_RGBX_SYSTEM_VECTORS '+json.dumps(m),flush=True)
                for stalls in map(int,a.stalls.split(',')):
                    exe=folder/'system.vvp';top='tb_c1_r2_video_rgbx_system'
                    for negative in ((1,2) if a.negative_only else (0,)):
                        ...
                        if negative:
                            ...
                            if r.returncode==0 or marker not in r.stdout or 'C1_R2_RGBX_SYSTEM_PASS ' in r.stdout:raise RuntimeError('negative control did not trip: '+r.stdout[-2000:])
                            print(f'C1_R2_RGBX_SYSTEM_NEGATIVE_PASS width={w} height={h} stalls={stalls} corruption={negative} actual_ram_mutation=1',flush=True)
                        else:
                            for line in r.stdout.splitlines():
                                if line.startswith('C1_R2_RGBX_SYSTEM_'):print(line,flush=True)
                            if r.returncode or r.stdout.count('C1_R2_RGBX_SYSTEM_PASS ')!=1 or 'FATAL' in r.stdout or 'ERROR' in r.stdout:raise RuntimeError((r.stdout+r.stderr)[-4000:])
    print('C1_R2_RGBX_SYSTEM_CLEAN temporary_vectors_and_simulator_removed=1',flush=True)
