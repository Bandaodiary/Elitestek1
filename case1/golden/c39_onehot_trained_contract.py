"""Explicit one-hot C39 model/source/resource contract, not a C37 relabel."""
import argparse
import json
import re
...
def bound_candidate(model):
    ...
    provenance.update(production_RTL_changed=True, candidate='C39_ONEHOT',
                      source_replacement_contract='golden/c39_onehot_sources.py')
...
def source_gate():
    ...
    for module, relative in {
        ...
        'c39_operand_unpack': 'rtl/c39_onehot/c39_operand_codec.sv',
    }.items():
    ...
    print('C39_ONEHOT_SOURCE_CONTRACT_PASS ' + json.dumps(dict(production_sources=49,
        candidate='C39_ONEHOT', max_channels=24, row_words=512, retained_C37_gate_preserved=True,
        model_ROM_matched=True, RTL_compiled=False, native_fps_claim=False), separators=(',', ':')), flush=True)
    return compiled


def resource_preflight():
    report = compare(['c39_host_onehot_pnr_20260915a'])
...
def main():
    ...
    if args.output:
        ...
        print('C39_ONEHOT_TRAINED_VECTOR_BUILD ' + json.dumps(dict(model_binding=provenance,
