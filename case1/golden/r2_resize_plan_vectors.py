"""C24 RGB source -> independent Resize -> DAG integer oracle; checker-only tensors.
...
def vectors(directory, width, height, profile='drop_res1', package=None, parameter_override=None):
    ...
    expected, frames, sources = [], [], []
    for frame in range(2):
        sw, sh = (width*2, height*2) if frame==0 else (width*3//2+1, height*3//2+3)
        source = source_image(sw,sh,30+frame)
        rgb = resize_bilinear_q16_u8(source,width,height)
        xs,xp = resize_axis_q16(sw,width)
        ys,yp = resize_axis_q16(sh,height)
        ...
        sources.append(dict(width=sw,height=sh,xs=xs,ys=ys,xp=xp,yp=yp,pixels=sw*sh))
    ...
    info = dict(sources=sources, actual_resize_golden=True, profile=profile, width=width, height=height, stage_count=len(nodes),
