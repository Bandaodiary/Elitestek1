"""C16 generated operator/DAG plan, retained C8 row pipeline and numeric golden."""
...
RENAMES={'c1_r2_mapped_window_store':'c1_r2_bulk_window_store','c1_r2_pw_banked_feeder':'c1_r2_pw_bulk_feeder',
         ...
         'c1_r2_tensor_row_loader':'c1_r2_tensor_stream_loader','c1_r2_microstyle_graph':'c1_r2_planned_pingpong_graph',
         'c1_r2_tensor_row_writer':'c1_r2_tensor_pingpong_writer'}
SOURCES=[next((s.replace(a,b) for a,b in RENAMES.items() if a in s),s) for s in GRAPH_SOURCES]+['rtl/r2/c1_r2_microstyle_plan.sv']
...
def main():
    ...
    top='tb_c1_r2_planned_pingpong_graph'
    with tempfile.TemporaryDirectory(prefix='c1_r2_plan_graph_',dir=ROOT/'sim') as tmp:
        for shape in args.shapes.split(','):
            w,h=map(int,shape.split('x'));folder=Path(tmp)/shape;folder.mkdir();m=vectors(folder,w,h)
            print('C1_R2_PLAN_GRAPH_VECTORS '+json.dumps(dict(m,cache=args.cache,overlap=args.overlap,compute_overlap=args.compute_overlap,refill_priority=args.refill_priority,memdiv=args.memory_div,latency=args.latency)),flush=True)
            for stalls in map(int,args.stalls.split(',')):
                ...
                if args.negative_only:
                    for fault,marker in ((1,'PINGPONG_MISMATCH stage=1'),(2,'unwritten/wrong producer stage=1')):
                        ...
                        print(f'C1_R2_PLAN_GRAPH_HANDOFF_NEGATIVE_PASS corruption={fault} detected_at_stage=1',flush=True)
                    continue
                c=subprocess.run(compile_cmd,capture_output=True,text=True,timeout=60)
                if c.returncode:raise RuntimeError(c.stderr[-3000:])
                pass_marker='C1_R2_PLAN_GRAPH_RESET_SUITE_PASS ' if args.reset_only else 'C1_R2_PLAN_GRAPH_PASS '
                ...
                with subprocess.Popen(run_cmd,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True) as child:
                    ...
                    try:
                        for raw in child.stdout:
                            ...
                            if s.startswith('C1_R2_PLAN_GRAPH_'):print(s,flush=True)
                            pass_count+=s.startswith(pass_marker);bad|='FATAL' in s or 'ERROR' in s
                        code=child.wait()
                    finally:timer.cancel()
                if code or bad or pass_count!=1:raise RuntimeError('\n'.join(tail))
    print('C1_R2_PLAN_GRAPH_CLEAN temporary_vectors_and_simulator_removed=1',flush=True)
