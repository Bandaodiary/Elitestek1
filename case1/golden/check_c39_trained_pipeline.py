...
from c39_trained_contract import ROOT,bound_candidate,candidate_nodes,camera_geometry
...
from c39_trained_contract import SOURCES,PLAN_SOURCE,FUSION_SOURCE,TOP,SIM,source_gate
from run_c39_trained_host_probe import CASES
...
def matrix(folder,status,config,provenance):
    ...
    entries=list(re.finditer(r'^C39_TRAINED_HOST_CASE_BEGIN ([^\r\n]+)\r?\n(.*?)^C39_TRAINED_HOST_CASE_END (\S+)\s*$',text,re.M|re.S))
    ...
    for expected,entry in zip(CASES,entries):
        ...
        body=entry[2]
        command=one_json(body,'C39_ACTUAL_COMPILE ')
        parameters={}
        for word in command:
        ...
        run_command=one_json(body,'C39_ACTUAL_RUN ')
        ...
        if expected['negative']:
            observed=one_json(body,'C39_NEGATIVE_OBSERVED ')
            ...
            printed=one_json(body,'C39_POSITIVE_CHECK ')
    ...
    summary=one_json(text,'C39_TRAINED_HOST_MATRIX_PASS ')
...
def xsim(folder,status,phase,config,provenance):
    ...
    if len(re.findall(r'^# xsim \{c39_trained_'+phase+r'\}',launch,re.M))!=1:
...
def main():
    ...
    folder=ROOT/'logs/c39_trained_host_runs'/args.run
    status=load(folder/'status.json')
    private=ROOT/'sim'/('c1_c39_trained_host_'+args.run)
    ...
    if status.get('production_RTL_changed') is not True or status.get('candidate')!='C39_NATIVE':
    ...
    if status.get('after_run'):
        ...
        prior_private=ROOT/'sim'/('c1_c39_trained_host_'+status['after_run'])
        ...
        prior_payload=one_json((folder/('predecessor_'+prior_phase+'_preflight.tail.log')).read_text(encoding='utf-8-sig'),
                               'C39_TRAINED_PIPELINE_PHASE_PASS ')
    ...
    verify_history(status,['c39_operator_preflight','c39_source_preflight'])
    ...
    print('C39_TRAINED_PIPELINE_PHASE_PASS '+json.dumps(payload,separators=(',',':')))
