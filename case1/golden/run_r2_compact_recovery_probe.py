"""C21 recoverable response errors in the compact production candidate."""
...
from run_r2_compact_host_probe import ROOT, SOURCES, PLAN_SOURCE, SIM
from r2_plan_vectors import vectors, compile_package, profile_nodes

TOP = 'tb_c1_r2_compact_recovery_system'
PREFIX = 'C1_R2_COMPACT_RECOVERY_'
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='c1_r2_compact_recovery_', dir=ROOT/'sim') as td:
        root = Path(td)
        for shape in args.shapes.split(','):
            ...
            print(PREFIX+'FIXTURE '+json.dumps(dict(width=w, height=h, stage_count=meta['stage_count'],
                  ...
                  frame_scalars=meta['frames'][0]['scalars'], actual_c21_host=True)), flush=True)
