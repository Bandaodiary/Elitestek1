"""C14 fresh-input leases + C13 CPU adapter; unchanged CNN numeric golden."""
...
SOURCES=C12+['rtl/r2/c1_r2_cpu_axi_adapter.sv','rtl/r2/c1_r2_video_fresh_leases.sv','rtl/r2/c1_r2_video_fresh_system.sv']
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='c1_r2_fresh_video_',dir=ROOT/'sim') as td:
        root=Path(td)
        if a.compile_only:
            c=subprocess.run(['D:/iverilog/bin/iverilog.exe','-g2012','-s','c1_r2_video_fresh_system','-o',str(root/'system.vvp'),*[str(ROOT/s) for s in SOURCES]],capture_output=True,text=True,timeout=90)
            if c.returncode:raise RuntimeError(c.stderr[-4000:])
            print(f'C1_R2_FRESH_VIDEO_SYSTEM_COMPILE_PASS sources={len(SOURCES)} scope=retained_core_plus_adapter_sources')
        else:
            for shape in a.shapes.split(','):
                w,h=map(int,shape.split('x'));folder=root/shape;folder.mkdir();m=vectors(folder,w,h)
                print('C1_R2_FRESH_VIDEO_SYSTEM_VECTORS '+json.dumps(m),flush=True)
                for stalls in map(int,a.stalls.split(',')):
                    exe=folder/'system.vvp';top='tb_c1_r2_fresh_video_system'
                    for negative in ((1,2) if a.negative_only else (0,)):
                        ...
                        if negative:
                            ...
                            if r.returncode==0 or marker not in r.stdout or 'C1_R2_FRESH_VIDEO_SYSTEM_PASS ' in r.stdout:raise RuntimeError('negative control did not trip: '+r.stdout[-2000:])
                            print(f'C1_R2_FRESH_VIDEO_SYSTEM_NEGATIVE_PASS width={w} height={h} stalls={stalls} corruption={negative} actual_ram_mutation=1',flush=True)
                        else:
                            for line in r.stdout.splitlines():
                                if line.startswith('C1_R2_FRESH_VIDEO_SYSTEM_'):print(line,flush=True)
                            if r.returncode or r.stdout.count('C1_R2_FRESH_VIDEO_SYSTEM_PASS ')!=1 or 'FATAL' in r.stdout or 'ERROR' in r.stdout:raise RuntimeError((r.stdout+r.stderr)[-4000:])
    print('C1_R2_FRESH_VIDEO_SYSTEM_CLEAN temporary_vectors_and_simulator_removed=1',flush=True)
