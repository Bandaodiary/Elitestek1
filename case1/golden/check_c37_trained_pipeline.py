"""Independent C37 phase checks: actual commands, graph math, full host contract.
...
from c37_trained_contract import ROOT,bound_candidate,candidate_nodes,camera_geometry
...
from c37_native_performance_contract import require_native_target
from c37_trained_contract import SOURCES,PLAN_SOURCE,FUSION_SOURCE,TOP,SIM,source_gate
from run_c37_trained_host_probe import CASES
...
def matrix(folder,status,config,provenance):
    ...
    entries=list(re.finditer(r'^C37_TRAINED_HOST_CASE_BEGIN ([^\r\n]+)\r?\n(.*?)^C37_TRAINED_HOST_CASE_END (\S+)\s*$',text,re.M|re.S))
    ...
    for expected,entry in zip(CASES,entries):
        ...
        body=entry[2]
        command=one_json(body,'C37_ACTUAL_COMPILE ')
        parameters={}
        for word in command:
        ...
        if [Path(word).resolve() for word in command if word.endswith('.sv')]!=source_paths:
            raise ValueError('actual Icarus sources differ from six C37 replacements plus two trained plans')
        run_command=one_json(body,'C37_ACTUAL_RUN ')
        ...
        if expected['negative']:
            observed=one_json(body,'C37_NEGATIVE_OBSERVED ')
            ...
            printed=one_json(body,'C37_POSITIVE_CHECK ')
    ...
    summary=one_json(text,'C37_TRAINED_HOST_MATRIX_PASS ')
...
def xsim(folder,status,phase,config,provenance):
    ...
    if [Path(s).resolve() for s in load(folder/(phase+'.sources.json'))]!=expected_sources or len(expected_sources)!=48:
    ...
    if len(re.findall(r'^# xsim \{c37_trained_'+phase+r'\}',launch,re.M))!=1:
...
def main():
    ...
    folder=ROOT/'logs/c37_trained_host_runs'/args.run
    status=load(folder/'status.json')
    private=ROOT/'sim'/('c1_c37_trained_host_'+args.run)
    ...
    if status.get('production_RTL_changed') is not True or status.get('candidate')!='C37':
        raise ValueError('C37 candidate provenance missing')
    ...
    if status.get('after_run'):
        ...
        prior_private=ROOT/'sim'/('c1_c37_trained_host_'+status['after_run'])
        ...
        prior_payload=one_json((folder/('predecessor_'+prior_phase+'_preflight.tail.log')).read_text(encoding='utf-8-sig'),
                               'C37_TRAINED_PIPELINE_PHASE_PASS ')
    ...
    print('C37_TRAINED_PIPELINE_PHASE_PASS '+json.dumps(payload,separators=(',',':')))
