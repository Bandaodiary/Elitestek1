"""C22 recoverable response errors in the overlay production candidate."""
...
from run_r2_overlay_host_probe import ROOT, SOURCES, PLAN_SOURCE, SIM
from r2_plan_vectors import vectors, compile_package, profile_nodes

TOP = 'tb_c1_r2_overlay_recovery_system'
PREFIX = 'C1_R2_OVERLAY_RECOVERY_'
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='c1_r2_overlay_recovery_', dir=ROOT/'sim') as td:
        root = Path(td)
        for shape in args.shapes.split(','):
            ...
            print(PREFIX+'FIXTURE '+json.dumps(dict(width=w, height=h, stage_count=meta['stage_count'],
                  ...
                  frame_scalars=meta['frames'][0]['scalars'], actual_c22_host=True)), flush=True)
