...
TOP = 'tb_c1_r2_resize_host_faults'
PREFIX = 'C1_R2_RESIZE_HOST_FAULT_'


def main():
    p = argparse.ArgumentParser()
    p.add_argument('--fault-mode', type=int,choices=(1,2,3),default=1)
    ...
    p.add_argument('--nn-target', type=int, choices=range(2, 7), default=3)
    a = p.parse_args()
            ...
            for shape in a.shapes.split(','):
                ...
                for stalls in map(int, a.stalls.split(',')):
                    for negative in ((1, 2) if a.negative_only else (0,)):
                        ...
                        opts = dict(FAULT_MODE=a.fault_mode, WIDTH=w, HEIGHT=h, STALLS=stalls, AW_WAIT_W=a.aw_wait_w,
