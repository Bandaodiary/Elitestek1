"""C28 whole-frame skip/disable/re-sync through actual Resize/Capture."""
import subprocess
import tempfile
from pathlib import Path
from run_r2_camera_safe_capture_probe import ROOT,SOURCES,EXTRA,vectors


def main():
    top='tb_c1_r2_camera_safe_capture_lifecycle'
    with tempfile.TemporaryDirectory(prefix='c1_r2_camera_safe_lifecycle_',dir=ROOT/'sim') as td:
        ...
        for stalls in (0,1):
            ...
            if r.returncode or r.stdout.count('C1_R2_CAMERA_SAFE_LIFECYCLE_PASS ')!=1:raise RuntimeError((r.stdout+r.stderr)[-3000:])
    print('C1_R2_CAMERA_SAFE_LIFECYCLE_CLEAN temporary_vectors_and_simulator_removed=1',flush=True)
