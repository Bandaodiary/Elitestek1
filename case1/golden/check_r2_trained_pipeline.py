"""Independent C36 phase checks: actual commands, graph math, full host contract.
...
from r2_trained_style_vectors import ROOT,bound_candidate,candidate_nodes,camera_geometry
...
from run_r2_fused_rgb2_host_probe import SOURCES,PLAN_SOURCE,FUSION_SOURCE,TOP,SIM
from run_r2_trained_host_probe import CASES
...
def matrix(folder,status,config,provenance):
    ...
    entries=list(re.finditer(r'^C36_TRAINED_HOST_CASE_BEGIN ([^\r\n]+)\r?\n(.*?)^C36_TRAINED_HOST_CASE_END (\S+)\s*$',text,re.M|re.S))
    ...
    for expected,entry in zip(CASES,entries):
        ...
        body=entry[2]
        command=one_json(body,'C36_ACTUAL_COMPILE ')
        parameters={}
        for word in command:
        ...
        if [Path(word).resolve() for word in command if word.endswith('.sv')]!=source_paths:
            raise ValueError('actual Icarus sources differ from unchanged RTL plus two trained plans')
        run_command=one_json(body,'C36_ACTUAL_RUN ')
        ...
        if expected['negative']:
            observed=one_json(body,'C36_NEGATIVE_OBSERVED ')
            ...
            printed=one_json(body,'C36_POSITIVE_CHECK ')
    ...
    summary=one_json(text,'C36_TRAINED_HOST_MATRIX_PASS ')
...
def xsim(folder,status,phase,config,provenance):
    ...
    if len(re.findall(r'^# xsim \{c36_trained_'+phase+r'\}',launch,re.M))!=1:
...
def main():
    ...
    folder=ROOT/'logs/r2_trained_host_runs'/args.run
    status=load(folder/'status.json')
    private=ROOT/'sim'/('c1_r2_trained_host_'+args.run)
    ...
    if status.get('after_run'):
        ...
        prior_private=ROOT/'sim'/('c1_r2_trained_host_'+status['after_run'])
        ...
        prior_payload=one_json((folder/('predecessor_'+prior_phase+'_preflight.tail.log')).read_text(encoding='utf-8-sig'),
                               'C36_TRAINED_PIPELINE_PHASE_PASS ')
    ...
    print('C36_TRAINED_PIPELINE_PHASE_PASS '+json.dumps(payload,separators=(',',':')))
