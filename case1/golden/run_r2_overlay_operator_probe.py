"""C22: run independent retained C5 references through actual overlay-engine bulk loads."""
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='c1_r2_overlay_operator_', dir=ROOT/'sim') as td:
        folder = Path(td)
        m = dict(commands=0, vectors=0, jobs=0) if a.boundary_only else vectors(folder)
        print('C1_R2_OVERLAY_OPERATOR_VECTORS '+json.dumps(m, separators=(',', ':')), flush=True)
        top = 'tb_c1_r2_cnn_overlay_engine'
        for stalls in map(int, a.stalls.split(',')):
            ...
            for line in r.stdout.splitlines():
                if line.startswith('C1_R2_OVERLAY_OPERATOR_'):
                    print(line, flush=True)
            marker = 'C1_R2_OVERLAY_OPERATOR_BOUNDARY_PASS ' if a.boundary_only else 'C1_R2_OVERLAY_OPERATOR_PASS '
            if r.returncode or r.stdout.count(marker) != 1 or 'FATAL' in r.stdout:
                raise RuntimeError((r.stdout+r.stderr)[-3500:])
    print('C1_R2_OVERLAY_OPERATOR_CLEAN temporary_vectors_and_simulator_removed=1', flush=True)
