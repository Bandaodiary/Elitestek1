...
with tempfile.TemporaryDirectory(prefix='c1_r2_rgbx_driver_',dir=ROOT/'sim') as td:
    obj=Path(td)/'video.o'
    c=subprocess.run([str(TOOL/'riscv-none-elf-gcc.exe'),'-march=rv32imac','-mabi=ilp32','-std=c11','-O2','-Wall','-Wextra','-Werror','-ffreestanding',
                      '-I',str(ROOT/'software/include'),'-c',str(ROOT/'software/src/c1_r2_rgbx_video.c'),'-o',str(obj)],capture_output=True,text=True,timeout=30)
    ...
    for symbol in ('c1_r2x_control','c1_r2x_result_read','c1_r2x_interrupts'):
    ...
    print(f'C1_R2_RGBX_DRIVER_COMPILE_PASS rv32imac=1 ilp32=1 functions=3 fences={fences} hardware_execution=0')
print('C1_R2_RGBX_DRIVER_CLEAN temporary_object_removed=1')
