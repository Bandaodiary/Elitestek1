"""C25 unstoppable RGB -> ROI/CDC -> real Resize/Capture AXI; not CNN/board FPS."""
import argparse
import json
import subprocess
import tempfile
...
EXTRA=['rtl/r2/c1_r2_async_pixel_fifo.sv','rtl/r2/c1_r2_camera_ingress.sv',
...
def main():
    ...
    a=p.parse_args();top='tb_c1_r2_camera_capture'
    ...
    if a.vectors_only:
        ...
        print('C1_R2_CAMERA_VECTORS '+json.dumps(meta),flush=True);return
    if not set(a.stalls.split(','))<={'0','1'}:p.error('stalls must be 0 or 1')
    with tempfile.TemporaryDirectory(prefix='c1_r2_camera_capture_',dir=ROOT/'sim') as td:
        folder=Path(td);plus=vectors(folder,*geometry)
        print('C1_R2_CAMERA_VECTORS '+json.dumps(dict(parameters=opts,plusargs=plus,source_backpressure=False,cnn_included=False)),flush=True)
        for stalls in map(int,a.stalls.split(',')):
            ...
            if r.returncode or r.stdout.count('C1_R2_CAMERA_CAPTURE_PASS ')!=1:raise RuntimeError((r.stdout+r.stderr)[-3500:])
    print('C1_R2_CAMERA_CAPTURE_CLEAN temporary_vectors_and_simulator_removed=1',flush=True)
