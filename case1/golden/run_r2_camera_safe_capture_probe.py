"""C28 unstoppable RGB -> ROI/CDC -> real Resize/Capture AXI; not CNN/board FPS."""
import argparse
import json
import subprocess
import tempfile
...
EXTRA=['rtl/r2/c1_r2_async_pixel_fifo_guarded.sv','rtl/r2/c1_r2_camera_ingress_guarded.sv',
...
def vectors(folder,sw,sh,rx,ry,rw,rh,ow,oh):
...
def main():
    ...
    a=p.parse_args();top='tb_c1_r2_camera_safe_capture'
    ...
    opts.update(FD=a.fifo or (1024 if a.native else 64),FAULTS=0 if a.native else 1)
    ...
    if a.native:opts.update(CORE_HALF=3.333,CAM_HALF=6.734,HBLANK=280,VBLANK=45)
    if a.vectors_only:
        ...
        meta=dict(parameters=opts,plusargs=vectors(a.vectors_only,*geometry),source_backpressure=False,
                  native_shape=a.native,cnn_included=False)
        (a.vectors_only/'metadata.json').write_text(json.dumps(meta),encoding='utf-8')
        print('C1_R2_CAMERA_SAFE_VECTORS '+json.dumps(meta),flush=True);return
    if not set(a.stalls.split(','))<={'0','1'}:p.error('stalls must be 0 or 1')
    with tempfile.TemporaryDirectory(prefix='c1_r2_camera_safe_capture_',dir=ROOT/'sim') as td:
        folder=Path(td);plus=vectors(folder,*geometry)
        print('C1_R2_CAMERA_SAFE_VECTORS '+json.dumps(dict(parameters=opts,plusargs=plus,source_backpressure=False,cnn_included=False)),flush=True)
        for stalls in map(int,a.stalls.split(',')):
            ...
            c=subprocess.run(['D:/iverilog/bin/iverilog.exe','-g2012','-s',top,
                ...
                *[str(ROOT/s) for s in SOURCES+EXTRA+['sim/c1_r2_axi_memory_bfm.sv',f'sim/{top}.sv']]],capture_output=True,text=True,timeout=90)
            ...
            if r.returncode or r.stdout.count('C1_R2_CAMERA_SAFE_CAPTURE_PASS ')!=1:raise RuntimeError((r.stdout+r.stderr)[-3500:])
    print('C1_R2_CAMERA_SAFE_CAPTURE_CLEAN temporary_vectors_and_simulator_removed=1',flush=True)
