...
SOURCES = [(PROJECT.parent/e.attrib['name']).resolve().relative_to(ROOT).as_posix()
           ...
           if e.tag.rsplit('}', 1)[-1] == 'design_file' and e.attrib['name'] != 'c1_ti60_r2_credit_rgb2_host96.sv']
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='c1_r2_credit_rgb2_host_', dir=temp_parent) as td:
        root = Path(td)
        if a.compile_only:
            ...
            c = subprocess.run(['D:/iverilog/bin/iverilog.exe', '-g2012', '-s', 'c1_r2_credit_rgb2_host_system',
...
if __name__ == '__main__':
    ...
    if '--paired' in sys.argv:
        ...
        import run_r2_rgb2_host_probe as retained
        retained.main()
    main()
