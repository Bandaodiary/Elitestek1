"""C31 feature-resize generated plan through real RGBX/AXI/video/host RTL, not CPU IP."""
...
from r2_fused_camera_vectors import ROOT, camera_geometry, vectors, compile_package, profile_nodes
from r2_plan_package import verify

PROJECT = ROOT/'efinity/c1_ti60_r2_fused_rgb2_host96.xml'
PLAN_SOURCE = 'rtl/r2/c1_r2_row_fused_microstyle_plan.sv'
FUSION_SOURCE = 'rtl/r2/c1_r2_row_fusion_plan.sv'
...
SOURCES = [(PROJECT.parent/e.attrib['name']).resolve().relative_to(ROOT).as_posix()
           ...
           if e.tag.rsplit('}', 1)[-1] == 'design_file' and e.attrib['name'] != 'c1_ti60_r2_fused_rgb2_host96.sv']
...
def main():
    ...
    p.add_argument('--frame-divisor',type=int,choices=(1,2,3),default=2)
    ...
    with tempfile.TemporaryDirectory(prefix='c1_r2_fused_rgb2_host_', dir=temp_parent) as td:
        root = Path(td)
        if a.compile_only:
            ...
            c = subprocess.run(['D:/iverilog/bin/iverilog.exe', '-g2012', '-s', 'c1_r2_fused_rgb2_host_system',
                                '-o', str(root/'host.vvp'), *[str(ROOT/s) for s in SOURCES]],
                               capture_output=True, text=True, timeout=90)
            if c.returncode:
                raise RuntimeError(c.stderr[:1800]+'\n'+c.stderr[-1800:])
            ...
            for shape in a.shapes.split(','):
                ...
                m['frame_divisor']=a.frame_divisor;m['source_pixels_per_word']=2;m['actual_vs_de']=True
                print(PREFIX+'VECTORS '+json.dumps(m), flush=True)
                sources = [str(ROOT/s) for s in SOURCES if s not in (PLAN_SOURCE,FUSION_SOURCE)]+[str(folder/'package/execution_plan.sv'),str(folder/'fusion_plan.sv')]
                for stalls in map(int, a.stalls.split(',')):
                    for negative in ((1, 2) if a.negative_only else (0,)):
                        ...
                        opts = dict(WIDTH=w, HEIGHT=h, STALLS=stalls, AW_WAIT_W=a.aw_wait_w,
                                    FRAME_DIVISOR=a.frame_divisor,CAMERA_SW=sw,CAMERA_SH=sh,CAMERA_RX=rx,CAMERA_RY=ry,CAMERA_RW=rw,CAMERA_RH=rh,
                                    NN_TARGET=a.nn_target, NEGATIVE_CONTROL=negative,
                                    STAGE_COUNT=m['stage_count'], RGB_STAGE=m['rgb_stage'],FUSED_DW_STAGE=m['dw_stage'],FUSED_PW_STAGE=m['pw_stage'])
