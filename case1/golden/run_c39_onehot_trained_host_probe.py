"""C39 native-format regression on the actual AXI/video/host RTL."""
import argparse
import json
from pathlib import Path
import subprocess

from c39_onehot_trained_contract import (ROOT,bound_candidate,build_vectors,camera_geometry,
    SOURCES,PLAN_SOURCE,FUSION_SOURCE,SIM,TOP,PREFIX,source_gate)
...
def main():
    ...
    for case in CASES:
        ...
        print('C39_ONEHOT_TRAINED_HOST_CASE_BEGIN '+json.dumps(case,separators=(',',':')),flush=True)
        ...
        print('C39_ONEHOT_ACTUAL_COMPILE '+json.dumps(command),flush=True)
        ...
        print('C39_ONEHOT_ACTUAL_RUN '+json.dumps(command),flush=True)
        ...
        if case['negative']:
            ...
            print('C39_ONEHOT_NEGATIVE_OBSERVED '+json.dumps(dict(record,fatal_lines=actual)),flush=True)
        else:
            ...
            print('C39_ONEHOT_POSITIVE_CHECK '+json.dumps(record,separators=(',',':')),flush=True)
        results.append(record)
        print('C39_ONEHOT_TRAINED_HOST_CASE_END '+case['id'],flush=True)
    print('C39_ONEHOT_TRAINED_HOST_MATRIX_PASS '+json.dumps(dict(model_binding=provenance,
