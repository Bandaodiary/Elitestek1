"""C30 banked Resize regression against retained R1 arithmetic and lifecycle."""
...
SOURCES = ['rtl/common/c1_ram_sdp_read_first.sv', 'rtl/video/r1_resize_request_q16.sv',
           'rtl/video/r1_bilinear_interp_rgb888.sv', 'rtl/r2/c1_r2_resize_overlap_system.sv',
           'rtl/r2/c1_r2_resize_pair_ram.sv', 'rtl/r2/c1_r2_resize_line_sampler.sv',
           'rtl/r2/c1_r2_resize_overlap_pipeline.sv']
...
def main():
    ...
    if a.vectors_only:
        ...
        print('C30_RESIZE_OVERLAP_FIXTURE '+json.dumps({k:v for k,v in m.items() if k!='cases'}),flush=True)
        return
    with tempfile.TemporaryDirectory(prefix='c30_resize_overlap_',dir=ROOT/'sim') as td:
        folder=Path(td)
        m=generate(folder,20260913)
        print('C30_RESIZE_OVERLAP_FIXTURE '+json.dumps({k:v for k,v in m.items() if k!='cases'}),flush=True)
        top='tb_c1_r2_resize_overlap_pipeline'
        for reset in (0,1):
            exe=folder/'test.vvp'
            ...
            for line in r.stdout.splitlines():
                if line.startswith(('C1_','C30_')): print(line+f' registered_abort_reset={reset}',flush=True)
            if r.returncode or r.stdout.count('C30_RESIZE_OVERLAP_PIPELINE_PASS ')!=1 or 'FATAL' in r.stdout:
                raise RuntimeError((r.stdout+r.stderr)[-4000:])
    print('C30_RESIZE_OVERLAP_CLEAN temporary_vectors_and_simulator_removed=1',flush=True)
