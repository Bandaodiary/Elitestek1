"""C31 source faults in the actual CPU/video/CNN shell; no direct RAM seeding."""
import argparse
import subprocess
import tempfile
from pathlib import Path
from run_r2_credit_rgb2_host_probe import ROOT, SOURCES, PLAN_SOURCE, SIM
from r2_camera_plan_vectors import vectors, compile_package, profile_nodes, camera_geometry

TOP = 'tb_c1_r2_credit_rgb2_host_faults'
...
def main():
    ...
    p.add_argument('--aw-wait-w',type=int,choices=(0,1,2),default=2)
    a = p.parse_args()
    ...
    with tempfile.TemporaryDirectory(prefix='c1_r2_credit_rgb2_host_fault_', dir=temp_parent) as td:
        root = Path(td)
        ...
        for fault in faults:
            for stall in stalls:
                ...
                opts = dict(WIDTH=8, HEIGHT=8, STALLS=stall, AW_WAIT_W=a.aw_wait_w, FAULT_MODE=fault, NN_TARGET=6,
