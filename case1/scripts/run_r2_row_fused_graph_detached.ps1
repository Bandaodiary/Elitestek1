...
[CmdletBinding()]
param(
    [switch]$Worker,[switch]$CheckOnly,[string]$RunId='c35_row_fused_graph_20260914_a',
    [ValidateRange(1,1800)][int]$TimeoutSeconds=1200
)
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
if($RunId -notmatch '^[A-Za-z0-9_-]+$'){throw 'invalid RunId'}
$depRun='c35_shadow_engine_20260914_c'
$depPid=28424;$depStart='2026-09-14T23:03:27.3307306+08:00'
$depStatus=Join-Path $caseRoot "logs\r2_shadow_engine_runs\$depRun\status.json"
$simRoot=(Resolve-Path (Join-Path $caseRoot 'sim')).Path
$depPrivate=Join-Path $simRoot "c1_r2_shadow_engine_$depRun"
$runRoot=Join-Path $simRoot "c1_r2_row_fused_graph_$RunId"
$runLog=Join-Path $caseRoot "logs\r2_row_fused_graph_runs\$RunId"
...
public static class C35GraphJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $current=[Diagnostics.Process]::GetCurrentProcess();$inJob=$false
    if(-not [C35GraphJobCheck]::IsProcessInJob($current.Handle,[IntPtr]::Zero,[ref]$inJob)){throw 'cannot verify Windows Job'}
    ...
    $state='running';$step='component';$message='full DAG row-fusion and spatial-source lifetime regression';Save-State
    $info=New-Object Diagnostics.ProcessStartInfo
    $info.FileName='D:\miniconda\miniconda\envs\p300_task3_bci3\python.exe'
    $info.Arguments='-B -u "'+(Join-Path $caseRoot 'golden\run_r2_row_fused_graph_probe.py')+'" --engine-run '+$depRun+' --temporary-parent "'+$runRoot+'"'
    ...
    if($child.ExitCode -ne 0 -or $err.Trim() -or
       [regex]::Matches($out,'(?m)^C35_ROW_FUSED_GRAPH_CASE ').Count -ne 5 -or
       [regex]::Matches($out,'(?m)^C35_ROW_FUSED_GRAPH_NEGATIVE_PASS ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C35_ROW_FUSED_GRAPH_PAIRED ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C35_SHADOW_ENGINE_EVIDENCE_PASS ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C35_ROW_FUSED_GRAPH_SUMMARY ').Count -ne 1 -or
       $out -notmatch '(?m)^C35_ROW_FUSED_GRAPH_CLEAN temporary_vectors_and_simulator_removed=1'){
        throw 'C35 full DAG evidence incomplete or failed'
    }
    $state='complete';$step='done';$message='full DAG row-transfer math/spill removal passed; not actual AXI/PNR/native fps'
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or
       (Split-Path -Leaf $resolved) -ne "c1_r2_row_fused_graph_$RunId"){throw 'unsafe own C35 cleanup target'}
