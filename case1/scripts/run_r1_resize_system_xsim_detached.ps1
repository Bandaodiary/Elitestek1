...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $caseRoot "logs\r1_resize_system_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "r1_resize_system_xsim_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_resize_system_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_r1_resize_system_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached R1 resize system verification worker started'

    Invoke-LoggedStep -Name 'vector_gen' -Tool $pythonExe -Arguments @(
        (Join-Path $goldenRoot 'generate_r1_resize_system_vectors.py'),
        '--output-dir', $runRoot,
        '--seed', '20260824'
    )
    Invoke-LoggedStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_r1_resize_system.sv')
        )
    Invoke-LoggedStep -Name 'xelab' -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @('tb_c1_r1_resize_system', '-s', 'tb_c1_r1_resize_system_sim')
    Invoke-LoggedStep -Name 'xsim' -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_r1_resize_system_sim', '-runall')

    $manifest = Get-Content -Raw -LiteralPath `
        (Join-Path $runRoot 'r1_resize_system_manifest.json') | ConvertFrom-Json
    $expectedPass = "C1_R1_RESIZE_SYSTEM_PASS configs=$($manifest.configs) " +
                    "requests=$($manifest.requests) outputs=$($manifest.outputs)"
