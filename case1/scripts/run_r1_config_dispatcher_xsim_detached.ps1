...
# Vivado/xsim are launched only by the hidden WMI worker below.  They are not
# descendants of the interactive Codex Windows Job and continue if that shell
# is interrupted.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "r1_config_dispatcher_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_r1_config_dispatcher_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_config_dispatcher_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'r1_config_dispatcher_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached R1 config-dispatcher worker started'
    Invoke-VivadoStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            '-sv',
            (Join-Path $rtlRoot 'control\c1_r1_config_dispatcher.sv'),
            (Join-Path $simRoot 'tb_c1_r1_config_dispatcher.sv')
        )
    Invoke-VivadoStep -Name 'xelab_r1_config_dispatcher' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @(
            'tb_c1_r1_config_dispatcher',
            '-s',
            'tb_c1_r1_config_dispatcher_sim'
        )
    Invoke-VivadoStep -Name 'xsim_r1_config_dispatcher' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_r1_config_dispatcher_sim', '-runall') `
        -ExpectedPass 'C1_R1_CONFIG_DISPATCHER_PASS'
    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'R1 config-dispatcher xsim regression complete'
} catch {
    $script:totalWatch.Stop()
