...
# Native 640x480 descriptor/parameter-arena preflight.
#
# The worker is launched outside the caller's Windows Job.  Only compact
# status/stdout/stderr files are retained; Vivado's run directory is removed
# after each invocation.  This test intentionally stops before the native
# CNN data plane, so it is safe to run before a board is available.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "native_artifact_preflight_runs\$RunId\status.json"
    $powerShell = "$env:SystemRoot\System32\WindowsPowerShell\v1.0\powershell.exe"
    $commandLine = "`"$powerShell`" -NoLogo -NoProfile -NonInteractive " +
        "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$PSCommandPath`" " +
        "-Worker -RunId $RunId" + $(if ($CompileOnly) { ' -CompileOnly' } else { '' })
    $workerPid = $null
    try {
...
$runRoot = Join-Path $simRoot "xsim_run_native_artifact_preflight_$RunId"
$runLogRoot = Join-Path $logRoot "native_artifact_preflight_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'native_artifact_preflight_status.json'
...
function Invoke-XsimStep {
    ...
    try {
        ...
        # Vivado occasionally emits a CR/LF-only stderr file; reject only
        # actual non-whitespace diagnostics.
...
try {
    Write-Status running setup 0 'detached native artifact preflight started'
    ...
    $sources = @(
        ...
        (Join-Path $simRoot 'tb_c1_r1_native_artifact_preflight.sv')
    )
    Invoke-XsimStep xvlog (Join-Path $vivadoBin 'xvlog.bat') `
        (@('-sv','-d','C1_LOCAL_NATIVE_ARTIFACT_VECTORS') + $sources)
    Invoke-XsimStep xelab (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_r1_native_artifact_preflight','-s','tb_c1_r1_native_artifact_preflight_sim')
    if ($CompileOnly) {
        $marker = 'C1_R1_NATIVE_ARTIFACT_PREFLIGHT_ELAB_PASS frame=640x480 stages=22 parameter_bytes=16896'
        # xelab has no user testbench process in this mode, so make the
        # structural gate explicit in the compact log and status file.
    ...
    Invoke-XsimStep xsim (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_r1_native_artifact_preflight_sim','-runall') 'C1_R1_NATIVE_ARTIFACT_PREFLIGHT_PASS'
    $watch.Stop(); Write-Status complete done 0 'C1_R1_NATIVE_ARTIFACT_PREFLIGHT_PASS'
} catch {
