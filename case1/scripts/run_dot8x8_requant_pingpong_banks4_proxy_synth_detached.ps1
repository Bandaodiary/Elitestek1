...
# Detached, self-cleaning Vivado proxy for the four-bank MAC scaling point.
...
$family = 'dot8x8_requant_pingpong_banks4_proxy_synth'
...
try {
    Set-Status 'running' 'vivado' 0 'detached four-bank ping-pong proxy started'
    # Warm only the installed runtime scripts; the private project is deleted.
    ...
    $tcl = Join-Path $scriptRoot 'synth_dot8x8_requant_pingpong_banks4_proxy.tcl'
    ...
    $marker = 'C1_DOT8X8_REQUANT_PINGPONG_BANKS4_PROXY_SYNTH_PASS'
    ...
    $metrics = [ordered]@{ part = 'xc7a200tsbg484-1'; banks = 4; lanes = 2;
        tag_fifo_depth = 16; slice_luts = Metric $util 'Slice LUTs*';
