...
# Win32_Process.Create starts the hidden worker outside the calling Codex
# shell's Windows Job object.  A run is successful only when xsim emits exactly
# one unique PASS marker, every tool exits zero, stderr is empty, and no
# Fatal/Error/FAIL diagnostic appears in any step output.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "descriptor_reader_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_descriptor_reader_run_$RunId"
$runLogRoot = Join-Path $logRoot "descriptor_reader_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'descriptor_reader_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached AXI descriptor-reader worker started'
    Invoke-VivadoStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            '-sv',
            (Join-Path $rtlRoot 'dma\c1_axi_descriptor_reader.sv'),
            (Join-Path $simRoot 'tb_c1_axi_descriptor_reader.sv')
        )
    Invoke-VivadoStep -Name 'xelab_descriptor_reader' -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @('tb_c1_axi_descriptor_reader', '-s', 'tb_c1_axi_descriptor_reader_sim')
    Invoke-VivadoStep -Name 'xsim_descriptor_reader' -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_axi_descriptor_reader_sim', '-runall') `
        -ExpectedPass 'C1_AXI_DESCRIPTOR_READER_PASS'
    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'AXI descriptor-reader xsim regression complete'
} catch {
    $script:totalWatch.Stop()
