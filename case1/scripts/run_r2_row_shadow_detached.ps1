# C35 isolated component test, serially after the already-running C34 pipeline.
# The waiting worker never kills, restarts or cleans its predecessor.
[CmdletBinding()]
param(
    [switch]$Worker,[switch]$CheckOnly,[string]$RunId='c35_row_shadow_20260914_a',
    [ValidateRange(1,1800)][int]$TimeoutSeconds=300
)
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
if($RunId -notmatch '^[A-Za-z0-9_-]+$'){throw 'invalid RunId'}
$depRun='c34_validation_after_writer_20260914_a'
$depPid=32300;$depStart='2026-09-14T21:07:36.5539398+08:00'
$depStatus=Join-Path $caseRoot "logs\r2_validation_pipeline_runs\$depRun\status.json"
$simRoot=(Resolve-Path (Join-Path $caseRoot 'sim')).Path
$depPrivate=Join-Path $simRoot "c1_r2_c34_pipeline_$depRun"
$runRoot=Join-Path $simRoot "c1_r2_row_shadow_$RunId"
$runLog=Join-Path $caseRoot "logs\r2_row_shadow_runs\$RunId"
...
function Pin-Predecessor{
    ...
    if($p.ProcessName -ne 'powershell' -or $p.StartTime.ToUniversalTime().Ticks -ne ([DateTime]::Parse($depStart)).ToUniversalTime().Ticks){
        $p.Dispose();throw 'predecessor PID reused'
    }
    $handle=$p.Handle
    return $p
}
function Require-PredecessorTerminal{
    $s=Read-Predecessor
    if($s.state -notin @('complete','failed') -or $s.private_directory_present -ne $false -or (Test-Path -LiteralPath $depPrivate)){
    ...
    if($s.active_child_pid){
        $p=Get-Process -Id $s.active_child_pid -ErrorAction SilentlyContinue
        if($p){
            try{
                if(-not $s.active_child_start -or $p.StartTime.ToUniversalTime().Ticks -eq ([DateTime]::Parse($s.active_child_start)).ToUniversalTime().Ticks){
...
$state='waiting';$step='predecessor';$runExit=0;$message='waiting for exact C34 pipeline exit'
...
function Save-State{
    [ordered]@{run_id=$RunId;state=$state;step=$step;exit_code=$runExit;message=$message;
        ...
        actual_CNN=$false;actual_AXI=$false;native_fps_claim=$false;physical_RAM_measured=$false}|ConvertTo-Json -Depth 4|
...
public static class C35ShadowJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $current=[Diagnostics.Process]::GetCurrentProcess();$inJob=$false
    if(-not [C35ShadowJobCheck]::IsProcessInJob($current.Handle,[IntPtr]::Zero,[ref]$inJob)){throw 'cannot verify Windows Job'}
    ...
    $state='running';$step='component';$message='one isolated RAM/packet regression';Save-State
    $info=New-Object Diagnostics.ProcessStartInfo
    $info.FileName='D:\miniconda\miniconda\envs\p300_task3_bci3\python.exe'
    $info.Arguments='-B -u "'+(Join-Path $caseRoot 'golden\run_r2_row_shadow_probe.py')+'" --temporary-parent "'+$runRoot+'"'
    ...
    if($child.ExitCode -ne 0 -or $err.Trim() -or
       [regex]::Matches($out,'(?m)^C35_ROW_SHADOW_PASS ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C35_ROW_SHADOW_NEGATIVE_PASS ').Count -ne 12 -or
       $out -notmatch '(?m)^C35_ROW_SHADOW_CLEAN temporary_simulator_removed=1' -or
       $out -notmatch '(?m)^C35_ROW_SHADOW_SUMMARY positive_configurations=1 assertion_negatives=12 '){
        throw 'C35 component evidence incomplete or failed'
    }
    $state='complete';$step='done';$message='isolated storage and packet RTL passed; not graph/PNR/fps evidence'
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or
       (Split-Path -Leaf $resolved) -ne "c1_r2_row_shadow_$RunId"){throw 'unsafe own C35 cleanup target'}
