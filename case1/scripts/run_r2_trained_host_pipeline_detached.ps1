# C36: actual trained weights on unchanged C35 production RTL.
# Queue behind the exact C35 reaper; never terminate/restart that simulation.
[CmdletBinding()]
param([switch]$Worker,[string]$RunId='c36_b_trained_host_20260915a',
...
$runRoot=Join-Path $simRoot "c1_r2_trained_host_$RunId"
$runLog=Join-Path $caseRoot "logs\r2_trained_host_runs\$RunId"
if((Test-Path -LiteralPath $runRoot) -or (Test-Path -LiteralPath $runLog)){throw 'C36 run already exists'}
if(-not $Worker){
    ...
    if($r.ReturnValue -ne 0){throw 'WMI C36 launch failed'}
    [ordered]@{run_id=$RunId;worker_pid=$r.ProcessId;status_path=(Join-Path $runLog 'status.json');waits_for_original_C35=$true;after_run=$AfterRun}|ConvertTo-Json -Compress
    exit 0
}
...
$state='waiting';$step='original_C35';$message='waiting for exact original C35 supervision recovery';$exitCode=0
...
function Save-State {
    [ordered]@{run_id=$RunId;state=$state;step=$step;message=$message;exit_code=$exitCode;
        ...
        production_RTL_changed=$false;native_fps_claim=$false}|ConvertTo-Json -Depth 6|
...
function Invoke-Step([string]$Name,[string]$Executable,[string[]]$Arguments,[string]$Directory,[int]$Limit) {
    ...
    try {
        ...
        $j=$false
        if(-not [C36TrainingJob]::IsProcessInJob($handle,[IntPtr]::Zero,[ref]$j)){throw 'child Job check failed'}
        ...
        if($Name -eq 'host_matrix') {
            ...
            if($out -notmatch '(?m)^C36_TRAINED_HOST_MATRIX_PASS '){throw 'missing real trained-host matrix result'}
        }
        if($Name -match '_xsim$') {
...
function Simulate-Xsim([string]$Name,[int]$Width,[int]$Height,[int]$NnTarget,[int]$Stalls) {
    ...
    Invoke-Step ($Name+'_vectors') $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\r2_trained_style_vectors.py'),
    ...
    $sources=@($project.SelectNodes("//*[local-name()='design_file']")|Where-Object {$_.name -ne 'c1_ti60_r2_fused_rgb2_host96.sv' -and
        (Split-Path -Leaf $_.name) -notin @('c1_r2_row_fused_microstyle_plan.sv','c1_r2_row_fusion_plan.sv')}|
        ForEach-Object {Join-Path (Join-Path $caseRoot 'efinity') $_.name})
    $sources += (Join-Path $folder 'package\execution_plan.sv'),(Join-Path $folder 'fusion_plan.sv')
    if($sources.Count -ne 48){throw 'wrong unchanged production source closure'}
    $sources|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $runLog ($Name+'.sources.json')) -Encoding UTF8
    $top='tb_c1_r2_fused_rgb2_host_system';$snapshot='c36_trained_'+$Name
    ...
    Invoke-Step ($Name+'_check') $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\check_r2_trained_pipeline.py'),
...
public static class C36TrainingJob {
...
'@
    ...
    if(-not [C36TrainingJob]::IsProcessInJob($current.Handle,[IntPtr]::Zero,[ref]$j)){throw 'worker Job check failed'}
    ...
    $recoveryPid=26432;$recoveryStart='2026-09-15T01:01:33.6535121+08:00'
    $anchor=Get-Process -Id $recoveryPid -ErrorAction SilentlyContinue
    if($anchor) {
        if($anchor.StartTime.ToUniversalTime().Ticks -ne ([DateTime]$recoveryStart).ToUniversalTime().Ticks){throw 'predecessor PID identity differs'}
        $handle=$anchor.Handle
        while(-not $anchor.WaitForExit(5000)){
            Save-State
            if($watch.Elapsed.TotalHours -gt 8){throw 'observation deadline; predecessor left untouched'}
        }
        $anchor.Dispose();$anchor=$null
    }
    if($AfterRun) {
        $priorDirectory=Join-Path $caseRoot "logs\r2_trained_host_runs\$AfterRun"
        $priorStatus=Join-Path $priorDirectory 'status.json'
        $prior=Get-Content -LiteralPath $priorStatus -Raw|ConvertFrom-Json
        $priorPrivate=Join-Path $simRoot "c1_r2_trained_host_$AfterRun"
        ...
        if($anchor) {
            ...
            if($anchor.StartTime.ToUniversalTime().Ticks -ne ([DateTime]$prior.worker_start).ToUniversalTime().Ticks){throw 'predecessor PID identity differs'}
    ...
    Save-State
    Invoke-Step 'c35_native_preflight' $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\check_r2_trained_preflight.py')) $runRoot 180
    if($AfterRun) {
        Invoke-Step ('predecessor_'+$priorPhase+'_preflight') $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\check_r2_trained_pipeline.py'),
    ...
    Invoke-Step 'host_matrix' $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\run_r2_trained_host_probe.py'),
        '--qat-run',$QatRun,'--run-directory',$matrix) $matrix 1800
    Invoke-Step 'matrix_check' $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\check_r2_trained_pipeline.py'),
    ...
    try {
        ...
        if($resolved -ne $runRoot -or -not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or
           (Split-Path -Leaf $resolved) -ne "c1_r2_trained_host_$RunId"){throw 'unsafe private cleanup path'}
        $peers=@(Get-Process -Name xsim,xsimk,xelab,xvlog,vvp,iverilog,ivl,efx_map,efx_pnr -ErrorAction SilentlyContinue)
