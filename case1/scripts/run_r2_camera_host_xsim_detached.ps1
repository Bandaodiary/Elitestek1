<## WMI-detached C26 graph simulation. Only bounded logs survive; vectors,
...
$runLog=Join-Path $caseRoot "logs\r2_camera_host_xsim_runs\$RunId"
...
$runRoot=Join-Path $simRoot "c1_r2_camera_host_xsim_$RunId"
...
$workerInJob=$null
$runState='running';$runStep='prepare';$runExit=0;$runMessage='detached C26 graph simulation'
function Save-State {
...
function Invoke-Step([string]$Name,[string]$Executable,[string[]]$Arguments) {
    ...
    try {
        ...
        if($Name -eq 'xsim') {
            $markers=@(Get-Content -LiteralPath $stdout | Where-Object {$_ -match '^C1_R2_CAMERA_HOST_SYSTEM_'})
            $markers|Set-Content -LiteralPath (Join-Path $runLog 'result.log') -Encoding UTF8
            if(@($markers|Where-Object {$_ -match '^C1_R2_CAMERA_HOST_SYSTEM_PASS '}).Count -ne 1){throw 'missing graph pass marker'}
        }
    } finally {$p.Dispose()}
}
try {
...
public static class C26JobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C26JobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'Cannot verify Windows Job isolation'}
    ...
    [xml]$project=Get-Content -LiteralPath (Join-Path $caseRoot 'efinity\c1_ti60_r2_camera_host96.xml') -Raw
    $sources=@($project.SelectNodes("//*[local-name()='design_file']")|Where-Object {$_.name -ne 'c1_ti60_r2_camera_host96.sv' -and (Split-Path -Leaf $_.name) -ne 'execution_plan.sv'}|ForEach-Object {Join-Path (Join-Path $caseRoot 'efinity') $_.name})
    $sources += (Join-Path $runRoot 'package\execution_plan.sv')
    $top='tb_c1_r2_camera_host_system';$bankWords=if($FrameWidth*$FrameHeight -gt 8192){524288}else{16384}
    ...
    Invoke-Step 'xsim' (Join-Path $vivado 'xsim.bat') (@('r2_camera_host_sim','-runall','-testplusarg',"DIR=$vectorsPath",'-testplusarg',"P=$($meta.parameter_words)",'-testplusarg',"I=$($meta.input_words)",'-testplusarg',"E=$($meta.expected_words)")+$sourceArgs)
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_camera_host_xsim_$RunId"){throw 'unsafe cleanup target'}
    try {
