...
# Detached Vivado proxy synthesis for the optional two-lane write fabric.
# Only compact reports are copied out; the private project tree is deleted.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "tensor_mem_axi128_write_parallel_fabric_synth_runs\$RunId\status.json"
...
$runRoot = Join-Path $caseRoot "sim\.tmp_tensor_mem_axi128_write_parallel_fabric_synth_$RunId"
$runLogRoot = Join-Path $logRoot "tensor_mem_axi128_write_parallel_fabric_synth_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'tensor_mem_axi128_write_parallel_fabric_synth_status.json'
...
try {
    Write-Status 'running' 'synthesis' 0 'detached write parallel fabric proxy synthesis started'
    # Vivado may spawn a helper process for synthesis.  Touch the small RT Tcl
    # tree first so that the helper sees the same installation files even when
    # the worker was created by WMI in a fresh environment.
    ...
    $args = @('-mode','batch','-nolog','-nojournal','-notrace',
              '-source',(Join-Path $scriptRoot 'synth_tensor_mem_axi128_write_parallel_fabric.tcl'))
    ...
    if ($all -notmatch 'C1_TENSOR_MEM_AXI128_WRITE_PARALLEL_FABRIC_PROXY_SYNTH_PASS') {
        throw 'Vivado did not emit the synthesis PASS marker'
    }
    $reportDir = Join-Path $runRoot 'reports'
    $util = Join-Path $reportDir 'utilization.rpt'
    $timing = Join-Path $reportDir 'timing.rpt'
    $summary = [ordered]@{
        run_id = $RunId; part = 'xc7a200tsbg484-1'; lanes = 2;
        block_logical = 16; req_fifo_depth = 32; burst_beats = 16;
        fabric_fifo_depth = 4; tag_fifo_depth = 64
    }
    function Get-Metric([string]$text, [string]$label) {
        $m = [regex]::Match($text, '(?m)^\|\s*' +
            [regex]::Escape($label) + '\s*\|\s*([0-9,]+(?:\.[0-9]+)?)\s*\|')
    ...
    if (Test-Path -LiteralPath $timing) {
        ...
        $timing_lines = $t -split "`r?`n"
        for ($ti = 0; $ti -lt $timing_lines.Count; $ti++) {
            if ($timing_lines[$ti] -match 'WNS\(ns\)' -and
                $timing_lines[$ti] -match 'TNS\(ns\)') {
                for ($tj = $ti + 1; $tj -lt [math]::Min($ti + 10, $timing_lines.Count); $tj++) {
                    $tm = [regex]::Match($timing_lines[$tj],
                        '^\s*([-+]?[0-9]+(?:\.[0-9]+)?)\s+([-+]?[0-9]+(?:\.[0-9]+)?)\s+')
                    if ($tm.Success) {
                        $summary.wns_ns = [double]$tm.Groups[1].Value
                        $summary.tns_ns = [double]$tm.Groups[2].Value
                        break
                    }
                }
    ...
    # Keep only a bounded timing excerpt for diagnosis; never retain the full
    # Vivado project/report tree in the workspace.
    ...
    $watch.Stop()
    Write-Status 'complete' 'done' 0 'C1_TENSOR_MEM_AXI128_WRITE_PARALLEL_FABRIC_PROXY_SYNTH_PASS'
} catch {
    $watch.Stop()
