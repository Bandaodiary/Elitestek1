<## WMI-detached C30 camera/Resize/Capture simulation, without CNN. Only bounded logs survive; vectors,
...
##>
[CmdletBinding()]
param([switch]$Worker,[switch]$Native,[switch]$Vendor,[switch]$ExpectOverflow,[string]$RunId='', [int]$FifoDepth=0,
...
if($Native -and $Vendor){throw 'vendor integration fixture is the bounded interior test'}
if($FifoDepth -eq 0){$FifoDepth=if($Native){512}else{64}}
if($FifoDepth -lt 2 -or $FifoDepth -gt 8192 -or ($FifoDepth -band ($FifoDepth-1)) -ne 0){throw 'invalid FIFO depth'}
$runLog=Join-Path $caseRoot "logs\r2_demo_rgb2_capture_xsim_runs\$RunId"
...
$runRoot=Join-Path $simRoot "c1_r2_demo_rgb2_capture_xsim_$RunId"
...
$workerInJob=$null
$runState='running';$runStep='prepare';$runExit=0;$runMessage='detached C30 camera/Resize/Capture simulation'
function Save-State {
    [ordered]@{run_id=$RunId;state=$runState;step=$runStep;exit_code=$runExit;message=$runMessage;worker_pid=$PID;
        ...
        overlap_resize=$true;actual_vendor_rtl=[bool]$Vendor;native_shape=[bool]$Native;expected_overflow=[bool]$ExpectOverflow;fifo_depth=$FifoDepth;stalls=$Stalls;source_backpressure_allowed=$false;
...
function Invoke-Step([string]$Name,[string]$Executable,[string[]]$Arguments) {
    ...
    try {
        ...
        if($Name -eq 'xsim') {
            $markers=@(Get-Content -LiteralPath $stdout | Where-Object {$_ -cmatch '^C1_R2_DEMO_RGB2_|^C30_'})
            $markers|Set-Content -LiteralPath (Join-Path $runLog 'result.log') -Encoding UTF8
            if(@($markers|Where-Object {$_ -match '^C1_R2_DEMO_RGB2_CAPTURE_PASS '}).Count -ne 1){throw 'missing capture pass marker'}
            if(@($markers|Where-Object {$_ -ceq 'C30_RESIZE_SELECTION overlap=1'}).Count -ne 1){throw 'actual overlap implementation missing'}
            $producer='C30_RASTER_PRODUCER vendor_rtl='+[int][bool]$Vendor+' final_pair_waits_for_vs=1 no_ready=1'
            if(@($markers|Where-Object {$_ -ceq $producer}).Count -ne 1){throw 'actual raster producer mismatch'}
        }
    } finally {$p.Dispose()}
}
try {
...
public static class C30JobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C30JobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'Cannot verify Windows Job isolation'}
    $workerInJob=$jobValue
    if($workerInJob){throw 'Worker is in a Windows Job; refusing to launch Vivado'}
    Save-State
    $vectorArgs=@('-B','-u',(Join-Path $caseRoot 'golden\run_r2_demo_rgb2_capture_probe.py'),'--vectors-only',$runRoot,'--fifo',"$FifoDepth")
    $vectorArgs += '--overlap-resize'
    if($Vendor){$vectorArgs += '--vendor'}
    ...
    [xml]$project=Get-Content -LiteralPath (Join-Path $caseRoot 'efinity\c1_ti60_r2_rgb2_capture512.xml') -Raw
    $sources=@($project.SelectNodes("//*[local-name()='design_file']")|Where-Object {$_.name -ne 'c1_ti60_r2_rgb2_capture512.sv'}|ForEach-Object {Join-Path (Join-Path $caseRoot 'efinity') $_.name})
    if($Vendor) {
        $vendorRoot='D:\contest\Ti60F225_DemoBoard_v4\10_Ti60f225_sc431hai2hdmi_demo\Ti60f225_sc431hai2hdmi_v1'
        [xml]$vendorProject=Get-Content -LiteralPath (Join-Path $vendorRoot 'ti60f225_oob.xml') -Raw
        $vendorFiles=@('rtl/true_dual_port_ram.v','rtl/debayer/rgb_gain.v','rtl/debayer/line_buffer.v','rtl/debayer/raw_to_rgb.v','rtl/debayer/debayer_top_2to1.v')
        foreach($file in $vendorFiles) {
            if(@($vendorProject.SelectNodes("//*[local-name()='design_file']")|Where-Object {$_.name -ceq $file}).Count -ne 1){throw 'vendor active closure mismatch'}
            $sources+=Join-Path $vendorRoot $file
        }
    }
    $top='tb_c1_r2_demo_rgb2_capture'
    Invoke-Step 'xvlog' (Join-Path $vivado 'xvlog.bat') (@('-sv','-d','C30_RESIZE_OVERLAP')+$sources+(Join-Path $caseRoot 'sim\c1_r2_axi_memory_bfm.sv')+(Join-Path $caseRoot "sim\$top.sv"))
    $genericArgs=@()
    ...
    Invoke-Step 'xelab' (Join-Path $vivado 'xelab.bat') (@($top,'-s','r2_demo_rgb2_capture_sim','-mt','2')+$genericArgs)
    ...
    Invoke-Step 'xsim' (Join-Path $vivado 'xsim.bat') (@('r2_demo_rgb2_capture_sim','-runall','-testplusarg',"DIR=$vectorsPath")+$plusArgs)
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_demo_rgb2_capture_xsim_$RunId"){throw 'unsafe cleanup target'}
    try {
