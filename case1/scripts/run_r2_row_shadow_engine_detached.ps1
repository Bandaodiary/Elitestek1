...
[CmdletBinding()]
param(
    [switch]$Worker,[switch]$CheckOnly,[string]$RunId='c35_shadow_engine_20260914_a',
    [ValidateRange(1,1800)][int]$TimeoutSeconds=1200
)
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
if($RunId -notmatch '^[A-Za-z0-9_-]+$'){throw 'invalid RunId'}
$depRun='c35_pw_shadow_20260914_b'
$depPid=43900;$depStart='2026-09-14T22:57:50.1329656+08:00'
$depStatus=Join-Path $caseRoot "logs\r2_pw_shadow_runs\$depRun\status.json"
$simRoot=(Resolve-Path (Join-Path $caseRoot 'sim')).Path
$depPrivate=Join-Path $simRoot "c1_r2_pw_shadow_$depRun"
$runRoot=Join-Path $simRoot "c1_r2_shadow_engine_$RunId"
$runLog=Join-Path $caseRoot "logs\r2_shadow_engine_runs\$RunId"
...
$state='waiting';$step='predecessor';$runExit=0;$message='waiting for exact C35 PW feeder worker exit'
...
public static class C35EngineJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $current=[Diagnostics.Process]::GetCurrentProcess();$inJob=$false
    if(-not [C35EngineJobCheck]::IsProcessInJob($current.Handle,[IntPtr]::Zero,[ref]$inJob)){throw 'cannot verify Windows Job'}
    ...
    $state='running';$step='component';$message='real DW/PW shared-MAC arithmetic and retained operator fallback';Save-State
    $info=New-Object Diagnostics.ProcessStartInfo
    $info.FileName='D:\miniconda\miniconda\envs\p300_task3_bci3\python.exe'
    $info.Arguments='-B -u "'+(Join-Path $caseRoot 'golden\run_r2_row_shadow_engine_probe.py')+'" --pw-run '+$depRun+' --temporary-parent "'+$runRoot+'"'
    ...
    if($child.ExitCode -ne 0 -or $err.Trim() -or
       [regex]::Matches($out,'(?m)^C35_SHADOW_ENGINE_CASE ').Count -ne 10 -or
       [regex]::Matches($out,'(?m)^C35_SHADOW_ENGINE_NEGATIVE_PASS ').Count -ne 3 -or
       [regex]::Matches($out,'(?m)^C35_SHADOW_ENGINE_FALLBACK_PASS ').Count -ne 2 -or
       [regex]::Matches($out,'(?m)^C35_PW_SHADOW_EVIDENCE_PASS ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C35_SHADOW_ENGINE_SUMMARY ').Count -ne 1 -or
       $out -notmatch '(?m)^C35_SHADOW_ENGINE_CLEAN temporary_vectors_and_simulator_removed=1'){
        throw 'C35 component evidence incomplete or failed'
    }
    $state='complete';$step='done';$message='real shared-MAC DW/PW rows and fallback passed; not full graph/PNR/fps evidence'
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or
       (Split-Path -Leaf $resolved) -ne "c1_r2_shadow_engine_$RunId"){throw 'unsafe own C35 cleanup target'}
