...
# Detached negative-path xsim for beat-record response FIFO.  The worker is
# launched through WMI and removes its private xsim directory on completion.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "xsim_runs\tensor_read_beatfifo_malformed\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_run_tensor_read_beatfifo_malformed_$RunId"
$runLogRoot = Join-Path $logRoot "xsim_runs\tensor_read_beatfifo_malformed\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_tensor_read_beatfifo_malformed_status.json'
...
try {
    Write-Status 'running' 'setup' 0 'detached beat-FIFO malformed worker started'
    Invoke-Step 'xvlog' (Join-Path $vivadoBin 'xvlog.bat') @(
        '-sv','-nolog',
        (Join-Path $rtlRoot 'dma\c1_tensor_mem_axi128_read_burst_client.sv'),
        (Join-Path $simRoot 'tb_c1_tensor_mem_axi128_read_burst_client_beatfifo_malformed.sv'))
    Invoke-Step 'xelab' (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_tensor_mem_axi128_read_burst_client_beatfifo_malformed','-s',
        'tb_c1_tensor_mem_axi128_read_burst_client_beatfifo_malformed_sim','-nolog')
    Invoke-Step 'xsim' (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_tensor_mem_axi128_read_burst_client_beatfifo_malformed_sim','-runall','-nolog') `
        'C1_TENSOR_MEM_AXI128_READ_BURST_CLIENT_BEAT_FIFO_MALFORMED_PASS'
    $watch.Stop(); Write-Status 'complete' 'done' 0 'C1_TENSOR_MEM_AXI128_READ_BURST_CLIENT_BEAT_FIFO_MALFORMED_PASS'
} catch {
