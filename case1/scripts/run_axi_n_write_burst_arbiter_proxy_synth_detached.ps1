...
# WMI-detached Vivado proxy synthesis.  The private report/project directory
# is deleted after extracting a compact summary, so this does not leave a
# multi-hundred-MB simulation or project tree behind.
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$logRoot=Join-Path $caseRoot 'logs\axi_n_write_burst_arbiter_proxy_synth_runs'
$scriptFile=Join-Path $caseRoot 'scripts\synth_axi_n_write_burst_arbiter_128.tcl'
$vivadoBin='D:\vivado\vivado\Vivado\2023.1\bin'
$vivadoRoot='D:\vivado\vivado\Vivado\2023.1'
$runFamily='axi_n_write_burst_arbiter_proxy_synth'
if(-not $Worker){
...
function Metric([string]$txt,[string]$label){
    ...
    if($m.Success){return [double]::Parse($m.Groups[1].Value,[Globalization.CultureInfo]::InvariantCulture)} return $null
}
...
try{
    Write-Status 'running' 'vivado' 0 'detached write-arbiter proxy synthesis started'
    # Pre-read only the small runtime Tcl files to avoid Vivado's lazy lookup
    # failure on detached workers.
    $buf=New-Object byte[] 65536;$rtRoot=Join-Path $vivadoRoot 'scripts\rt'
    Get-ChildItem -LiteralPath $rtRoot -Filter '*.tcl' -File -Recurse|ForEach-Object{$s=[IO.File]::OpenRead($_.FullName);while($s.Read($buf,0,$buf.Length)-gt 0){};$s.Dispose()}
    ...
    $marker='C1_AXI_N_WRITE_BURST_ARBITER_PROXY_SYNTH_PASS'
    ...
    $metrics=[ordered]@{part='xc7a200tsbg484-1';clients=3;fifo_depth=4;clock_mhz=100.0;luts=Metric $util 'Slice LUTs*';registers=Metric $util 'Slice Registers';bram_tiles=Metric $util 'Block RAM Tile';dsps=Metric $util 'DSPs';wns_ns=$tim.wns_ns;tns_ns=$tim.tns_ns;timing_met=($null -ne $tim.wns_ns -and $tim.wns_ns -ge 0.0);marker=$marker}
