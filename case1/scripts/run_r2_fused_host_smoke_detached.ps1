...
[CmdletBinding()]
param(
    [switch]$Worker,[switch]$CheckOnly,[string]$RunId='c35_fused_host_smoke_20260914_a',
...
$runRoot=Join-Path $simRoot "c1_r2_fused_host_$RunId"
$runLog=Join-Path $caseRoot "logs\r2_fused_rgb2_host_runs\$RunId"
...
public static class C35HostJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $current=[Diagnostics.Process]::GetCurrentProcess();$inJob=$false
    if(-not [C35HostJobCheck]::IsProcessInJob($current.Handle,[IntPtr]::Zero,[ref]$inJob)){throw 'cannot verify Windows Job'}
    ...
    $state='running';$step='component';$message='real fused host AXI camera CPU/display smoke';Save-State
    $info=New-Object Diagnostics.ProcessStartInfo
    $info.FileName='D:\miniconda\miniconda\envs\p300_task3_bci3\python.exe'
    $info.Arguments='-B -u "'+(Join-Path $caseRoot 'golden\run_r2_fused_host_smoke.py')+'" --fault-run '+$depRun+' --temporary-parent "'+$runRoot+'"'
    ...
    if($child.ExitCode -ne 0 -or $err.Trim() -or
       ...
       [regex]::Matches($out,'(?m)^C1_R2_FUSED_RGB2_HOST_SYSTEM_PASS ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C35_FUSED_HOST_SMOKE_PASS ').Count -ne 1 -or
       $out -notmatch '(?m)^C1_R2_FUSED_RGB2_HOST_SYSTEM_CLEAN temporary_vectors_and_simulator_removed=1'){
        throw 'C35 real host smoke evidence incomplete or failed'
    }
    $state='complete';$step='done';$message='fused AXI camera CPU/display smoke passed; not native fps/PNR/vendor CPU IP'
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or
       (Split-Path -Leaf $resolved) -ne "c1_r2_fused_host_$RunId"){throw 'unsafe own C35 cleanup target'}
