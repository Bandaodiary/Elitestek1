...
# WMI launches a hidden worker outside the calling Codex Windows Job.  Every
# tool step must exit zero, leave stderr empty, and contain no Fatal/Error/FAIL
# diagnostic.  XSim must emit exactly one subsystem-specific PASS marker.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "r1_config_loader_subsystem_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_r1_config_loader_subsystem_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_config_loader_subsystem_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'r1_config_loader_subsystem_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached R1 config-loader-subsystem worker started'
    Invoke-StrictStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_r1_config_loader_subsystem.sv')
        )
    Invoke-StrictStep -Name 'xelab_r1_config_loader_subsystem' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @('tb_c1_r1_config_loader_subsystem', '-s',
                     'tb_c1_r1_config_loader_subsystem_sim')
    Invoke-StrictStep -Name 'xsim_r1_config_loader_subsystem' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_r1_config_loader_subsystem_sim', '-runall') `
        -ExpectedPass 'C1_R1_CONFIG_LOADER_SUBSYSTEM_PASS'

    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'R1 config-loader-subsystem xsim regression complete'
} catch {
    $script:totalWatch.Stop()
