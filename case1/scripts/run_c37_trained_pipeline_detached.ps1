# C37: independent six-source resource candidate, actual three-model host checks.
# Serial, WMI-detached, two CPUs, no waves; existing C35/C36 workers unchanged.
[CmdletBinding()]
param([switch]$Worker,[string]$RunId='c37_trained_host_20260915a',
...
$runRoot=Join-Path $simRoot "c1_c37_trained_host_$RunId"
$runLog=Join-Path $caseRoot "logs\c37_trained_host_runs\$RunId"
if((Test-Path -LiteralPath $runRoot) -or (Test-Path -LiteralPath $runLog)){throw 'C37 run already exists'}
if(-not $Worker){
    ...
    if($r.ReturnValue -ne 0){throw 'WMI C37 launch failed'}
...
$state='waiting';$step='admission';$message='checking C37 predecessor and single-worker admission';$exitCode=0
...
function Save-State {
    [ordered]@{run_id=$RunId;state=$state;step=$step;message=$message;exit_code=$exitCode;
        ...
        production_RTL_changed=$true;candidate='C37';native_fps_claim=$false}|ConvertTo-Json -Depth 6|
...
function Invoke-Step([string]$Name,[string]$Executable,[string[]]$Arguments,[string]$Directory,[int]$Limit) {
    ...
    try {
        ...
        $j=$false
        if(-not [C37TrainingJob]::IsProcessInJob($handle,[IntPtr]::Zero,[ref]$j)){throw 'child Job check failed'}
        ...
        if($Name -eq 'host_matrix') {
            ...
            if($out -notmatch '(?m)^C37_TRAINED_HOST_MATRIX_PASS '){throw 'missing real trained-host matrix result'}
        }
        if($Name -match '_xsim$') {
...
function Simulate-Xsim([string]$Name,[int]$Width,[int]$Height,[int]$NnTarget,[int]$Stalls) {
    ...
    Invoke-Step ($Name+'_vectors') $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\c37_trained_contract.py'),
    ...
    [xml]$project=Get-Content -LiteralPath (Join-Path $caseRoot 'efinity\c1_ti60_c37_resource24.xml') -Raw
    $sources=@($project.SelectNodes("//*[local-name()='design_file']")|Where-Object {$_.name -ne 'c1_ti60_c37_resource24.sv' -and
    ...
    if($sources.Count -ne 48){throw 'wrong C37 production source closure'}
    $sources|ConvertTo-Json|Set-Content -LiteralPath (Join-Path $runLog ($Name+'.sources.json')) -Encoding UTF8
    $top='tb_c1_r2_fused_rgb2_host_system';$snapshot='c37_trained_'+$Name
    ...
    Invoke-Step ($Name+'_check') $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\check_c37_trained_pipeline.py'),
...
public static class C37TrainingJob {
...
'@
    ...
    if(-not [C37TrainingJob]::IsProcessInJob($current.Handle,[IntPtr]::Zero,[ref]$j)){throw 'worker Job check failed'}
    ...
    if($AfterRun) {
        $priorDirectory=Join-Path $caseRoot "logs\c37_trained_host_runs\$AfterRun"
        $priorStatus=Join-Path $priorDirectory 'status.json'
        $prior=Get-Content -LiteralPath $priorStatus -Raw|ConvertFrom-Json
        $priorPrivate=Join-Path $simRoot "c1_c37_trained_host_$AfterRun"
    ...
    Save-State
    Invoke-Step 'c37_source_preflight' $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\c37_trained_contract.py')) $runRoot 180
    ...
    if($AfterRun) {
        Invoke-Step ('predecessor_'+$priorPhase+'_preflight') $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\check_c37_trained_pipeline.py'),
    ...
    Invoke-Step 'host_matrix' $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\run_c37_trained_host_probe.py'),
        '--qat-run',$QatRun,'--run-directory',$matrix) $matrix 1800
    Invoke-Step 'matrix_check' $python @('-X','utf8','-B','-u',(Join-Path $caseRoot 'golden\check_c37_trained_pipeline.py'),
    ...
    try {
        ...
        if($resolved -ne $runRoot -or -not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or
           (Split-Path -Leaf $resolved) -ne "c1_c37_trained_host_$RunId"){throw 'unsafe private cleanup path'}
