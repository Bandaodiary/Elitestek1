# C32 real AXI two-writer diagnostic; does not change the retained host.
[CmdletBinding()]
param(
    [switch]$Worker,[string]$RunId='',
    [string]$WriterUnitRun='c32_writer_matrix_20260914_a',
...
$runLog=Join-Path $caseRoot "logs\r2_cutthrough_write_axi_runs\$RunId"
...
$runRoot=Join-Path $simRoot "c1_r2_cutthrough_write_axi_$RunId"
...
$state='running';$step='prepare';$runExit=0;$message='C32 actual AXI competing-writer diagnostic'
...
function Invoke-PythonProbe([string]$Arguments,[string]$Stem,[int]$Seconds){
    ...
    try{
        if(-not $process.Start()){throw 'could not start C32 AXI probe'}
        ...
        if($timedOut){throw 'C32 AXI probe timeout; partial checkpoints retained'}
        if($null -eq $process.ExitCode -or $process.ExitCode -ne 0){throw "C32 $Stem failed exit=$($process.ExitCode)"}
        return $out
...
public static class C32AxiJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C32AxiJobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'cannot check Job isolation'}
    ...
    $out=Invoke-PythonProbe ('-B -u "'+(Join-Path $caseRoot 'golden\run_r2_cutthrough_write_contention.py')+'" --temporary-parent "'+$runRoot+'"') 'result' $TimeoutSeconds
    if([regex]::Matches($out,'(?m)^C32_WRITE_AXI_PASS ').Count -ne 24 -or
       [regex]::Matches($out,'(?m)^C32_WRITE_AXI_COMPARE ').Count -ne 12 -or
       [regex]::Matches($out,'(?m)^C32_WRITE_AXI_CLEAN temporary_vectors_and_simulator_removed=1').Count -ne 1 -or
       $out -cmatch 'FATAL|ERROR:|Traceback|RuntimeError'){throw 'C32 AXI evidence incomplete'}
    $state='complete';$step='done';$message='C32 AXI diagnostic completed; signed contention deltas are not a speedup gate'
}catch{$state='failed';$step='error';$runExit=1;$message=$_.Exception.Message}
finally{
    $resolved=[IO.Path]::GetFullPath($runRoot)
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_cutthrough_write_axi_$RunId"){throw 'unsafe private cleanup target'}
