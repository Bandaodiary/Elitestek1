...
# Detached xsim regression for C8 line-cache capacity and fault handling.
# Win32_Process creates the worker outside the caller's Windows Job so xsimk
# remains alive if the invoking Codex command process exits.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "xsim_runs\window_line_cache_c8_faults\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_run_window_line_cache_c8_faults_$RunId"
$runLog = Join-Path $logRoot "xsim_runs\window_line_cache_c8_faults\$RunId"
$statusPath = Join-Path $runLog 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_runs\window_line_cache_c8_faults_status.json'
...
try {
    Write-Status 'running' 'setup' 0 'detached C8 fault worker started'
    Invoke-Step 'xvlog' (Join-Path $vivadoBin 'xvlog.bat') @(
        '-sv',
        (Join-Path $rtlRoot 'cnn\c1_window_line_cache_c8.sv'),
        (Join-Path $simRoot 'tb_c1_window_line_cache_c8_faults.sv')
    )
    Invoke-Step 'xelab' (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_window_line_cache_c8_faults',
        '-s',
        'tb_c1_window_line_cache_c8_faults_sim'
    )
    Invoke-Step 'xsim' (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_window_line_cache_c8_faults_sim',
        '-runall'
    ) 'C1_WINDOW_LINE_CACHE_C8_FAULT_PASS'
    $script:watch.Stop()
    Write-Status 'complete' 'done' 0 'C1_WINDOW_LINE_CACHE_C8_FAULT_PASS'
} catch {
    $script:watch.Stop()
