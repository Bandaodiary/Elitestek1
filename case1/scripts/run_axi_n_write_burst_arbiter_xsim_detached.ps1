...
# Detached boardless regression for the optional multi-descriptor write
# arbiter.  WMI places the worker outside the caller's Windows Job.  The
# private Vivado run tree is removed in finally; only compact logs/status are
# retained.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "xsim_runs\axi_n_write_burst_arbiter\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_run_axi_n_write_burst_arbiter_$RunId"
$runLogRoot = Join-Path $logRoot "xsim_runs\axi_n_write_burst_arbiter\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_axi_n_write_burst_arbiter_status.json'
...
try {
    Write-Status 'running' 'setup' 0 'detached ID-less multi-descriptor write arbiter started'
    ...
    Invoke-Step 'xelab' $xelab @(
        ...
        'tb_c1_axi_n_write_burst_arbiter_128_sim', '-nolog')
    Invoke-Step 'xsim' $xsim @(
        'tb_c1_axi_n_write_burst_arbiter_128_sim', '-runall', '-nolog') `
