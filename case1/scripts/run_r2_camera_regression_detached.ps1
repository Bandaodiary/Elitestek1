# C26 Icarus regressions survive a desktop turn interruption. No waves.
[CmdletBinding()]
param([switch]$Worker,[string]$RunId='',
      [ValidateSet('matrix','variant','faults')][string]$TestKind='matrix',
...
$runLog=Join-Path $caseRoot "logs\r2_camera_regression_runs\$RunId"
...
$runRoot=Join-Path $simRoot "c1_r2_camera_regression_$RunId"
...
$state='running';$step='prepare';$runExit=0;$message='C26 detached regression';$workerInJob=$null
...
public static class C26RegressionJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C26RegressionJobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'Cannot verify Windows Job isolation'}
    ...
    $scriptName=if($TestKind -eq 'faults'){'run_r2_camera_host_fault_probe.py'}else{'run_r2_camera_host_probe.py'}
    ...
    $prefix=if($TestKind -eq 'faults'){'C1_R2_CAMERA_HOST_FAULT_'}else{'C1_R2_CAMERA_HOST_SYSTEM_'}
    $expected=if($TestKind -eq 'faults'){8}else{4}
    ...
    $lines=@(Get-Content -LiteralPath (Join-Path $runLog 'result.log')|Where-Object {$_ -cnotmatch '^C1_R2_CAMERA_HOST_SYSTEM_VECTORS '})
    $passCount=@($lines|Where-Object {$_ -match ('^'+$prefix+'PASS ')}).Count
    ...
    $state='complete';$step='done';$message='C26 regression passed'
} catch {$state='failed';$step='error';$runExit=1;$message=$_.Exception.Message}
finally {
    $resolved=[IO.Path]::GetFullPath($runRoot)
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_camera_regression_$RunId"){throw 'unsafe cleanup target'}
    try {
