...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $caseRoot `
        "logs\r1_parameter_bank_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "r1_parameter_bank_xsim_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_parameter_bank_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_r1_parameter_bank_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached R1 parameter bank verification worker started'
    ...
    Invoke-LoggedStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_r1_parameter_bank.sv')
        )
    Invoke-LoggedStep -Name 'xelab' -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @(
            'tb_c1_r1_parameter_bank',
            '-s', 'tb_c1_r1_parameter_bank_sim'
        )
    Invoke-LoggedStep -Name 'xsim' -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_r1_parameter_bank_sim', '-runall')

    $expectedPass = 'C1_R1_PARAMETER_BANK_PASS words=1056 ' +
                    'switches=2 errors=2 aborts=1'
