...
# The foreground process only creates a detached Win32_Process worker.  The
# Vivado tools launched by that worker are therefore not members of the Codex
# Windows Job.  Passing requires zero exits, empty stderr, no fatal/error/fail
# diagnostics, and exactly one loader-specific PASS marker.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot `
        "axi_parameter_loader_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_axi_parameter_loader_run_$RunId"
$runLogRoot = Join-Path $logRoot "axi_parameter_loader_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'axi_parameter_loader_status.json'
...
function Invoke-VivadoStep {
    ...
    if ($ExpectedPass) {
        ...
        if ($passCount -ne 1) {
            throw "$Name emitted $passCount copies of required marker " +
                  "$ExpectedPass; expected exactly one"
        }
    }
    Write-Status -State 'running' -Step $Name -ExitCode 0 `
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached AXI parameter-loader verification worker started'
    Invoke-VivadoStep -Name 'xvlog' `
        ...
        -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_axi_parameter_loader.sv')
        )
    Invoke-VivadoStep -Name 'xelab_axi_parameter_loader' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @(
            'tb_c1_axi_parameter_loader',
            '-s',
            'tb_c1_axi_parameter_loader_sim'
        )
    Invoke-VivadoStep -Name 'xsim_axi_parameter_loader' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_axi_parameter_loader_sim', '-runall') `
        -ExpectedPass 'C1_AXI_PARAMETER_LOADER_PASS words=1056 successes=2 errors=5 aborts=2 early_rlast=1 missing_rlast=1'
    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'AXI parameter-loader xsim regression complete'
} catch {
    $script:totalWatch.Stop()
