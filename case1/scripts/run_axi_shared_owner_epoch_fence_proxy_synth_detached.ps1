...
# Detached Vivado proxy for the control-only owner/epoch fence.  The worker
# keeps only compact status/summary/log files; the private synthesis tree is
# removed in finally so no large project is retained.
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$logRoot=Join-Path $caseRoot 'logs\axi_shared_owner_epoch_fence_proxy_synth_runs'
$tcl=Join-Path $caseRoot 'scripts\synth_axi_shared_owner_epoch_fence_proxy.tcl'
...
$runRoot=Join-Path $caseRoot "sim\xsim_run_axi_shared_owner_epoch_fence_proxy_$RunId"
...
try{
    Write-Status 'running' 'vivado' 0 'detached owner/epoch fence proxy synthesis started'
    ...
    $marker='C1_AXI_SHARED_OWNER_EPOCH_FENCE_PROXY_SYNTH_PASS'
    ...
    $metrics=[ordered]@{part='xc7a200tsbg484-1';clock_mhz=100.0;epoch_width=4;luts=Metric $util 'Slice LUTs*';registers=Metric $util 'Slice Registers';bram_tiles=Metric $util 'Block RAM Tile';dsps=Metric $util 'DSPs';wns_ns=$tim.wns_ns;tns_ns=$tim.tns_ns;timing_met=($null -ne $tim.wns_ns -and $tim.wns_ns -ge 0.0);marker=$marker}
