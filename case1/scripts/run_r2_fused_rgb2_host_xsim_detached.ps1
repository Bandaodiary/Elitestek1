<## WMI-detached C35 graph simulation. Only bounded logs survive; vectors,
...
param([switch]$Worker,[string]$RunId='', [int]$FrameWidth=12,[int]$FrameHeight=12,
      [ValidateSet('microstyle24','drop_res1')][string]$Profile='microstyle24',[int]$MemoryDiv=2,[int]$CommandLatency=20,[int]$Stalls=0,[int]$AwWaitW=0,[int]$NnTarget=6,[ValidateRange(1,3)][int]$FrameDivisor=2,[int]$TimeoutSeconds=600,
      [ValidateRange(-1,1)][int]$ClocksNative=-1,[ValidateRange(0,1)][int]$CpuStartNegative=0)
...
$runRoot=Join-Path $simRoot "c1_r2_fused_rgb2_host_xsim_$RunId"
...
$runState='running';$runStep='prepare';$runExit=0;$runMessage='detached C35 graph simulation'
$safeCleanup=$true;$stepPid=$null;$stepStart=$null;$freeMemoryKiB=$null
function Save-State {
    [ordered]@{run_id=$RunId;state=$runState;step=$runStep;exit_code=$runExit;message=$runMessage;worker_pid=$PID;worker_start=$workerStart;step_pid=$stepPid;step_start=$stepStart;free_memory_kib_before_launch=$freeMemoryKiB;workload_budget=$workerBudget;clocks_native=$ClocksNative;cpu_start_negative=$CpuStartNegative;
        ...
        frame_divisor=$FrameDivisor;source_pixels_per_word=2;actual_vs_de=$true;camera_backpressure_allowed=$false;actual_roi=$true;native_timing=($FrameWidth -eq 640 -and $FrameHeight -eq 480);performance_claim=$false;stalls=$Stalls;aw_wait_w=$AwWaitW;nn_target=$NnTarget;profile=$Profile;simulator_directory_present=(Test-Path -LiteralPath $runRoot);worker_in_windows_job=$workerInJob}|ConvertTo-Json|Set-Content -LiteralPath $statusPath -Encoding UTF8
}
function Invoke-Step([string]$Name,[string]$Executable,[string[]]$Arguments) {
    ...
    try {
        ...
        $stepHandle=$p.Handle
        $script:stepPid=$p.Id;$script:stepStart=$p.StartTime.ToString('o');Save-State
        ...
        # Retain ALL compact checkpoints even on failure, before private cleanup.
        # Never retain vectors, waves, netlists or the full simulator transcript.
        if($Name -eq 'xsim') {
            $invocation=@(Get-Content -LiteralPath $stdout -TotalCount 24 | Where-Object {$_ -match '^Running: .*xsim.exe '})
            $invocation|Set-Content -LiteralPath (Join-Path $runLog 'xsim.invocation.log') -Encoding UTF8
            $markers=@(Get-Content -LiteralPath $stdout | Where-Object {$_ -match '^C1_R2_FUSED_RGB2_HOST_SYSTEM_'})
        ...
        if($Name -eq 'xsim') {
            if(@($markers|Where-Object {$_ -match '^C1_R2_FUSED_RGB2_HOST_SYSTEM_PASS '}).Count -ne 1){throw 'missing graph pass marker'}
        }
    } finally {
...
public static class C35JobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C35JobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'Cannot verify Windows Job isolation'}
    ...
    if($freeMemoryKiB -lt 8388608){throw 'less than 8 GiB free; no xsim launch'}
    . (Join-Path $PSScriptRoot 'set_fpga_worker_budget.ps1')
    Save-State
    Invoke-Step 'preflight' $python @('-B','-u',(Join-Path $caseRoot 'golden\check_r2_fused_host_regression.py'),'--run','c35_fused_host_regression_20260915_a')
    if($FrameWidth -eq 640 -and $FrameHeight -eq 480){
        Invoke-Step 'xsim_smoke_preflight' $python @('-B','-u',(Join-Path $caseRoot 'golden\check_r2_fused_xsim_evidence.py'),'--run','c35_fused_xsim_smoke_20260915_a')
    }
    Invoke-Step 'vectors' $python @('-B','-u',(Join-Path $caseRoot 'golden\generate_r2_fused_camera_host_xsim_vectors.py'),'--output',$runRoot,'--width',"$FrameWidth",'--height',"$FrameHeight",'--profile',$Profile)
    $meta=Get-Content -LiteralPath (Join-Path $runRoot 'metadata.json') -Raw|ConvertFrom-Json
    $meta|Add-Member -NotePropertyName frame_divisor -NotePropertyValue $FrameDivisor
    $meta|Add-Member -NotePropertyName source_pixels_per_word -NotePropertyValue 2
    $meta|Add-Member -NotePropertyName actual_vs_de -NotePropertyValue $true
    ...
    Copy-Item -LiteralPath (Join-Path $runRoot 'fusion_plan.sv') -Destination (Join-Path $runLog 'fusion_plan.sv')
    [xml]$project=Get-Content -LiteralPath (Join-Path $caseRoot 'efinity\c1_ti60_r2_fused_rgb2_host96.xml') -Raw
    $sources=@($project.SelectNodes("//*[local-name()='design_file']")|Where-Object {$_.name -ne 'c1_ti60_r2_fused_rgb2_host96.sv' -and (Split-Path -Leaf $_.name) -notin @('c1_r2_row_fused_microstyle_plan.sv','c1_r2_row_fusion_plan.sv')}|ForEach-Object {Join-Path (Join-Path $caseRoot 'efinity') $_.name})
    $sources += (Join-Path $runRoot 'package\execution_plan.sv'),(Join-Path $runRoot 'fusion_plan.sv')
    if($sources.Count -ne 48){throw 'incorrect fused source closure'}
    $top='tb_c1_r2_fused_rgb2_host_system';$bankWords=if($FrameWidth*$FrameHeight -gt 8192){524288}else{16384}
    ...
    $cameraGenerics=@('-generic_top',"FRAME_DIVISOR=$FrameDivisor")
    if($ClocksNative -ne -1){$cameraGenerics+=@('-generic_top',"CLOCKS_NATIVE=$ClocksNative")}
    $cameraGenerics+=@('-generic_top',"CPU_START_NEGATIVE=$CpuStartNegative")
    ...
    Invoke-Step 'xelab' (Join-Path $vivado 'xelab.bat') (@($top,'-s','r2_fused_rgb2_host_sim','-generic_top',"BANK_WORDS=$bankWords",'-generic_top',"WIDTH=$FrameWidth",'-generic_top',"HEIGHT=$FrameHeight",'-generic_top',"MAX_CYCLES=$cycleLimit",'-generic_top',"NN_TARGET=$NnTarget",'-generic_top',"STAGE_COUNT=$($meta.stage_count)",'-generic_top',"RGB_STAGE=$($meta.rgb_stage)",'-generic_top',"FUSED_DW_STAGE=$($meta.dw_stage)",'-generic_top',"FUSED_PW_STAGE=$($meta.pw_stage)",'-generic_top',"MEMORY_DIV=$MemoryDiv",'-generic_top',"COMMAND_LATENCY=$CommandLatency",'-generic_top',"STALLS=$Stalls",'-generic_top',"AW_WAIT_W=$AwWaitW",'-mt','2')+$cameraGenerics)
    ...
    Invoke-Step 'xsim' (Join-Path $vivado 'xsim.bat') (@('r2_fused_rgb2_host_sim','-runall','-testplusarg',"DIR=$vectorsPath",'-testplusarg',"P=$($meta.parameter_words)",'-testplusarg',"I=$($meta.input_words)",'-testplusarg',"E=$($meta.expected_words)",'-testplusarg',"DW=$($meta.dw_packets)")+$sourceArgs)
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_fused_rgb2_host_xsim_$RunId"){throw 'unsafe cleanup target'}
    try {
        # The worker owns the global single-heavy mutex. Any remaining EDA
        # descendant means cleanup is deferred, never a reason to delete live files.
        $remaining=@(Get-Process -Name xsim,xsimk,xelab,xvlog,vvp,iverilog,ivl,efx_map,efx_pnr -ErrorAction SilentlyContinue)
        if($remaining.Count -or -not $safeCleanup){throw 'tool still active or exit unverified; private directory retained'}
