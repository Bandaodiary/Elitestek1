...
# Launch Vivado/xsim through Win32_Process.Create so the worker is outside the
# Codex desktop Windows Job.  This is intentionally a separate native-DMA
# runner: a stalled loopback cannot terminate a full portable-SoC experiment.
...
if (-not $Worker) {
    ...
    [ordered]@{
        ...
        status_path = Join-Path $logRoot "native_dma_loopback_runs\$RunId\status.json"
    } | ConvertTo-Json
    exit 0
}
...
$runRoot = Join-Path $simRoot "native_dma_loopback_run_$RunId"
$runLogRoot = Join-Path $logRoot "native_dma_loopback_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'native_dma_loopback_status.json'
...
try {
    Write-Status running setup 0 'detached native read/write DMA loopback worker started'
    $sources = @(
        ...
        (Join-Path $rtlRoot 'dma\c1_axi_write_skid_bridge.sv'),
        (Join-Path $rtlRoot 'dma\c1_axi2_serial_arbiter_128.sv'),
        (Join-Path $simRoot 'tb_c1_r1_native_dma_loopback.sv')
    )
    Invoke-VivadoStep xvlog (Join-Path $vivadoBin 'xvlog.bat') (@('-sv') + $sources)
    Invoke-VivadoStep xelab (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_r1_native_dma_loopback','-s','tb_c1_r1_native_dma_loopback_sim')
    if ($CompileOnly) {
        $script:watch.Stop()
        Write-Status complete elaboration 0 `
            'C1_R1_NATIVE_DMA_LOOPBACK_ELAB_PASS frame=3x640x480'
        exit 0
    }
    Invoke-VivadoStep xsim (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_r1_native_dma_loopback_sim','-runall') `
        'C1_R1_NATIVE_DMA_LOOPBACK_PASS'
    $script:watch.Stop()
    Write-Status complete done 0 'C1_R1_NATIVE_DMA_LOOPBACK_PASS frame=3x640x480'
} catch {
    $script:watch.Stop()
    ...
    if (Test-Path -LiteralPath $runRoot -PathType Container) {
        ...
        if ((Split-Path -Parent $resolvedRun) -ne $resolvedSim -or
            (Split-Path -Leaf $resolvedRun) -notmatch '^native_dma_loopback_run_[A-Za-z0-9_-]+$') {
            throw "Refusing cleanup outside native DMA simulation directory: $resolvedRun"
        }
