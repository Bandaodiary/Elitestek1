...
# Launch all Vivado tools from a WMI-created hidden worker so xsim is outside
# the Codex Windows Job.  Success requires zero tool exits, empty stderr,
# clean Fatal/Error/FAIL diagnostics, and one exact self-check PASS marker.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot `
        "xsim_runs\tensor_mem_axi128_bridge\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_run_tensor_mem_axi128_bridge_$RunId"
$runLogRoot = Join-Path $logRoot `
    "xsim_runs\tensor_mem_axi128_bridge\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot `
    'xsim_runs\tensor_mem_axi128_bridge_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached tensor-memory AXI128 bridge worker started'
    Invoke-VivadoStep -Name 'xvlog' `
        -Tool (Join-Path $vivadoBin 'xvlog.bat') -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_tensor_mem_axi128_bridge.sv')
        )
    Invoke-VivadoStep -Name 'xelab' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') -Arguments @(
            'tb_c1_tensor_mem_axi128_bridge',
            '-s', 'tb_c1_tensor_mem_axi128_bridge_sim'
        )
    Invoke-VivadoStep -Name 'xsim' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') -Arguments @(
            'tb_c1_tensor_mem_axi128_bridge_sim', '-runall'
        ) -ExpectedPass 'C1_TENSOR_MEM_AXI128_BRIDGE_PASS'
    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'C1_TENSOR_MEM_AXI128_BRIDGE_PASS'
} catch {
    $script:totalWatch.Stop()
