...
# The caller only creates this hidden worker through WMI.  Vivado and xsim are
# never descendants of the Codex Windows Job.  The worker accepts a run only
# when every tool returns zero, every stderr is empty, no Fatal/Error/FAIL is
# present, and xsim emits exactly one dedicated PASS marker.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "apb_r1_mux_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_apb_r1_mux_run_$RunId"
$runLogRoot = Join-Path $logRoot "apb_r1_mux_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'apb_r1_mux_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached APB R1 mux worker started'
    Invoke-VivadoStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            '-sv',
            (Join-Path $rtlRoot 'control\c1_apb_r1_mux.sv'),
            (Join-Path $simRoot 'tb_c1_apb_r1_mux.sv')
        )
    Invoke-VivadoStep -Name 'xelab_apb_r1_mux' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @('tb_c1_apb_r1_mux', '-s', 'tb_c1_apb_r1_mux_sim')
    Invoke-VivadoStep -Name 'xsim_apb_r1_mux' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_apb_r1_mux_sim', '-runall') `
        -ExpectedPass 'C1_APB_R1_MUX_PASS'
    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'APB R1 mux xsim regression complete'
} catch {
    $script:totalWatch.Stop()
