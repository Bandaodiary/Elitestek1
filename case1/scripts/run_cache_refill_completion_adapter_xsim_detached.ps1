...
# Detached compact xsim runner for the exact-count cancellation adapter.
# WMI starts the worker outside the caller's Windows Job; raw simulator files
# live only in a private runRoot and are removed in finally.
...
if (-not $Worker) {
    if ([string]::IsNullOrWhiteSpace($RunId)) {
        $RunId = 'completion_' + (Get-Date -Format 'yyyyMMdd_HHmmss')
    }
    if ($RunId -notmatch '^[A-Za-z0-9_-]+$') { throw 'invalid RunId' }
    $runLog = Join-Path $logRoot "xsim_runs\cache_refill_completion_adapter\$RunId"
...
if ($RunId -notmatch '^[A-Za-z0-9_-]+$') { throw 'invalid RunId' }
$runRoot = Join-Path $simRoot "xsim_cache_refill_completion_adapter_$RunId"
$runLogRoot = Join-Path $logRoot "xsim_runs\cache_refill_completion_adapter\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_cache_refill_completion_adapter_status.json'
...
try {
    Write-Status 'running' 'setup' 0 'detached completion-adapter worker started'
    ...
    @(
        ...
        (Join-Path $simRoot 'tb_c1_cache_refill_completion_adapter.sv')
    ...
    Invoke-Step 'xelab' (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_cache_refill_completion_adapter','-s',
        'tb_c1_cache_refill_completion_adapter_sim','-nolog')
    $marker='C1_CACHE_REFILL_COMPLETION_ADAPTER_PASS'
    Invoke-Step 'xsim' (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_cache_refill_completion_adapter_sim','-runall','-nolog') $marker
