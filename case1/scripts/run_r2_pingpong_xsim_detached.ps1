<## WMI-detached C8 graph simulation. Only bounded logs survive; vectors,
...
$runLog=Join-Path $caseRoot "logs\r2_pingpong_xsim_runs\$RunId"
...
$runRoot=Join-Path $simRoot "c1_r2_pingpong_xsim_$RunId"
...
$workerInJob=$null
$runState='running';$runStep='prepare';$runExit=0;$runMessage='detached C8 graph simulation'
function Save-State {
...
function Invoke-Step([string]$Name,[string]$Executable,[string[]]$Arguments) {
    ...
    try {
        ...
        if($Name -eq 'xsim') {
            $markers=@(Get-Content -LiteralPath $stdout | Where-Object {$_ -match '^C1_R2_PINGPONG_'})
            $markers|Set-Content -LiteralPath (Join-Path $runLog 'result.log') -Encoding UTF8
            if(@($markers|Where-Object {$_ -match '^C1_R2_PINGPONG_PASS '}).Count -ne 1){throw 'missing graph pass marker'}
        }
    } finally {$p.Dispose()}
}
try {
...
public static class C8JobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C8JobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'Cannot verify Windows Job isolation'}
    ...
    [xml]$project=Get-Content -LiteralPath (Join-Path $caseRoot 'efinity\c1_ti60_r2_pingpong96.xml') -Raw
    $sources=@($project.SelectNodes("//*[local-name()='design_file']")|Where-Object {$_.name -ne 'c1_ti60_r2_pingpong96.sv'}|ForEach-Object {Join-Path (Join-Path $caseRoot 'efinity') $_.name})
    $top='tb_c1_r2_microstyle_pingpong_graph';$bankWords=if($FrameWidth*$FrameHeight -gt 8192){524288}else{16384}
    Invoke-Step 'xvlog' (Join-Path $vivado 'xvlog.bat') (@('-sv')+$sources+(Join-Path $caseRoot "sim\$top.sv"))
    Invoke-Step 'xelab' (Join-Path $vivado 'xelab.bat') @($top,'-s','r2_pingpong_sim','-generic_top',"BANK_WORDS=$bankWords",'-generic_top','FRAME_RUNS=1','-generic_top','MAX_CYCLES=40000000','-generic_top',"MEMORY_DIV=$MemoryDiv",'-generic_top',"COMMAND_LATENCY=$CommandLatency",'-mt','2')
    $vectorsPath=$runRoot.Replace('\','/')
    Invoke-Step 'xsim' (Join-Path $vivado 'xsim.bat') @('r2_pingpong_sim','-runall','-testplusarg',"DIR=$vectorsPath",'-testplusarg',"W=$FrameWidth",'-testplusarg',"H=$FrameHeight",'-testplusarg',"P=$($meta.parameter_words)",'-testplusarg',"I=$($meta.input_words)",'-testplusarg',"E=$($meta.expected_words)")
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_pingpong_xsim_$RunId"){throw 'unsafe cleanup target'}
