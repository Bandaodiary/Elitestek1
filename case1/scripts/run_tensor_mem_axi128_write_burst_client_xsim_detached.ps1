...
# Detached Vivado/xsim worker for the boardless write-burst prototype.  The
# simulator owns a private run tree and removes it in finally; only compact
# stdout/stderr/status files remain under case1/logs.
...
if (-not $Worker) {
    if ($ReqPopRefill -and $ReqPopRefillBaseline) {
        throw 'ReqPopRefill and ReqPopRefillBaseline are mutually exclusive'
    }
    if ([string]::IsNullOrWhiteSpace($RunId)) {
        $RunId = [guid]::NewGuid().ToString('N')
    }
    $statusPath = Join-Path $logRoot "xsim_runs\tensor_write_burst\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_run_tensor_write_burst_$RunId"
$runLogRoot = Join-Path $logRoot "xsim_runs\tensor_write_burst\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_runs\tensor_write_burst_status.json'
...
try {
    ...
    Write-Status 'running' 'setup' 0 'detached tensor write-burst worker started'
    $xvlogArgs = @('-sv', '-nolog')
    if ($ReqPopRefill) { $xvlogArgs += @('-d', 'C1_WRITE_REQ_POP_REFILL_TB') }
    $topName = 'tb_c1_tensor_mem_axi128_write_burst_client'
    $simName = 'tb_c1_tensor_mem_axi128_write_burst_client_sim'
    $tbSource = Join-Path $simRoot 'tb_c1_tensor_mem_axi128_write_burst_client.sv'
    $marker = 'C1_TENSOR_MEM_AXI128_WRITE_BURST_CLIENT_PASS'
    if ($ReqPopRefill -or $ReqPopRefillBaseline) {
        $topName = 'tb_c1_tensor_mem_axi128_write_burst_client_req_pop_refill'
        $simName = 'tb_c1_tensor_mem_axi128_write_burst_client_req_pop_refill_sim'
        $tbSource = Join-Path $simRoot 'tb_c1_tensor_mem_axi128_write_burst_client_req_pop_refill.sv'
        $marker = 'C1_TENSOR_MEM_AXI128_WRITE_BURST_CLIENT_REQ_POP_REFILL_PASS'
    }
    $xvlogArgs += @(
