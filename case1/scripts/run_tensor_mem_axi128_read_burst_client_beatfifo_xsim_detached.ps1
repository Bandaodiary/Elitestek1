...
# Detached xsim regression for the optional beat-record response FIFO.
# Win32_Process.Create keeps Vivado outside the caller's Windows Job; the
# private simulator tree is deleted when the worker exits.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "xsim_runs\tensor_read_beatfifo\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_run_tensor_read_beatfifo_$RunId"
$runLogRoot = Join-Path $logRoot "xsim_runs\tensor_read_beatfifo\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_tensor_read_beatfifo_status.json'
...
try {
    Write-Status 'running' 'setup' 0 'detached beat-FIFO worker started'
    Invoke-Step 'xvlog' (Join-Path $vivadoBin 'xvlog.bat') @(
        '-sv', '-nolog', '-d', 'C1_BEAT_FIFO_TB',
        (Join-Path $rtlRoot 'dma\c1_tensor_mem_axi128_read_burst_client.sv'),
        (Join-Path $simRoot 'tb_c1_tensor_mem_axi128_read_burst_client.sv'))
    Invoke-Step 'xelab' (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_tensor_mem_axi128_read_burst_client', '-s',
        'tb_c1_tensor_mem_axi128_read_burst_client_beatfifo_sim', '-nolog')
    Invoke-Step 'xsim' (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_tensor_mem_axi128_read_burst_client_beatfifo_sim', '-runall', '-nolog') `
        'C1_TENSOR_MEM_AXI128_READ_BURST_CLIENT_BEAT_FIFO_PASS'
    $watch.Stop()
