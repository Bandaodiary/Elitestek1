...
# Win32_Process.Create launches the hidden Vivado worker outside the calling
# Codex Windows Job.  Success requires all zero exit codes, empty stderr, no
# Fatal/Error/FAIL diagnostics, and exactly one resolver PASS marker.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "frame_pair_resolver_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_frame_pair_resolver_run_$RunId"
$runLogRoot = Join-Path $logRoot "frame_pair_resolver_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'frame_pair_resolver_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached frame-pair resolver worker started'
    Invoke-VivadoStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_frame_pair_resolver.sv')
        )
    Invoke-VivadoStep -Name 'xelab_frame_pair_resolver' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @(
            'tb_c1_frame_pair_resolver',
            '-s',
            'tb_c1_frame_pair_resolver_sim'
        )
    Invoke-VivadoStep -Name 'xsim_frame_pair_resolver' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_frame_pair_resolver_sim', '-runall') `
        -ExpectedPass 'C1_FRAME_PAIR_RESOLVER_PASS'
    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'frame-pair resolver xsim regression complete'
} catch {
    $script:totalWatch.Stop()
