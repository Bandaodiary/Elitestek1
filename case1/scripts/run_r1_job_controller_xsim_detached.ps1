...
# xsim are therefore never children of the calling Codex Windows Job.  A run
# passes only when all tools return zero, stderr is empty, diagnostics contain
# no Fatal/Error/FAIL, and xsim emits exactly one controller PASS marker.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "r1_job_controller_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_r1_job_controller_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_job_controller_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'r1_job_controller_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached R1 job-controller worker started'
    Invoke-VivadoStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            '-sv',
            (Join-Path $rtlRoot 'control\c1_r1_job_controller.sv'),
            (Join-Path $simRoot 'tb_c1_r1_job_controller.sv')
        )
    Invoke-VivadoStep -Name 'xelab_r1_job_controller' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @(
            'tb_c1_r1_job_controller',
            '-s',
            'tb_c1_r1_job_controller_sim'
        )
    Invoke-VivadoStep -Name 'xsim_r1_job_controller' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_r1_job_controller_sim', '-runall') `
        -ExpectedPass 'C1_R1_JOB_CONTROLLER_PASS'
    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'R1 job-controller xsim regression complete'
} catch {
    $script:totalWatch.Stop()
