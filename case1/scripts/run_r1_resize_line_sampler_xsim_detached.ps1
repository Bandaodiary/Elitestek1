...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $caseRoot `
        "logs\r1_resize_line_sampler_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "r1_resize_line_sampler_xsim_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_resize_line_sampler_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_r1_resize_line_sampler_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached R1 resize line sampler verification worker started'
    ...
    Invoke-LoggedStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_r1_resize_line_sampler.sv')
        )
    Invoke-LoggedStep -Name 'xelab' -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @(
            'tb_c1_r1_resize_line_sampler',
            '-s', 'tb_c1_r1_resize_line_sampler_sim'
        )
    Invoke-LoggedStep -Name 'xsim' -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_r1_resize_line_sampler_sim', '-runall')

    $manifest = Get-Content -Raw -LiteralPath `
        (Join-Path $runRoot 'r1_resize_line_sampler_manifest.json') |
        ConvertFrom-Json
    $expectedPass = "C1_R1_RESIZE_LINE_SAMPLER_PASS " +
                    "configs=$($manifest.configs) " +
                    "outputs=$($manifest.outputs) " +
                    "errors=$($manifest.error_checks)"
