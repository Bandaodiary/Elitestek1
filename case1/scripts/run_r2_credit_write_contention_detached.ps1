# C33 real AXI two-writer diagnostic; does not change the retained host.
[CmdletBinding()]
param(
    [switch]$Worker,[string]$RunId='',
    [string]$WriterUnitRun='c32_writer_matrix_20260914_d',
    [ValidateRange(1,3600)][int]$TimeoutSeconds=900
)
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
if($RunId -eq ''){$RunId=[guid]::NewGuid().ToString('N')}
if($RunId -notmatch '^[A-Za-z0-9_-]+$' -or $WriterUnitRun -notmatch '^[A-Za-z0-9_-]+$'){throw 'invalid RunId'}
$runLog=Join-Path $caseRoot "logs\r2_credit_write_axi_runs\$RunId"
$statusPath=Join-Path $runLog 'status.json'
if(-not $Worker){
    if(Test-Path -LiteralPath $runLog){throw 'RunId exists'}
    $unitStatus=Join-Path $caseRoot "logs\r2_cutthrough_writer_runs\$WriterUnitRun\status.json"
    ...
    $command="`"$ps`" -NoLogo -NoProfile -NonInteractive -WindowStyle Hidden -ExecutionPolicy Bypass -File `"$PSCommandPath`" -Worker -RunId $RunId -WriterUnitRun $WriterUnitRun -TimeoutSeconds $TimeoutSeconds"
    try{
...
$runRoot=Join-Path $simRoot "c1_r2_credit_write_axi_$RunId"
...
$state='running';$step='prepare';$runExit=0;$message='C33 actual AXI competing-writer diagnostic'
...
function Save-State{
    [ordered]@{run_id=$RunId;state=$state;step=$step;exit_code=$runExit;message=$message;
        ...
        predecessor_writer_run=$WriterUnitRun;elapsed_seconds=[math]::Round($watch.Elapsed.TotalSeconds,3);run_directory=$runRoot;
...
function Invoke-PythonProbe([string]$Arguments,[string]$Stem,[int]$Seconds){
    ...
    try{
        if(-not $process.Start()){throw 'could not start C33 AXI probe'}
        ...
        if($timedOut){throw 'C33 AXI probe timeout; partial checkpoints retained'}
        if($null -eq $process.ExitCode -or $process.ExitCode -ne 0){throw "C33 $Stem failed exit=$($process.ExitCode)"}
        return $out
...
public static class C33AxiJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C33AxiJobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'cannot check Job isolation'}
    ...
    $gate=Invoke-PythonProbe ('-B -u "'+(Join-Path $caseRoot 'golden\check_r2_cutthrough_writer_evidence.py')+'" --run '+$WriterUnitRun) 'writer_gate' 60
    if($gate -notmatch 'C32_WRITER_UNIT_GATE_PASS '){throw 'writer predecessor gate incomplete'}
    $step='regression';Save-State
    $out=Invoke-PythonProbe ('-B -u "'+(Join-Path $caseRoot 'golden\run_r2_credit_write_contention.py')+'" --temporary-parent "'+$runRoot+'"') 'result' $TimeoutSeconds
    if([regex]::Matches($out,'(?m)^C33_WRITE_AXI_PASS ').Count -ne 58 -or
       [regex]::Matches($out,'(?m)^C33_WRITE_AXI_COMPARE ').Count -ne 12 -or
       [regex]::Matches($out,'(?m)^C33_WRITE_AXI_NEGATIVE_PASS ').Count -ne 2 -or
       [regex]::Matches($out,'(?m)^C33_WRITE_AXI_SUMMARY ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C33_WRITE_AXI_CLEAN temporary_vectors_and_simulator_removed=1').Count -ne 1 -or
       $out -cmatch 'FATAL|ERROR:|Traceback|RuntimeError'){throw 'C33 AXI evidence incomplete'}
    $state='complete';$step='done';$message='C33 AXI diagnostic completed; signed contention deltas are not a speedup gate'
}catch{$state='failed';$step='error';$runExit=1;$message=$_.Exception.Message}
finally{
    $resolved=[IO.Path]::GetFullPath($runRoot)
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_credit_write_axi_$RunId"){throw 'unsafe private cleanup target'}
