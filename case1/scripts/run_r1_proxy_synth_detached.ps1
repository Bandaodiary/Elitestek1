...
# WMI launches the worker outside the caller's Windows Job.  Vivado then runs
# as a child of that detached worker and records durable status/logs.
...
if (-not $Worker) {
    ...
    [ordered]@{
        ...
        status_path = (Join-Path $logRoot "r1_proxy_synth_runs\$RunId\status.json")
    } | ConvertTo-Json
    return
}
...
$runRoot = Join-Path $caseRoot "sim\r1_proxy_synth_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_proxy_synth_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'r1_proxy_synth_status.json'
$vivado = 'D:\vivado\vivado\Vivado\2023.1\bin\vivado.bat'
$vivadoRtScripts = 'D:\vivado\vivado\Vivado\2023.1\scripts\rt'
$tcl = Join-Path $PSScriptRoot 'synth_r1_components_proxy.tcl'
$expectedComponents = 14
...
try {
    # Some files in this local Vivado installation carry Windows on-demand
    # recall attributes.  Read the small Tcl set before Vivado launches so its
    # helper cannot misreport a real file as missing.  This is read-only.
    ...
    Get-ChildItem -LiteralPath $vivadoRtScripts -Filter '*.tcl' -File -Recurse |
        ForEach-Object {
            ...
            while ($prewarmStream.Read(
                $prewarmBuffer, 0, $prewarmBuffer.Length) -gt 0) {}
            $prewarmStream.Dispose()
        }
    Write-Status running vivado 0 'detached R1 component proxy synthesis started'
    ...
    if ([regex]::Matches($stdout, 'C1_R1_PROXY_COMPONENT_PASS').Count -ne $expectedComponents) {
        throw "Vivado did not emit exactly $expectedComponents component PASS markers"
    }
    if ([regex]::Matches(
            $stdout,
            "C1_R1_PROXY_SYNTH_PASS components=$expectedComponents").Count -ne 1) {
        throw 'Vivado did not emit the unique final PASS marker'
    }
    $watch.Stop()
    Write-Status complete done 0 "C1_R1_PROXY_SYNTH_PASS components=$expectedComponents"
} catch {
    $watch.Stop()
