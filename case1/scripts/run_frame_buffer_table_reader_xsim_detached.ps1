...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $caseRoot "logs\frame_buffer_table_reader_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "frame_buffer_table_reader_xsim_run_$RunId"
$runLogRoot = Join-Path $logRoot "frame_buffer_table_reader_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_frame_buffer_table_reader_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached framebuffer table reader verification worker started'

    Invoke-LoggedStep -Name 'vector_gen' -Tool $pythonExe -Arguments @(
        (Join-Path $goldenRoot 'generate_frame_buffer_table_reader_vectors.py'),
        '--output-dir', $runRoot
    )
    Invoke-LoggedStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_axi_frame_buffer_table_reader.sv')
        )
    Invoke-LoggedStep -Name 'xelab' -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @('tb_c1_axi_frame_buffer_table_reader',
                     '-s', 'tb_c1_axi_frame_buffer_table_reader_sim')
    Invoke-LoggedStep -Name 'xsim' -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_axi_frame_buffer_table_reader_sim', '-runall')

    $manifest = Get-Content -Raw -LiteralPath `
        (Join-Path $runRoot 'frame_buffer_table_reader_manifest.json') | ConvertFrom-Json
    $passPrefix = "C1_AXI_FRAME_BUFFER_TABLE_READER_PASS entries=$($manifest.entries)"
    $simOutput = Get-Content -Raw -LiteralPath (Join-Path $runLogRoot 'xsim.stdout.log')
    if ([regex]::Matches($simOutput, [regex]::Escape($passPrefix)).Count -ne 1) {
        throw "xsim output does not contain exactly one PASS marker: $passPrefix"
    }
    ...
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 -Message $passPrefix
} catch {
