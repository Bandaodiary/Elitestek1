...
[CmdletBinding()]
param(
    [switch]$Worker,[switch]$CheckOnly,[string]$RunId='c35_row_fused_fault_20260914_a',
    [ValidateRange(1,1800)][int]$TimeoutSeconds=1200
)
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
if($RunId -notmatch '^[A-Za-z0-9_-]+$'){throw 'invalid RunId'}
$depRun='c35_row_fused_wide_20260914_b'
$depPid=42096;$depStart='2026-09-14T23:49:01.9732253+08:00'
$depStatus=Join-Path $caseRoot "logs\r2_row_fused_wide_runs\$depRun\status.json"
$simRoot=(Resolve-Path (Join-Path $caseRoot 'sim')).Path
$depPrivate=Join-Path $simRoot "c1_r2_row_fused_wide_$depRun"
$runRoot=Join-Path $simRoot "c1_r2_row_fused_fault_$RunId"
$runLog=Join-Path $caseRoot "logs\r2_row_fused_fault_runs\$RunId"
...
public static class C35FusionFaultJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $current=[Diagnostics.Process]::GetCurrentProcess();$inJob=$false
    if(-not [C35FusionFaultJobCheck]::IsProcessInJob($current.Handle,[IntPtr]::Zero,[ref]$inJob)){throw 'cannot verify Windows Job'}
    ...
    $state='running';$step='component';$message='directed fused-stage read/write errors and system resets';Save-State
    $info=New-Object Diagnostics.ProcessStartInfo
    $info.FileName='D:\miniconda\miniconda\envs\p300_task3_bci3\python.exe'
    $info.Arguments='-B -u "'+(Join-Path $caseRoot 'golden\run_r2_row_fused_fault_probe.py')+'" --wide-run '+$depRun+' --temporary-parent "'+$runRoot+'"'
    ...
    if($child.ExitCode -ne 0 -or $err.Trim() -or
       [regex]::Matches($out,'(?m)^C35_ROW_FUSED_GRAPH_WIDE_EVIDENCE_PASS ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C35_ROW_FUSED_FAULTS_EVIDENCE ').Count -ne 1 -or
       $out -notmatch '(?m)^C35_ROW_FUSED_FAULTS_CLEAN temporary_vectors_and_simulator_removed=1'){
        throw 'C35 fused error/reset evidence incomplete or failed'
    }
    $state='complete';$step='done';$message='fused-stage row-transfer errors and system resets passed; not AXI/native fps/PNR'
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or
       (Split-Path -Leaf $resolved) -ne "c1_r2_row_fused_fault_$RunId"){throw 'unsafe own C35 cleanup target'}
