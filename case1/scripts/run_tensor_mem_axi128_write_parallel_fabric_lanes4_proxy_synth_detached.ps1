...
# Detached Vivado proxy synthesis for the optional four-lane write fabric.
# Only compact reports are retained; the private project tree is removed.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "tensor_mem_axi128_write_parallel_fabric_lanes4_synth_runs\$RunId\status.json"
...
$runRoot = Join-Path $caseRoot "sim\.tmp_tensor_mem_axi128_write_parallel_fabric_lanes4_synth_$RunId"
$runLogRoot = Join-Path $logRoot "tensor_mem_axi128_write_parallel_fabric_lanes4_synth_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'tensor_mem_axi128_write_parallel_fabric_lanes4_synth_status.json'
...
try {
    Write-Status 'running' 'synthesis' 0 'detached four-lane write-fabric proxy synthesis started'
    # Warm the small runtime Tcl files for a WMI-created worker without
    # retaining a persistent Vivado project/runtime tree.
    ...
    $args = @('-mode','batch','-nolog','-nojournal','-notrace','-source',
              (Join-Path $scriptRoot 'synth_tensor_mem_axi128_write_parallel_fabric_lanes4.tcl'))
    ...
    if ($all -notmatch 'C1_TENSOR_MEM_AXI128_WRITE_PARALLEL_LANES4_PROXY_SYNTH_PASS') {
        throw 'Vivado did not emit the four-lane synthesis PASS marker'
    }
    $reportDir = Join-Path $runRoot 'reports'
    $util = Join-Path $reportDir 'utilization.rpt'
    $timing = Join-Path $reportDir 'timing.rpt'
    $summary = [ordered]@{
        run_id = $RunId; part = 'xc7a200tsbg484-1'; lanes = 4;
        block_logical = 16; req_fifo_depth = 32; burst_beats = 16;
        fabric_fifo_depth = 6; tag_fifo_depth = 128
    }
    ...
    if (Test-Path -LiteralPath $timing) {
        ...
        for ($ti = 0; $ti -lt $timing_lines.Count; $ti++) {
            if ($timing_lines[$ti] -match 'WNS\(ns\)' -and $timing_lines[$ti] -match 'TNS\(ns\)') {
                for ($tj = $ti + 1; $tj -lt [math]::Min($ti + 10, $timing_lines.Count); $tj++) {
                    $tm = [regex]::Match($timing_lines[$tj], '^\s*([-+]?[0-9]+(?:\.[0-9]+)?)\s+([-+]?[0-9]+(?:\.[0-9]+)?)\s+')
                    if ($tm.Success) {
    ...
    $watch.Stop()
    Write-Status 'complete' 'done' 0 'C1_TENSOR_MEM_AXI128_WRITE_PARALLEL_LANES4_PROXY_SYNTH_PASS'
} catch {
    $watch.Stop()
