param(
    [switch]$Worker,
    [string]$RunId = '',
    [switch]$EmptyArBypass
)

# Detached boardless regression for the optional ID-less multi-burst read
# arbiter.  WMI creates the worker outside the caller's Windows Job; the worker
# keeps only small stdout/stderr/status files and removes its Vivado run tree.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "xsim_runs\axi_n_read_burst_arbiter\$RunId\status.json"
    $powerShell = "$env:SystemRoot\System32\WindowsPowerShell\v1.0\powershell.exe"
    $commandLine = "`"$powerShell`" -NoLogo -NoProfile -NonInteractive " +
        "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$PSCommandPath`" " +
        "-Worker -RunId $RunId" +
        $(if ($EmptyArBypass) { ' -EmptyArBypass' } else { '' })
...
$runRoot = Join-Path $simRoot "xsim_run_axi_n_read_burst_arbiter_$RunId"
$runLogRoot = Join-Path $logRoot "xsim_runs\axi_n_read_burst_arbiter\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_axi_n_read_burst_arbiter_status.json'
...
function Invoke-Step {
    param([string]$Name, [string]$Tool, [string[]]$ToolArgs,
          [string]$Marker = '')
    ...
    $all = $stdout + "`n" + $stderr
    if ($all -match '(?im)(^|\s)(Fatal|Error):|\$fatal|C1_AXI_N_READ_BURST_ARBITER_FAIL') {
...
try {
    Write-Status 'running' 'setup' 0 'detached ID-less multi-burst arbiter started'
    $xvlog = Join-Path $vivadoBin 'xvlog.bat'
    $xelab = Join-Path $vivadoBin 'xelab.bat'
    $xsim = Join-Path $vivadoBin 'xsim.bat'
    $defines = if ($EmptyArBypass) {
        @('-d', 'C1_EMPTY_AR_BYPASS_TB')
    } else { @() }
    $xvlogArgs = @('-sv', '-nolog') + $defines + @(
        (Join-Path $rtlRoot 'dma\c1_axi_n_read_burst_arbiter_128.sv'),
        (Join-Path $simRoot 'tb_c1_axi_n_read_burst_arbiter_128.sv'))
    Invoke-Step 'xvlog' $xvlog $xvlogArgs
    Invoke-Step 'xelab' $xelab @(
        'tb_c1_axi_n_read_burst_arbiter_128', '-s',
        'tb_c1_axi_n_read_burst_arbiter_128_sim', '-nolog')
    Invoke-Step 'xsim' $xsim @(
        'tb_c1_axi_n_read_burst_arbiter_128_sim', '-runall', '-nolog') `
        'C1_AXI_N_READ_BURST_ARBITER_PASS'
    $watch.Stop()
    Write-Status 'complete' 'done' 0 'C1_AXI_N_READ_BURST_ARBITER_PASS'
} catch {
    $watch.Stop()
