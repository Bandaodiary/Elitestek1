...
# Detached Vivado/xsim runner for the opt-in AW-before-payload seam.  The
# caller only creates a WMI worker; Vivado/xsim are children of that worker,
# not of the Codex Windows job.  The private xsim tree is deleted in finally,
# leaving only compact status/stdout/stderr logs under case1/logs.
$ErrorActionPreference = 'Stop'
$caseRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$rtlFile = Join-Path $caseRoot 'rtl\dma\c1_axi128_write_mlp.sv'
$tbFile = Join-Path $caseRoot 'sim\tb_c1_axi128_write_mlp_aw_before_payload.sv'
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "xsim_runs\axi128_write_mlp_aw_before_payload\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_run_axi128_write_mlp_aw_before_payload_$RunId"
$runLogRoot = Join-Path $logRoot "xsim_runs\axi128_write_mlp_aw_before_payload\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_axi128_write_mlp_aw_before_payload_status.json'
...
try {
    Write-Status 'running' 'setup' 0 'detached AW-before-payload xsim worker started'
    Invoke-Step 'xvlog' (Join-Path $vivadoBin 'xvlog.bat') @(
        '-sv', '-nolog', $rtlFile, $tbFile)
    Invoke-Step 'xelab' (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_axi128_write_mlp_aw_before_payload',
        '-s', 'tb_c1_axi128_write_mlp_aw_before_payload_sim', '-nolog')
    Invoke-Step 'xsim' (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_axi128_write_mlp_aw_before_payload_sim', '-runall', '-nolog') `
        'C1_AXI128_WRITE_MLP_AW_EARLY_PASS'
    $watch.Stop()
    Write-Status 'complete' 'done' 0 'C1_AXI128_WRITE_MLP_AW_EARLY_PASS'
} catch {
    $watch.Stop()
