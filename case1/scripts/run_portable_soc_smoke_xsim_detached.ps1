...
# The outer invocation uses Win32_Process.Create so Vivado/xsim and all of
# their descendants live outside the caller's Windows Job.  The worker itself
# remains hidden and writes an atomic-enough JSON status plus per-step logs.
...
if (-not $Worker) {
    ...
    # Managed desktop sessions may deny Win32_Process.Create.  Fall back to
    # the native helper, which requests CREATE_BREAKAWAY_FROM_JOB so the
    # Vivado/xsim worker is still independent of the calling Windows Job.
    $workerPid = $null
    try {
        ...
        # Continue to the native breakaway path below.
    }
    ...
    [ordered]@{
        ...
        status_path = Join-Path $logRoot `
            "portable_soc_smoke_runs\$RunId\status.json"
    } | ConvertTo-Json
    return
}

if ($RunId -notmatch '^[A-Za-z0-9_-]+$') { throw 'bad RunId' }
$runRoot = Join-Path $simRoot "portable_soc_smoke_run_$RunId"
$runLogRoot = Join-Path $logRoot "portable_soc_smoke_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'portable_soc_smoke_status.json'
...
try {
    Write-Status running setup 0 'detached portable SoC smoke started'

    # Packages must precede consumers.  Remaining RTL can be compiled in one
    # unit because module declaration order does not constrain elaboration.
    $packageSources = @(
    ...
    $sources = @($packageSources) + @($remainingSources) +
        @((Join-Path $simRoot 'tb_c1_r1_portable_soc_smoke.sv'))

    # The complete portable-SoC source set is long enough to exceed the
    # Windows command-line limit when passed as individual xvlog arguments.
    # Keep the response file inside the disposable runRoot and use ASCII so
    # xvlog does not treat a UTF-8 BOM as part of the first path.
    ...
    Invoke-XsimStep xelab (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_r1_portable_soc_smoke','-s',
        'tb_c1_r1_portable_soc_smoke_sim')
    Invoke-XsimStep xsim (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_r1_portable_soc_smoke_sim','-runall') `
        'C1_R1_PORTABLE_SOC_SMOKE_PASS'

    $watch.Stop()
    Write-Status complete done 0 'C1_R1_PORTABLE_SOC_SMOKE_PASS'
} catch {
    $watch.Stop()
    Write-Status failed $script:currentStep 1 $_.Exception.Message
    exit 1
} finally {
    # The full source set can generate a large xsim database.  Remove the
    # disposable run tree even when compile/elaboration fails.
