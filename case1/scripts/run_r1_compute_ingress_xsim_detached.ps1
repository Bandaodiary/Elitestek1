...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $caseRoot `
        "logs\r1_compute_ingress_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "r1_compute_ingress_xsim_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_compute_ingress_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_r1_compute_ingress_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached R1 compute ingress verification worker started'
    ...
    Invoke-LoggedStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            ...
            (Join-Path $simRoot 'c1_rv_hold_checker.sv'),
            (Join-Path $simRoot 'tb_c1_r1_compute_ingress.sv')
        )
    Invoke-LoggedStep -Name 'xelab' -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @(
            'tb_c1_r1_compute_ingress',
            '-s', 'tb_c1_r1_compute_ingress_sim'
        )
    Invoke-LoggedStep -Name 'xsim' -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_r1_compute_ingress_sim', '-runall')

    $manifest = Get-Content -Raw -LiteralPath `
        (Join-Path $runRoot 'r1_resize_line_sampler_manifest.json') |
        ConvertFrom-Json
    $expectedPass = "C1_R1_COMPUTE_INGRESS_PASS " +
                    "configs=$($manifest.configs) " +
                    "outputs=$($manifest.outputs) aborts=2"
