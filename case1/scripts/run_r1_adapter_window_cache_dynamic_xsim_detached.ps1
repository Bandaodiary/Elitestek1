...
# WMI launches the hidden worker outside the Codex Windows Job.  Both the
# original adapter marker and the dynamic cache-chain marker must occur once.
$ErrorActionPreference = 'Stop'
$caseRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$rtlRoot = Join-Path $caseRoot 'rtl'
$simRoot = Join-Path $caseRoot 'sim'
$logRoot = Join-Path $caseRoot 'logs'
$testName = 'r1_adapter_window_cache_dynamic'
$topName = 'tb_c1_r1_adapter_window_cache_dynamic'
$passMarker = 'C1_R1_ADAPTER_WINDOW_CACHE_DYNAMIC_PASS'
...
function Invoke-VivadoStep {
    ...
    if ($CheckMarkers) {
        $passCount = [regex]::Matches(
            $stdout, [regex]::Escape($passMarker)).Count
        $baseCount = [regex]::Matches(
            $stdout, [regex]::Escape($baseMarker)).Count
        if ($passCount -ne 1 -or $baseCount -ne 1) {
            throw "$Name marker mismatch dynamic=$passCount base=$baseCount"
        }
    }
    Write-Status -State 'running' -Step $Name -ExitCode 0 `
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached adapter-window-cache dynamic worker started'
    Invoke-VivadoStep -Name 'xvlog' `
        -Tool (Join-Path $vivadoBin 'xvlog.bat') -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_r1_adapter_window_cache_dynamic.sv')
        )
