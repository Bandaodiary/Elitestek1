...
# WMI launches the hidden worker outside the calling Codex Windows Job.  Each
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "window3x3_same_c8_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_window3x3_same_c8_run_$RunId"
$runLogRoot = Join-Path $logRoot "window3x3_same_c8_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'window3x3_same_c8_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached C8 3x3 SAME window worker started'
    Invoke-StrictStep -Name 'vector_gen' -Tool $pythonExe -Arguments @(
        (Join-Path $goldenRoot 'generate_window3x3_same_c8_vectors.py'),
        '--output-dir', $runRoot,
        '--seed', '20260824'
    )
    $manifest = Get-Content -Raw -LiteralPath `
        (Join-Path $runRoot 'window3x3_same_c8_vectors.json') | ConvertFrom-Json
    if ($manifest.frames -ne 9 -or $manifest.max_width -ne 640 -or
        $manifest.input_pixels -ne 1390 -or
        $manifest.output_windows -ne 402 -or
        $manifest.line_buffer_bytes -ne 10240) {
        throw 'window vector manifest does not match testbench constants'
    }
    ...
    Invoke-StrictStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            ...
            (Join-Path $rtlRoot 'cnn\c1_s8_window3x3_same_c8.sv'),
            (Join-Path $simRoot 'tb_c1_s8_window3x3_same_c8.sv')
        )
    Invoke-StrictStep -Name 'xelab_window3x3_same_c8' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @('tb_c1_s8_window3x3_same_c8', '-s', 'tb_c1_s8_window3x3_same_c8_sim')
    Invoke-StrictStep -Name 'xsim_window3x3_same_c8' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_s8_window3x3_same_c8_sim', '-runall') `
        -ExpectedPass 'C1_S8_WINDOW3X3_SAME_C8_PASS'

    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'C8 3x3 SAME window xsim regression complete'
} catch {
    $script:totalWatch.Stop()
