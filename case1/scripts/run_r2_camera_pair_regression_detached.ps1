...
[CmdletBinding()]
param([switch]$Worker,[string]$RunId='',[switch]$OverlapResize,
      [ValidateSet('matrix','odd','native256','native512','native1024')][string]$TestKind='matrix',
...
$runLog=Join-Path $caseRoot "logs\r2_camera_pair_regression_runs\$RunId"
...
$runRoot=Join-Path $simRoot "c1_r2_camera_pair_regression_$RunId"
...
public static class C30RegressionJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C30RegressionJobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'Cannot verify Windows Job isolation'}
    ...
    $arguments=@('-B','-u',(Join-Path $caseRoot 'golden\run_r2_camera_pair_capture_probe.py'),'--temporary-parent',$runRoot)
    ...
    $prefix='C1_R2_CAMERA_PAIR_CAPTURE_'
    $expected=if($TestKind -like 'native*'){1}else{2}
    $lines=@(Get-Content -LiteralPath (Join-Path $runLog 'result.log')|Where-Object {$_ -cnotmatch '^C1_R2_CAMERA_PAIR_VECTORS '})
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_camera_pair_regression_$RunId"){throw 'unsafe cleanup target'}
    try {
