...
# Detached xsim regression for the bit-exact C8 multi-group line cache.
# The Win32_Process worker intentionally lives outside the caller's Windows
# Job, so xsim/xsimk can complete even if the Codex command process exits.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "xsim_runs\window_line_cache_c8\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_run_window_line_cache_c8_$RunId"
$runLog = Join-Path $logRoot "xsim_runs\window_line_cache_c8\$RunId"
$statusPath = Join-Path $runLog 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_runs\window_line_cache_c8_status.json'
...
try {
    Write-Status 'running' 'setup' 0 'detached C8 line cache worker started'
    Invoke-Step 'xvlog' (Join-Path $vivadoBin 'xvlog.bat') @(
        '-sv',
        (Join-Path $rtlRoot 'cnn\c1_window_line_cache_c8.sv'),
        (Join-Path $simRoot 'tb_c1_window_line_cache_c8.sv')
    )
    Invoke-Step 'xelab' (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_window_line_cache_c8',
        '-s',
        'tb_c1_window_line_cache_c8_sim'
    )
    Invoke-Step 'xsim' (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_window_line_cache_c8_sim',
        '-runall'
    ) 'C1_WINDOW_LINE_CACHE_C8_PASS'
    $script:watch.Stop()
    Write-Status 'complete' 'done' 0 'C1_WINDOW_LINE_CACHE_C8_PASS'
} catch {
    $script:watch.Stop()
