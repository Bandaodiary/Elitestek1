...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $caseRoot `
        "logs\r1_compute_shell_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "r1_compute_shell_xsim_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_compute_shell_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_r1_compute_shell_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached R1 compute-shell verification worker started'

    Invoke-LoggedStep -Name 'xvlog' `
        -Tool (Join-Path $vivadoBin 'xvlog.bat') -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_r1_compute_shell.sv')
        )
    Invoke-LoggedStep -Name 'xelab' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') -Arguments @(
            'tb_c1_r1_compute_shell',
            '-s', 'tb_c1_r1_compute_shell_sim'
        )
    Invoke-LoggedStep -Name 'xsim' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') -Arguments @(
            'tb_c1_r1_compute_shell_sim', '-runall'
        )

    $passPattern = 'C1_R1_COMPUTE_SHELL_PASS starts=4 cnn_starts=4 done=3 aborts=1 cnn_faults=1 frames=3'
    $simOutput = Get-Content -Raw -LiteralPath `
        (Join-Path $runLogRoot 'xsim.stdout.log')
    if ([regex]::Matches(
            $simOutput, [regex]::Escape($passPattern)).Count -ne 1) {
        throw "xsim output does not contain exactly one PASS marker: $passPattern"
    }
