...
# Detached, compact xsim regression for the real serial arbiter + owner/epoch
# fence composition.  No waveform database is retained.
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$rtlRoot=Join-Path $caseRoot 'rtl\dma'
$simRoot=Join-Path $caseRoot 'sim'
$logRoot=Join-Path $caseRoot 'logs\axi_shared_owner_epoch_fence_arbiter_runs'
...
$tbFile=Join-Path $simRoot 'tb_c1_axi_shared_owner_epoch_fence_arbiter.sv'
...
$runRoot=Join-Path $caseRoot "sim\xsim_run_axi_shared_owner_epoch_fence_arbiter_$RunId"
...
try{
    Write-Status running setup 0 'detached arbiter/fence xsim started'
    Invoke-Step xvlog (Join-Path 'D:\vivado\vivado\Vivado\2023.1\bin' 'xvlog.bat') @('-sv',$fenceFile,$arbiterFile,$tbFile)
    Invoke-Step xelab (Join-Path 'D:\vivado\vivado\Vivado\2023.1\bin' 'xelab.bat') @('tb_c1_axi_shared_owner_epoch_fence_arbiter','-s','tb_c1_axi_shared_owner_epoch_fence_arbiter_sim')
    Invoke-Step xsim (Join-Path 'D:\vivado\vivado\Vivado\2023.1\bin' 'xsim.bat') @('tb_c1_axi_shared_owner_epoch_fence_arbiter_sim','-runall') 'C1_AXI_SHARED_OWNER_EPOCH_FENCE_ARBITER_PASS'
    $watch.Stop();Write-Status complete done 0 'C1_AXI_SHARED_OWNER_EPOCH_FENCE_ARBITER_PASS'
}catch{
