...
# The public entry point uses WMI to create a hidden worker outside the
# calling Codex Windows Job.  Every tool step must exit zero, leave stderr
# empty, and contain no Fatal/Error/FAIL diagnostic.  XSim must emit exactly
# one module-specific PASS marker.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "residual_add_c8_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_residual_add_c8_run_$RunId"
$runLogRoot = Join-Path $logRoot "residual_add_c8_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'residual_add_c8_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached C8 residual-add worker started'
    Invoke-StrictStep -Name 'vector_gen' -Tool $pythonExe -Arguments @(
        (Join-Path $goldenRoot 'generate_residual_add_c8_vectors.py'),
        '--output-dir', $runRoot,
        '--seed', '20260824'
    )
    $manifest = Get-Content -Raw -LiteralPath `
        (Join-Path $runRoot 'residual_add_c8_vectors.json') | ConvertFrom-Json
    if ($manifest.frames -ne 6 -or $manifest.pixels -ne 149 -or
        $manifest.directed_lane_pairs.Count -ne 8) {
        throw 'residual-add vector manifest does not match testbench constants'
    }

    Invoke-StrictStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            '-sv',
            (Join-Path $rtlRoot 'cnn\c1_residual_add_c8.sv'),
            (Join-Path $simRoot 'tb_c1_residual_add_c8.sv')
        )
    Invoke-StrictStep -Name 'xelab_residual_add_c8' `
        -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @('tb_c1_residual_add_c8', '-s', 'tb_c1_residual_add_c8_sim')
    Invoke-StrictStep -Name 'xsim_residual_add_c8' `
        -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_residual_add_c8_sim', '-runall') `
        -ExpectedPass 'C1_RESIDUAL_ADD_C8_PASS'

    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'C8 residual-add xsim regression complete'
} catch {
    $script:totalWatch.Stop()
