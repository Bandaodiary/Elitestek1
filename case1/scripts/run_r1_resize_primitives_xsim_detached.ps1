...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $caseRoot "logs\r1_resize_primitive_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "r1_resize_primitives_xsim_run_$RunId"
$runLogRoot = Join-Path $logRoot "r1_resize_primitive_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_r1_resize_primitives_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached R1 resize primitive verification worker started'

    Invoke-LoggedStep -Name 'vector_gen' -Tool $pythonExe -Arguments @(
        (Join-Path $goldenRoot 'generate_r1_resize_primitive_vectors.py'),
        '--output-dir', $runRoot,
        '--random-count', '10000',
        '--seed', '20260824'
    )
    Invoke-LoggedStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            ...
            (Join-Path $simRoot 'tb_c1_r1_resize_primitives.sv')
        )
    Invoke-LoggedStep -Name 'xelab' -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @('tb_c1_r1_resize_primitives', '-s', 'tb_c1_r1_resize_primitives_sim')
    Invoke-LoggedStep -Name 'xsim' -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_r1_resize_primitives_sim', '-runall')

    $manifest = Get-Content -Raw -LiteralPath `
        (Join-Path $runRoot 'c1_r1_resize_primitive_vectors.json') | ConvertFrom-Json
    $expectedPass = "C1_R1_RESIZE_PRIMITIVES_PASS configs=$($manifest.configs) " +
                    "requests=$($manifest.requests) interp=$($manifest.interpolation_vectors)"
