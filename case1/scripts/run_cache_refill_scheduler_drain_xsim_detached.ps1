...
# Detached, self-cleaning xsim runner for EMIT_DRAIN_WORDS=1.  The worker is
# created with Win32_Process.Create so the Vivado process is not tied to the
# interactive Windows job.  Only compact marker/tail logs survive cleanup.
...
if (-not $Worker) {
    if ([string]::IsNullOrWhiteSpace($RunId)) {
        $RunId = 'drain_' + (Get-Date -Format 'yyyyMMdd_HHmmss')
    }
    if ($RunId -notmatch '^[A-Za-z0-9_-]+$') { throw 'invalid RunId' }
    $statusPath = Join-Path $logRoot "xsim_runs\cache_refill_scheduler_drain\$RunId\status.json"
...
if ($RunId -notmatch '^[A-Za-z0-9_-]+$') { throw 'invalid RunId' }
$runRoot = Join-Path $simRoot "xsim_cache_refill_scheduler_drain_$RunId"
$runLogRoot = Join-Path $logRoot "xsim_runs\cache_refill_scheduler_drain\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'xsim_runs\cache_refill_scheduler_drain_status.json'
...
try {
    Write-Status 'running' 'setup' 0 'detached drain-stream worker started'
    Invoke-Step 'xvlog' (Join-Path $vivadoBin 'xvlog.bat') @(
        '-sv', '-nolog',
        (Join-Path $rtlRoot 'dma\c1_cache_refill_scheduler.sv'),
        (Join-Path $simRoot 'tb_c1_cache_refill_scheduler_drain.sv'))
    Invoke-Step 'xelab' (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_cache_refill_scheduler_drain', '-s',
        'tb_c1_cache_refill_scheduler_drain_sim', '-nolog')
    $marker = 'C1_CACHE_REFILL_SCHEDULER_DRAIN_PASS'
    Invoke-Step 'xsim' (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_cache_refill_scheduler_drain_sim', '-runall', '-nolog') $marker
    $watch.Stop()
