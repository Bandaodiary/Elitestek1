# C35 PW feeder test, serially after the already-queued row-shadow component.
# The waiting worker never kills, restarts or cleans its predecessor.
[CmdletBinding()]
param(
    [switch]$Worker,[switch]$CheckOnly,[string]$RunId='c35_pw_shadow_20260914_a',
    [ValidateRange(1,1800)][int]$TimeoutSeconds=300
)
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
if($RunId -notmatch '^[A-Za-z0-9_-]+$'){throw 'invalid RunId'}
$depRun='c35_row_shadow_20260914_a'
$depPid=40772;$depStart='2026-09-14T22:09:15.6290215+08:00'
$depStatus=Join-Path $caseRoot "logs\r2_row_shadow_runs\$depRun\status.json"
$simRoot=(Resolve-Path (Join-Path $caseRoot 'sim')).Path
$depPrivate=Join-Path $simRoot "c1_r2_row_shadow_$depRun"
$runRoot=Join-Path $simRoot "c1_r2_pw_shadow_$RunId"
$runLog=Join-Path $caseRoot "logs\r2_pw_shadow_runs\$RunId"
...
function Pin-Predecessor{
    ...
    if($p.ProcessName -ne 'powershell' -or $p.StartTime.ToUniversalTime().Ticks -ne ([DateTime]::Parse($depStart)).ToUniversalTime().Ticks){
        $p.Dispose()
        # A different start time proves this is NOT the predecessor. Accept
        # reuse only with the exact original run's complete/clean evidence;
        # never wait on or terminate the unrelated process now using its PID.
...
$state='waiting';$step='predecessor';$runExit=0;$message='waiting for exact C35 row-shadow worker exit'
...
public static class C35PwShadowJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $current=[Diagnostics.Process]::GetCurrentProcess();$inJob=$false
    if(-not [C35PwShadowJobCheck]::IsProcessInJob($current.Handle,[IntPtr]::Zero,[ref]$inJob)){throw 'cannot verify Windows Job'}
    ...
    $state='running';$step='component';$message='one isolated RAM-to-PW feeder paired regression';Save-State
    $info=New-Object Diagnostics.ProcessStartInfo
    $info.FileName='D:\miniconda\miniconda\envs\p300_task3_bci3\python.exe'
    $info.Arguments='-B -u "'+(Join-Path $caseRoot 'golden\run_r2_pw_shadow_probe.py')+'" --row-shadow-run '+$depRun+' --temporary-parent "'+$runRoot+'"'
    ...
    if($child.ExitCode -ne 0 -or $err.Trim() -or
       [regex]::Matches($out,'(?m)^C35_PW_SHADOW_FEEDER_PASS ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C35_ROW_SHADOW_EVIDENCE_PASS ').Count -ne 1 -or
       $out -notmatch '(?m)^C35_PW_SHADOW_CLEAN temporary_simulator_removed=1'){
        throw 'C35 component evidence incomplete or failed'
    }
    $state='complete';$step='done';$message='isolated RAM-to-PW feeder RTL passed; not full graph/PNR/fps evidence'
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or
       (Split-Path -Leaf $resolved) -ne "c1_r2_pw_shadow_$RunId"){throw 'unsafe own C35 cleanup target'}
