...
# Launch Vivado/xsim through a WMI-created worker so long simulations are not
...
if (-not $Worker) {
    ...
    [ordered]@{run_id=$RunId;worker_pid=[int]$result.ProcessId;
        status_path=(Join-Path $logRoot "native_capture_table_runs\$RunId\status.json")} |
        ConvertTo-Json
    return
}

if ($RunId -notmatch '^[A-Za-z0-9_-]+$') { throw 'bad RunId' }
$runRoot = Join-Path $simRoot "native_capture_table_run_$RunId"
$runLogRoot = Join-Path $logRoot "native_capture_table_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'native_capture_table_status.json'
...
function Write-Status([string]$State,[string]$Step,[int]$ExitCode,[string]$Message) {
    $value=[ordered]@{run_id=$RunId;frame='3x642x482_to_640x480';state=$State;step=$Step;
...
try {
    Write-Status running setup 0 'detached native capture/table preflight started'
    $packageSources=@(
    ...
    $sources=@($packageSources)+@($remaining)+@((Join-Path $simRoot 'tb_c1_r1_native_capture_table.sv'))
    Invoke-XsimStep xvlog (Join-Path $vivadoBin 'xvlog.bat') (@('-sv')+$sources)
    Invoke-XsimStep xelab (Join-Path $vivadoBin 'xelab.bat') @(
        'tb_c1_r1_native_capture_table','-s','tb_c1_r1_native_capture_table_sim')
    if ($CompileOnly) {
        $watch.Stop()
        Write-Status complete elaboration 0 'C1_R1_NATIVE_CAPTURE_TABLE_ELAB_PASS frame=3x642x482_to_640x480'
        return
    }
    Invoke-XsimStep xsim (Join-Path $vivadoBin 'xsim.bat') @(
        'tb_c1_r1_native_capture_table_sim','-runall') `
        'C1_R1_NATIVE_CAPTURE_TABLE_PASS'
    $watch.Stop()
    Write-Status complete done 0 'C1_R1_NATIVE_CAPTURE_TABLE_PASS frame=3x642x482_to_640x480'
} catch {
    $watch.Stop()
