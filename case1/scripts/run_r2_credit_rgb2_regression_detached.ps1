# C33 Icarus regressions survive a desktop turn interruption. No waves.
[CmdletBinding()]
param([switch]$Worker,[string]$RunId='', [ValidateRange(1,3)][int]$FrameDivisor=2,
      [ValidateSet('smoke','paired','matrix','variant','negative','faults')][string]$TestKind='matrix',
...
$runLog=Join-Path $caseRoot "logs\r2_credit_rgb2_regression_runs\$RunId"
...
$runRoot=Join-Path $simRoot "c1_r2_credit_rgb2_regression_$RunId"
...
$state='running';$step='prepare';$runExit=0;$message='C33 detached regression';$workerInJob=$null
...
public static class C33RegressionJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C33RegressionJobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'Cannot verify Windows Job isolation'}
    ...
    $scriptName=if($TestKind -eq 'faults'){'run_r2_credit_rgb2_host_fault_probe.py'}else{'run_r2_credit_rgb2_host_probe.py'}
    ...
    $prefix=if($TestKind -eq 'faults'){'C1_R2_CREDIT_RGB2_HOST_FAULT_'}else{'C1_R2_CREDIT_RGB2_HOST_SYSTEM_'}
    $expected=if($TestKind -eq 'startup'){2}elseif($TestKind -in @('smoke','paired')){1}elseif($TestKind -in @('faults','negative')){8}else{4}
    ...
    $lines=@(Get-Content -LiteralPath (Join-Path $runLog 'result.log')|Where-Object {$_ -cnotmatch '^C1_R2_CREDIT_RGB2_HOST_SYSTEM_VECTORS '})
    ...
    if($TestKind -eq 'startup'){
        $cleanCount=@($lines|Where-Object {$_ -eq 'C33_STARTUP_CLEAN temporary_vectors_and_simulator_removed=1'}).Count
        if(@($lines|Where-Object {$_ -match '^C33_STARTUP_NEGATIVE_PASS '}).Count -ne 1 -or @($lines|Where-Object {$_ -match '^C33_STARTUP_POSITIVE_PASS '}).Count -ne 2){throw 'startup clock/negative coverage incomplete'}
    }
    ...
    $state='complete';$step='done';$message='C33 regression passed'
} catch {$state='failed';$step='error';$runExit=1;$message=$_.Exception.Message}
finally {
    $resolved=[IO.Path]::GetFullPath($runRoot)
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_credit_rgb2_regression_$RunId"){throw 'unsafe cleanup target'}
    try {
