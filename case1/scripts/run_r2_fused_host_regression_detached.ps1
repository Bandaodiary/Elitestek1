# C35 actual AXI host regression after completed fused-stage recovery.
# The waiting worker never kills, restarts or cleans its predecessor.
[CmdletBinding()]
param(
    [switch]$Worker,[switch]$CheckOnly,[string]$RunId='c35_fused_host_regression_20260915_a',
    [ValidateRange(1,1800)][int]$TimeoutSeconds=1200
)
$ErrorActionPreference='Stop'
$caseRoot=(Resolve-Path (Join-Path $PSScriptRoot '..')).Path
if($RunId -notmatch '^[A-Za-z0-9_-]+$'){throw 'invalid RunId'}
$depRun='c35_row_fused_fault_20260914_b'
$depPid=44588;$depStart='2026-09-14T23:50:22.8167837+08:00'
$depStatus=Join-Path $caseRoot "logs\r2_row_fused_fault_runs\$depRun\status.json"
$simRoot=(Resolve-Path (Join-Path $caseRoot 'sim')).Path
$depPrivate=Join-Path $simRoot "c1_r2_row_fused_fault_$depRun"
$runRoot=Join-Path $simRoot "c1_r2_fused_regression_$RunId"
$runLog=Join-Path $caseRoot "logs\r2_fused_rgb2_regression_runs\$RunId"
...
function Save-State{
    [ordered]@{run_id=$RunId;state=$state;step=$step;exit_code=$runExit;message=$message;
        ...
        full_DAG=($state -eq 'complete');actual_AXI=($state -eq 'complete');native_fps_claim=$false;physical_RAM_measured=$false}|ConvertTo-Json -Depth 4|
...
public static class C35RegressionJobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $current=[Diagnostics.Process]::GetCurrentProcess();$inJob=$false
    if(-not [C35RegressionJobCheck]::IsProcessInJob($current.Handle,[IntPtr]::Zero,[ref]$inJob)){throw 'cannot verify Windows Job'}
    ...
    $state='running';$step='component';$message='fused AXI host original/variant/stall/negative matrix';Save-State
    $info=New-Object Diagnostics.ProcessStartInfo
    $info.FileName='D:\miniconda\miniconda\envs\p300_task3_bci3\python.exe'
    $info.Arguments='-B -u "'+(Join-Path $caseRoot 'golden\run_r2_fused_host_regression.py')+'" --fault-run '+$depRun+' --temporary-parent "'+$runRoot+'"'
    ...
    if($child.ExitCode -ne 0 -or $err.Trim() -or
       [regex]::Matches($out,'(?m)^C35_FUSED_FAULT_GATE_PASS ').Count -ne 1 -or
       [regex]::Matches($out,'(?m)^C1_R2_FUSED_RGB2_HOST_SYSTEM_PASS ').Count -ne 5 -or
       [regex]::Matches($out,'(?m)^C35_FUSED_HOST_REGRESSION_PASS ').Count -ne 1 -or
       $out -notmatch '(?m)^C1_R2_FUSED_RGB2_HOST_SYSTEM_CLEAN temporary_vectors_and_simulator_removed=1'){
        throw 'C35 full host regression evidence incomplete or failed'
    }
    $state='complete';$step='done';$message='fused AXI host matrix passed; not native fps/vendor CPU IP'
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or
       (Split-Path -Leaf $resolved) -ne "c1_r2_fused_regression_$RunId"){throw 'unsafe own C35 cleanup target'}
