<## WMI-detached C19 graph simulation. Only bounded logs survive; vectors,
...
param([switch]$Worker,[string]$RunId='', [int]$FrameWidth=12,[int]$FrameHeight=12,
      [ValidateSet('microstyle24')][string]$Profile='microstyle24',[int]$MemoryDiv=2,[int]$CommandLatency=20,[int]$Stalls=1,[int]$AwWaitW=2,[int]$NnTarget=4,[ValidateRange(1,4)][int]$FaultMode=4,[int]$TimeoutSeconds=600)
...
$runRoot=Join-Path $simRoot "c1_r2_host_recovery_xsim_$RunId"
...
$workerInJob=$null
$runState='running';$runStep='prepare';$runExit=0;$runMessage='detached C19 graph simulation'
function Save-State {
    [ordered]@{run_id=$RunId;state=$runState;step=$runStep;exit_code=$runExit;message=$runMessage;worker_pid=$PID;
        ...
        stalls=$Stalls;aw_wait_w=$AwWaitW;nn_target=$NnTarget;profile=$Profile;fault_mode=$FaultMode;simulator_directory_present=(Test-Path -LiteralPath $runRoot);worker_in_windows_job=$workerInJob}|ConvertTo-Json|Set-Content -LiteralPath $statusPath -Encoding UTF8
}
function Invoke-Step([string]$Name,[string]$Executable,[string[]]$Arguments) {
    ...
    try {
        ...
        if($Name -eq 'xsim') {
            $markers=@(Get-Content -LiteralPath $stdout | Where-Object {$_ -match '^C1_R2_HOST_RECOVERY_'})
            $markers|Set-Content -LiteralPath (Join-Path $runLog 'result.log') -Encoding UTF8
            if(@($markers|Where-Object {$_ -match '^C1_R2_HOST_RECOVERY_PASS '}).Count -ne 1 -or @($markers|Where-Object {$_ -match '^C1_R2_HOST_RECOVERY_RESULT '}).Count -ne 1){throw 'missing recovery pass/result marker'}
        }
    } finally {$p.Dispose()}
}
try {
...
public static class C19JobCheck {
    [DllImport("kernel32.dll",SetLastError=true)]
    public static extern bool IsProcessInJob(IntPtr process,IntPtr job,out bool result);
}
'@
    $jobValue=$false
    if(-not [C19JobCheck]::IsProcessInJob([Diagnostics.Process]::GetCurrentProcess().Handle,[IntPtr]::Zero,[ref]$jobValue)){throw 'Cannot verify Windows Job isolation'}
    ...
    [xml]$project=Get-Content -LiteralPath (Join-Path $caseRoot 'efinity\c1_ti60_r2_planned_host96.xml') -Raw
    $sources=@($project.SelectNodes("//*[local-name()='design_file']")|Where-Object {$_.name -ne 'c1_ti60_r2_planned_host96.sv' -and (Split-Path -Leaf $_.name) -ne 'execution_plan.sv'}|ForEach-Object {Join-Path (Join-Path $caseRoot 'efinity') $_.name})
    $sources += (Join-Path $runRoot 'package\execution_plan.sv')
    $top='tb_c1_r2_host_recovery_system';$bankWords=if($FrameWidth*$FrameHeight -gt 8192){524288}else{16384}
    $cycleLimit=3000000
    ...
    Invoke-Step 'xelab' (Join-Path $vivado 'xelab.bat') @($top,'-s','r2_host_recovery_sim','-generic_top',"BANK_WORDS=$bankWords",'-generic_top',"WIDTH=$FrameWidth",'-generic_top',"HEIGHT=$FrameHeight",'-generic_top',"MAX_CYCLES=$cycleLimit",'-generic_top',"NN_TARGET=$NnTarget",'-generic_top',"FAULT_MODE=$FaultMode",'-generic_top',"STAGE_COUNT=$($meta.stage_count)",'-generic_top',"RGB_STAGE=$($meta.rgb_stage)",'-generic_top',"MEMORY_DIV=$MemoryDiv",'-generic_top',"COMMAND_LATENCY=$CommandLatency",'-generic_top',"STALLS=$Stalls",'-generic_top',"AW_WAIT_W=$AwWaitW",'-mt','2')
    $vectorsPath=$runRoot.Replace('\','/')
    Invoke-Step 'xsim' (Join-Path $vivado 'xsim.bat') @('r2_host_recovery_sim','-runall','-testplusarg',"DIR=$vectorsPath",'-testplusarg',"P=$($meta.parameter_words)",'-testplusarg',"I=$($meta.input_words)",'-testplusarg',"E=$($meta.expected_words)")
    ...
    if(-not $resolved.StartsWith($simRoot.TrimEnd('\')+'\',[StringComparison]::OrdinalIgnoreCase) -or (Split-Path -Leaf $resolved) -ne "c1_r2_host_recovery_xsim_$RunId"){throw 'unsafe cleanup target'}
    try {
