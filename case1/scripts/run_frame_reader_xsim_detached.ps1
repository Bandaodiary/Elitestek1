...
# object.  Success requires exactly one unique PASS marker, no Fatal/Error/FAIL
# diagnostics, and an empty xsim stderr; process exit code zero is insufficient.
...
if (-not $Worker) {
    ...
    $statusPath = Join-Path $logRoot "frame_reader_runs\$RunId\status.json"
...
$runRoot = Join-Path $simRoot "xsim_frame_reader_run_$RunId"
$runLogRoot = Join-Path $logRoot "frame_reader_runs\$RunId"
$statusPath = Join-Path $runLogRoot 'status.json'
$latestStatusPath = Join-Path $logRoot 'frame_reader_status.json'
...
try {
    Write-Status -State 'running' -Step 'setup' -ExitCode 0 `
        -Message 'detached AXI frame-reader worker started'
    Invoke-VivadoStep -Name 'xvlog' -Tool (Join-Path $vivadoBin 'xvlog.bat') `
        -Arguments @(
            '-sv',
            (Join-Path $rtlRoot 'dma\c1_axi_xrgb_frame_reader.sv'),
            (Join-Path $simRoot 'tb_c1_axi_xrgb_frame_reader.sv')
        )
    Invoke-VivadoStep -Name 'xelab_frame_reader' -Tool (Join-Path $vivadoBin 'xelab.bat') `
        -Arguments @('tb_c1_axi_xrgb_frame_reader', '-s', 'tb_c1_axi_xrgb_frame_reader_sim')
    Invoke-VivadoStep -Name 'xsim_frame_reader' -Tool (Join-Path $vivadoBin 'xsim.bat') `
        -Arguments @('tb_c1_axi_xrgb_frame_reader_sim', '-runall') `
        -ExpectedPass 'C1_AXI_XRGB_FRAME_READER_PASS'
    $script:totalWatch.Stop()
    Write-Status -State 'complete' -Step 'done' -ExitCode 0 `
        -Message 'AXI XRGB frame-reader xsim regression complete'
} catch {
    $script:totalWatch.Stop()
